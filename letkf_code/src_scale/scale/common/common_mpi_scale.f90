...
!  call FILE_EXTERNAL_INPUT_CARTESC_setup
  ...
  ! setup nesting grid
  call COMM_CARTESC_NEST_setup ( QA_MP, QA_CH, QA_AE, ATMOS_PHY_MP_TYPE )
...
!  call MPI_BCAST(nlon*nlat, topo2d, MPI_r_size, mmean_rank_e, MPI_COMM_e, ierr)
!  write(6,*) "===CHECK=== set_common_mpi_grid  aft BCAST max min topo2d ",maxval(topo2d(:,:)) ,minval(topo2d(:,:))
...
subroutine monit_obs_mpi(v3dg, v2dg, monit_step)
  ...
  character(len=5) :: cmem

  if (.not. allocated(topo2d)) then
    allocate(topo2d(nlon,nlat))
    call read_topo(LETKF_TOPOGRAPHY_IN_BASENAME, topo2d)
!    call MPI_BCAST(topo2d,     nlon*nlat, MPI_r_size,  mmean_rank_e, MPI_COMM_e, ierr)
!    write(6,*) "topo2d BCAST. max min ",maxval(topo2d), minval(topo2d),"myrank_e=",myrank_e
  end if
  ...
  ! 
  if (myrank_e == mmean_rank_e .or. OBSDEP_OUT_ALLMEM ) then
    ...
    if (myrank_e == mmean_rank_e) then
      ...
      write(cmem,'(A,I4.4)') '_', myrank_e+1 
    endif 
    ...
    if (OBSDEP_OUT .and. monit_step == 2) then
      ...
      if (myrank_d == 0) then
        if ( OBSDEP_OUT_NC ) then
          ...
          call write_obs_dep_nc( trim(OBSDEP_OUT_BASENAME)//trim(cmem)//'.nc', &
          ...
          call write_obs_dep( trim(OBSDEP_OUT_BASENAME)//trim(cmem)//'.dat', &
                              obsdep_g_nobs, obsdep_g_set, &
                              obsdep_g_idx, obsdep_g_qc, &
  ...
  if (DEPARTURE_STAT_ALL_PROCESSES.and. (.not. OBSDEP_OUT_ALLMEM)) then
    call mpi_timer('', 2, barrier=MPI_COMM_e)
  ...
  if (DEPARTURE_STAT_ALL_PROCESSES .or. myrank_e == mmean_rank_e .or. OBSDEP_OUT_ALLMEM ) then
    if ( LOG_OUT ) then
...
subroutine write_ensmean(filename, v3d, v2d, calced, monit_step)
  ...
  if (OBSDEP_OUT_ALLMEM) then
    call gather_grd_mpi_alltoall(1, nens, v3d, v2d, v3dg, v2dg)
  else
