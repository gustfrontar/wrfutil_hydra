...
!-------------------------------------------------------------------------------
  ...
  if ( LOG_LEVEL >= 2 .and. LOG_OUT ) then
    ...
    do ictype = 1, nctype
      ...
      select case (ityp)
      case (12,22) ! vertical localization in Z *** RADARC and AWSARG
        write(vloc_show,'(F7.2,A4)') vert_loc_ctype(ictype)/1000.0d0, '[km]'
      case default ! vertical localization in ln(p)
        write(vloc_show,'(F6.2,A5)') vert_loc_ctype(ictype), '[lnp]'
      end select
      write (6, '(A6,1x,A3,2x,A3,F9.2,A11,F9.2,I9,F9.2,F12.2,F8.2)') obtypelist(ityp), obelmlist(ielm_u), use_obs_print, hori_loc_ctype(ictype)/1000.0d0, &
                  vloc_show, TIME_LOCAL(ityp)/1000.0d0, MAX_NOBS_PER_GRID(ityp), &
