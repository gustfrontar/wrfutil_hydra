...
!  else if (obtypelist(obtyp) == "RADARC" .or. obtypelist(obtyp) == "AWSARG") then !!! vloc in z 
  else if (obtypelist(obtyp) == "RADARC") then 
    nd_v = ABS(obs(obset)%lev(obidx) - rz) / vert_loc_ctype(ic)             ! for PHARAD and AWSARG, use z-coordinate for vertical localization (including Ps)
    ...
    if ( obtypelist(obtyp) == "AWSARG") then !!! AWSARG : lev in z and vloc in lnP
      !!! approximation using standard scale height 8.5km = 290K  
      nd_v = ABS(LOG(  1.0e5 * exp( - obs(obset)%lev(obidx) * 9.81/287.05/290.0 )  ) - LOG(rlev)) / vert_loc_ctype(ic)
    else !!! sonde : lev in Pa
