                       ...
                       float rtdat[ELDIM][AZDIM][RDIM])
{
  ...
  hd->altitude  = 0.01 *     char2int32(bufrdhd + 176 + 0)
    +             0.01 *     char2int32(bufrdhd + 180 + 0);
  ...
  hd->status    = char2int32(bufrdhd + 336);
  ...
  if(hd->status != 0){
     if(hd->status & (int)0x0001 > 0 || hd->status & (int)0x0002 > 0){
       printf("# abnormal radar status: %d!\n", hd->status);
       goto READERR;
     } else {
       printf("# radar status: %d, continue.\n", hd->status);
     }
  }
  ...
  for(j = 0; j < aznum; j++) {    /*= AZ loop uses j =*/
    ...
    for(k = 0; k < elnum; k++) {   /*= EL loop uses k =*/
      ...
      // no data beyond rnum; set mesh_offset as the missing value
      for(i = rnum; i < RDIM; i++)
        rtdat[k][j][i] = mesh_offset;
