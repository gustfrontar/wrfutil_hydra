...
res=$(cd $rundir && sbatch $scrpname 2>&1)
jobid=$(echo $res | cut -d ' ' -f4)
...
job_end_check_slurm () {
#-------------------------------------------------------------------------------
# Check if a slurm job has ended.
#
# Usage: job_end_check_slurm JOBID
#
#   JOBID  Job ID monitored
#
# * Do not support exit code yet
...
local tmp
while true; do
  tmp=$(squeue --job ${JOBID} 2> /dev/null)
  tmp=$(echo $tmp | grep ${JOBID})
  if (($? != 0)) || [ "$tmp" == "" ] ; then
    break
  fi
  sleep 5s
