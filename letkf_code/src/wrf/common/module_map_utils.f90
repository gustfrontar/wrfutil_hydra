...
!   use module_debug
  ...
  ! Mean Earth Radius in m.  The value below is consistent
   ...
   real(r_size), parameter :: A_WGS84  = 6378137.0d0
   real(r_size), parameter :: B_WGS84  = 6356752.314d0
   real(r_size), parameter :: RE_WGS84 = A_WGS84
   real(r_size), parameter :: E_WGS84  = 0.081819192d0

   real(r_size), parameter :: A_NAD83  = 6378137.0d0
   real(r_size), parameter :: RE_NAD83 = A_NAD83
   real(r_size), parameter :: E_NAD83  = 0.0818187034d0
   ...
   TYPE proj_info
      ...
      REAL(r_size)             :: phi      ! For Rotated Lat/Lon -- domain half-extent in 
                                   !  degrees latitude
      REAL(r_size)             :: lambda   ! For Rotated Lat/Lon -- domain half-extend in
                                  !  degrees longitude
      REAL(r_size)             :: lat1     ! SW latitude (1,1) in degrees (-90->90N)
      REAL(r_size)             :: lon1     ! SW longitude (1,1) in degrees (-180->180E)
      REAL(r_size)             :: lat0     ! For Cassini, latitude of projection pole
      REAL(r_size)             :: lon0     ! For Cassini, longitude of projection pole
      REAL(r_size)             :: dx       ! Grid spacing in meters at truelats, used
                                   !  only for ps, lc, and merc projections
      REAL(r_size)             :: dy       ! Grid spacing in meters at truelats, used
                                   !  only for ps, lc, and merc projections
      REAL(r_size)             :: latinc   ! Latitude increment for cylindrical lat/lon
      REAL(r_size)             :: loninc   ! Longitude increment for cylindrical lat/lon
                                   !  also the lon increment for Gaussian grid
      REAL(r_size)             :: dlat     ! Lat increment for lat/lon grids
      REAL(r_size)             :: dlon     ! Lon increment for lat/lon grids
      REAL(r_size)             :: stdlon   ! Longitude parallel to y-axis (-180->180E)
      REAL(r_size)             :: truelat1 ! First true latitude (all projections)
      REAL(r_size)             :: truelat2 ! Second true lat (LC only)
      REAL(r_size)             :: hemi     ! 1 for NH, -1 for SH
      REAL(r_size)             :: cone     ! Cone factor for LC projections
      REAL(r_size)             :: polei    ! Computed i-location of pole point
      REAL(r_size)             :: polej    ! Computed j-location of pole point
      REAL(r_size)             :: rsw      ! Computed radius to SW corner
      REAL(r_size)             :: rebydx   ! Earth radius divided by dx
      REAL(r_size)             :: knowni   ! X-location of known lat/lon
      REAL(r_size)             :: knownj   ! Y-location of known lat/lon
      REAL(r_size)             :: re_m     ! Radius of spherical earth, meters
      REAL(r_size)             :: rho0     ! For Albers equal area
      REAL(r_size)             :: nc       ! For Albers equal area
      REAL(r_size)             :: bigc     ! For Albers equal area
      ...
      REAL(r_size), POINTER, DIMENSION(:) :: gauss_lat  ! Latitude array for Gaussian grid
 ...
 !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   SUBROUTINE map_set(proj_code, proj, lat1, lon1, lat0, lon0, knowni, knownj, dx, dy, latinc, &
      ...
      REAL(r_size), INTENT(IN), OPTIONAL        :: latinc
      REAL(r_size), INTENT(IN), OPTIONAL        :: loninc
      REAL(r_size), INTENT(IN), OPTIONAL        :: lat1
      REAL(r_size), INTENT(IN), OPTIONAL        :: lon1
      REAL(r_size), INTENT(IN), OPTIONAL        :: lat0
      REAL(r_size), INTENT(IN), OPTIONAL        :: lon0
      REAL(r_size), INTENT(IN), OPTIONAL        :: dx
      REAL(r_size), INTENT(IN), OPTIONAL        :: dy
      REAL(r_size), INTENT(IN), OPTIONAL        :: stdlon
      REAL(r_size), INTENT(IN), OPTIONAL        :: truelat1
      REAL(r_size), INTENT(IN), OPTIONAL        :: truelat2
      REAL(r_size), INTENT(IN), OPTIONAL        :: knowni
      REAL(r_size), INTENT(IN), OPTIONAL        :: knownj
      REAL(r_size), INTENT(IN), OPTIONAL        :: phi
      REAL(r_size), INTENT(IN), OPTIONAL        :: lambda
      REAL(r_size), INTENT(IN), OPTIONAL        :: r_earth
      TYPE(proj_info), INTENT(OUT)      :: proj

      INTEGER :: iter
      REAL(r_size) :: dummy_lon1
      REAL(r_size) :: dummy_lon0
      REAL(r_size) :: dummy_stdlon
   ...
   SUBROUTINE latlon_to_ij(proj, lat, lon, i, j)
      ...
      REAL(r_size), INTENT(IN)                     :: lat
      REAL(r_size), INTENT(IN)                     :: lon
      REAL(r_size), INTENT(OUT)                    :: i
      REAL(r_size), INTENT(OUT)                    :: j
   ...
   SUBROUTINE ij_to_latlon(proj, i, j, lat, lon)
      ...
      IMPLICIT NONE
      TYPE(proj_info),INTENT(IN)          :: proj
      REAL(r_size), INTENT(IN)                    :: i
      REAL(r_size), INTENT(IN)                    :: j
      REAL(r_size), INTENT(OUT)                   :: lat
      REAL(r_size), INTENT(OUT)                   :: lon
   ...
   SUBROUTINE llij_ps(lat,lon,proj,i,j)
      ...
      REAL(r_size), INTENT(OUT)              :: i !(x-index)
      REAL(r_size), INTENT(OUT)              :: j !(y-index)
   ...
   SUBROUTINE ijll_ps(i, j, proj, lat, lon)
      ...
      REAL(r_size), INTENT(IN)                    :: i    ! Column
      REAL(r_size), INTENT(IN)                    :: j    ! Row
      TYPE (proj_info), INTENT(IN)        :: proj
      
      ! Declare output arguments
      REAL(r_size), INTENT(OUT)                   :: lat     ! -90 -> 90 north
      REAL(r_size), INTENT(OUT)                   :: lon     ! -180 -> 180 East
   ...
   SUBROUTINE set_ps_wgs84(proj)
      ...
      ! Local variables
      real(r_size) :: h, mc, tc, t, rho

      h = proj%hemi

      mc = cos(h*proj%truelat1*deg2rad)/sqrt(1.0-(E_WGS84*sin(h*proj%truelat1*deg2rad))**2.0)
      tc = sqrt(((1.0-sin(h*proj%truelat1*deg2rad))/(1.0+sin(h*proj%truelat1*deg2rad)))* &
                (((1.0+E_WGS84*sin(h*proj%truelat1*deg2rad))/(1.0-E_WGS84*sin(h*proj%truelat1*deg2rad)))**E_WGS84 ))

      ! Find the i/j location of reference lat/lon with respect to the pole of the projection
      t =sqrt(((1.0-sin(h*proj%lat1*deg2rad))/(1.0+sin(h*proj%lat1*deg2rad)))* &
               (((1.0+E_WGS84*sin(h*proj%lat1*deg2rad))/(1.0-E_WGS84*sin(h*proj%lat1*deg2rad)) )**E_WGS84 ) )
      rho = h * (A_WGS84 / proj%dx) * mc * t / tc
      proj%polei = rho * sin((h*proj%lon1 - h*proj%stdlon)*deg2rad)
      proj%polej = -rho * cos((h*proj%lon1 - h*proj%stdlon)*deg2rad)
   ...
   SUBROUTINE llij_ps_wgs84(lat,lon,proj,i,j)
      ...
      t = sqrt(((1.0-sin(h*lat*deg2rad))/(1.0+sin(h*lat*deg2rad))) * &
               (((1.0+E_WGS84*sin(h*lat*deg2rad))/(1.0-E_WGS84*sin(h*lat*deg2rad)))**E_WGS84))

      ! Find the x/y location of the requested lat/lon with respect to the pole of the projection
      rho = (A_WGS84 / proj%dx) * mc * t / tc
      i = h *  rho * sin((h*lon - h*proj%stdlon)*deg2rad)
      j = h *(-rho)* cos((h*lon - h*proj%stdlon)*deg2rad)
   ...
   SUBROUTINE ijll_ps_wgs84(i, j, proj, lat, lon)
      ...
      ! Local variables
      real(r_size) :: h, mc, tc, t, rho, x, y
      real(r_size) :: chi, a, b, c, d
      ...
      lon = h*proj%stdlon*deg2rad + h*atan2(h*x,h*(-y))
   ...
   SUBROUTINE set_albers_nad83(proj)
      ...
      ! Local variables
      real(r_size) :: h, m1, m2, q1, q2, theta, q, sinphi

      h = proj%hemi

      m1 = cos(h*proj%truelat1*deg2rad)/sqrt(1.0-(E_NAD83*sin(h*proj%truelat1*deg2rad))**2.0)
      m2 = cos(h*proj%truelat2*deg2rad)/sqrt(1.0-(E_NAD83*sin(h*proj%truelat2*deg2rad))**2.0)

      sinphi = sin(proj%truelat1*deg2rad)
      q1 = (1.0-E_NAD83**2.0) * &
           ((sinphi/(1.0-(E_NAD83*sinphi)**2.0)) - 1.0/(2.0*E_NAD83) * log((1.0-E_NAD83*sinphi)/(1.0+E_NAD83*sinphi)))

      sinphi = sin(proj%truelat2*deg2rad)
      q2 = (1.0-E_NAD83**2.0) * &
           ((sinphi/(1.0-(E_NAD83*sinphi)**2.0)) - 1.0/(2.0*E_NAD83) * log((1.0-E_NAD83*sinphi)/(1.0+E_NAD83*sinphi)))

      if (proj%truelat1 == proj%truelat2) then
         proj%nc = sin(proj%truelat1*deg2rad)
      else
      ...
      ! Find the i/j location of reference lat/lon with respect to the pole of the projection
      sinphi = sin(proj%lat1*deg2rad)
      q = (1.0-E_NAD83**2.0) * &
           ((sinphi/(1.0-(E_NAD83*sinphi)**2.0)) - 1.0/(2.0*E_NAD83) * log((1.0-E_NAD83*sinphi)/(1.0+E_NAD83*sinphi)))

      proj%rho0 = h * (A_NAD83 / proj%dx) * sqrt(proj%bigc - proj%nc * q) / proj%nc 
      theta = proj%nc*(proj%lon1 - proj%stdlon)*deg2rad
   ...
   SUBROUTINE llij_albers_nad83(lat,lon,proj,i,j)
      ...
      ! Local variables
      real(r_size) :: h, q, rho, theta, sinphi

      h = proj%hemi

      sinphi = sin(h*lat*deg2rad)
      ...
      theta = proj%nc * (h*lon - h*proj%stdlon)*deg2rad
   ...
   SUBROUTINE ijll_albers_nad83(i, j, proj, lat, lon)
      ...
      ! Local variables
      real(r_size) :: h, q, rho, theta, beta, x, y
      real(r_size) :: a, b, c
      ...
      lat = h*lat*rad2deg
      lon = proj%stdlon + theta*rad2deg/proj%nc
   ...
   SUBROUTINE set_lc(proj)
      ...
      REAL(r_size)                               :: arg
      REAL(r_size)                               :: deltalon1
      REAL(r_size)                               :: tl1r
      REAL(r_size)                               :: ctl1r
      REAL(r_sngl)                               :: tmptlat1,tmptlat2,tmpcone
  
      ! Compute cone factor
      tmptlat1=REAL(proj%truelat1,r_sngl)
      tmptlat2=REAL(proj%truelat2,r_sngl)
      CALL lc_cone(tmptlat1,tmptlat2,tmpcone)
      proj%cone=REAL(tmpcone,r_size)
      ...
      tl1r = proj%truelat1 * deg2rad
      ctl1r = COS(tl1r)
  
      ! Compute the radius to our known lower-left (SW) corner
      proj%rsw = proj%rebydx * ctl1r/proj%cone * &
             (TAN((90.0d0*proj%hemi-proj%lat1)*deg2rad/2.0d0) / &
              TAN((90.0d0*proj%hemi-proj%truelat1)*deg2rad/2.0d0))**proj%cone
  
      ! Find pole point
      arg = proj%cone*(deltalon1*deg2rad)
   ...
   ! Subroutine to compute the cone factor of a Lambert Conformal projection
      ...
      IF (ABS(truelat1-truelat2) .GT. 0.1) THEN
         cone = ALOG10(COS(truelat1*REAL(deg2rad,r_sngl))) - &
                ALOG10(COS(truelat2*REAL(deg2rad,r_sngl)))
         cone = cone /(ALOG10(TAN((45.0 - ABS(truelat1)/2.0) * REAL(deg2rad,r_sngl))) - &
                ALOG10(TAN((45.0 - ABS(truelat2)/2.0) * REAL(deg2rad,r_sngl))))
      ELSE
         cone = SIN(ABS(truelat1)*REAL(deg2rad,r_sngl) )
      ENDIF
   ...
   ! 
      ...
      ! Input Args
      REAL(r_size), INTENT(IN)              :: i        ! Cartesian X coordinate
      REAL(r_size), INTENT(IN)              :: j        ! Cartesian Y coordinate
      TYPE(proj_info),INTENT(IN)    :: proj     ! Projection info structure
  
      ! Output Args                 
      REAL(r_size), INTENT(OUT)             :: lat      ! Latitude (-90->90 deg N)
      REAL(r_size), INTENT(OUT)             :: lon      ! Longitude (-180->180 E)
  
      ! Locals 
      REAL(r_size)                          :: inew
      REAL(r_size)                          :: jnew
      REAL(r_size)                          :: r
      REAL(r_size)                          :: chi,chi1,chi2
      REAL(r_size)                          :: r2
      REAL(r_size)                          :: xx
      REAL(r_size)                          :: yy
      ...
      chi1 = (90. - proj%hemi*proj%truelat1)*deg2rad
      chi2 = (90. - proj%hemi*proj%truelat2)*deg2rad
      ...
      ELSE
         ...
         ! Longitude
         lon = proj%stdlon + rad2deg * ATAN2(proj%hemi*xx,yy)/proj%cone
         lon = DMOD(lon+360.0d0, 360.0d0)
         ...
         ENDIF
         lat = (90.0-chi*rad2deg)*proj%hemi
   ...
   ! to the cartesian x/y on a Lambert Conformal projection.
      ...
      ! Input Args
      REAL(r_size), INTENT(IN)              :: lat      ! Latitude (-90->90 deg N)
      REAL(r_size), INTENT(IN)              :: lon      ! Longitude (-180->180 E)
      TYPE(proj_info),INTENT(IN)      :: proj     ! Projection info structure
  
      ! Output Args                 
      REAL(r_size), INTENT(OUT)             :: i        ! Cartesian X coordinate
      REAL(r_size), INTENT(OUT)             :: j        ! Cartesian Y coordinate
  
      ! Locals 
      REAL(r_size)                          :: arg
      REAL(r_size)                          :: deltalon
      REAL(r_size)                          :: tl1r
      REAL(r_size)                          :: rm
      REAL(r_size)                          :: ctl1r
      ...
      rm = proj%rebydx * ctl1r/proj%cone * &
           (TAN((90.0d0*proj%hemi-lat)*deg2rad/2.0d0) / &
            TAN((90.0d0*proj%hemi-proj%truelat1)*deg2rad/2.0d0))**proj%cone
  
      arg = proj%cone*(deltalon*deg2rad)
   ...
   SUBROUTINE set_merc(proj)
      ...
      clain = COS(deg2rad*proj%truelat1)
      ...
      IF (proj%lat1 .NE. 0.) THEN
         proj%rsw = (ALOG(REAL(TAN(0.5*((proj%lat1+90.)*deg2rad)),r_sngl)))/proj%dlon
      ENDIF
   ...
   SUBROUTINE llij_merc(lat, lon, proj, i, j)
      ...
      IF (deltalon .LT. -180.) deltalon = deltalon + 360.
      IF (deltalon .GT. 180.) deltalon = deltalon - 360.
      i = proj%knowni + (deltalon/(proj%dlon*rad2deg))
      j = proj%knownj + (ALOG(REAL(TAN(0.5*((lat + 90.) * deg2rad)),r_sngl))) / &
             proj%dlon - proj%rsw
   ...
   SUBROUTINE ijll_merc(i, j, proj, lat, lon)
      ...
      lat = 2.0*ATAN(EXP(proj%dlon*(proj%rsw + j-proj%knownj)))*rad2deg - 90.
      lon = (i-proj%knowni)*proj%dlon*rad2deg + proj%lon1
      IF (lon.GT.180.) lon = lon - 360.
      IF (lon.LT.-180.) lon = lon + 360.
      RETURN
   ...
   SUBROUTINE llij_latlon(lat, lon, proj, i, j)
      ...
      if ( i <  real(proj%nxmin,r_size)-0.5d0 ) i = i + real(proj%nxmax - proj%nxmin + 1,r_size)
      if ( i >= real(proj%nxmax,r_size)+0.5d0 ) i = i - real(proj%nxmax - proj%nxmin + 1,r_size)
   ...
   SUBROUTINE ijll_latlon(i, j, proj, lat, lon)
      ...
      REAL(r_size)                         :: i_work, j_work
      REAL(r_size)                         :: deltalat
      REAL(r_size)                         :: deltalon

      i_work = i
      if ( i <  real(proj%nxmin,r_size)-0.5d0 ) i_work = i + real(proj%nxmax - proj%nxmin + 1,r_size)
      if ( i >= real(proj%nxmax,r_size)+0.5d0 ) i_work = i - real(proj%nxmax - proj%nxmin + 1,r_size)
   ...
   SUBROUTINE llij_cyl(lat, lon, proj, i, j)
      ...
      ! Arguments
      real(r_size), intent(in) :: lat, lon
      real(r_size), intent(out) :: i, j
      type(proj_info), intent(in) :: proj
...
!      deltalon = lon - proj%stdlon
   ...
   SUBROUTINE ijll_cyl(i, j, proj, lat, lon)
      ...
      ! Arguments
      real(r_size), intent(in) :: i, j
      real(r_size), intent(out) :: lat, lon
      type(proj_info), intent(in) :: proj
...
!      lon = deltalon + proj%stdlon
   ...
   SUBROUTINE rotate_coords(ilat,ilon,olat,olon,lat_np,lon_np,lon_0,direction)
      ...
      REAL(r_size), INTENT(IN   ) :: ilat, ilon
      REAL(r_size), INTENT(  OUT) :: olat, olon
      REAL(r_size), INTENT(IN   ) :: lat_np, lon_np, lon_0
      INTEGER, INTENT(IN  ), OPTIONAL :: direction
      ! >=0, default : computational -> geographical
      ! < 0          : geographical  -> computational

      REAL(r_size) :: rlat, rlon
      REAL(r_size) :: phi_np, lam_np, lam_0, dlam
      REAL(r_size) :: sinphi, cosphi, coslam, sinlam

      ! Convert all angles to radians
      phi_np = lat_np * deg2rad
      lam_np = lon_np * deg2rad
      lam_0  = lon_0  * deg2rad
      rlat = ilat * deg2rad
      rlon = ilon * deg2rad
      ...
      END IF
      olat = rad2deg*ASIN(sinphi)
      olon = rad2deg*(ATAN2(sinlam,coslam)-dlam-lam_0+lam_np)
   ...
   SUBROUTINE llij_rotlatlon(lat, lon, proj, i_real, j_real)
      ...
      REAL(r_size), INTENT(IN) :: lat, lon
      REAL(r_size)             :: i, j
      REAL(r_size), INTENT(OUT) :: i_real, j_real
      TYPE (proj_info), INTENT(IN) :: proj
      
      ! Local variables
      INTEGER :: ii,imt,jj,jmt,k,krows,ncol,nrow,iri
      REAL(r_size) :: dphd,dlmd !Grid increments, degrees
      REAL(r_size) :: glatd  !Geographic latitude, positive north
      REAL(r_size) :: glond  !Geographic longitude, positive west
      REAL(r_size) :: col,d1,d2,d2r,dlm,dlm1,dlm2,dph,glat,glon,    &
      ...
      dphd = proj%phi/REAL((proj%jydim-1)/2,r_size)
      dlmd = proj%lambda/REAL(proj%ixdim-1,r_size)
...
!!! Added next line as a Kludge - not yet understood why needed
   ...
   SUBROUTINE ijll_rotlatlon(i, j, proj, lat,lon)
      ...
      REAL(r_size) :: dphd,dlmd !Grid increments, degrees
      REAL(r_size) :: arg1,arg2,d2r,fctr,glatr,glatd,glond,pi, &
   ...
   SUBROUTINE gausll ( nlat , lat_sp )
      ...
      INTEGER                            :: nlat , i
!      REAL (r_size) , PARAMETER       :: pi = 3.141592653589793
      REAL (r_size) , DIMENSION(nlat) :: cosc , gwt , sinc , colat , wos2 , lat
      REAL(r_size)             , DIMENSION(nlat) :: lat_sp
   ...
   SUBROUTINE lggaus( nlat, cosc, gwt, sinc, colat, wos2 )
      ...
      REAL (r_size) , DIMENSION(nlat) :: cosc , gwt , sinc , colat  , wos2 
      REAL (r_size) , PARAMETER       :: pi = 3.141592653589793
 
      !  Convergence criterion for iteration of cos latitude
 
      REAL(r_size) , PARAMETER :: xlim  = 1.0E-14
 
      INTEGER :: nzero, i, j
      REAL (r_size) :: fi, fi1, a, b, g, gm, gp, gt, delta, c, d
   ...
   SUBROUTINE lgord( f, cosc, n )
      ...
      REAL (r_size) :: s1, c4, a, b, fk, f, cosc, colat, c1, fn, ang
      INTEGER :: n, k
