...
! REAL(r_dble) :: a8(n,n)
  REAL(r_dble), allocatable :: a8(:,:), a8_tmp(:,:)                             ! KK
  REAL(r_dble) :: eival8(n)
! REAL(r_dble) :: eivec8(n,n)
  REAL(r_dble), allocatable :: eivec8(:,:), eivec8_tmp(:,:)                     ! KK
  REAL(r_dble) :: wrk1(n)
  REAL(r_dble) :: wrk2(n)
  INTEGER :: ierr,i,j,k,kk
  integer :: nm, ny
  logical :: ext                                                         ! KK

  CALL Eigen_get_matdims(n, nm, ny)                                             ! KK
  allocate( a8(nm, ny), eivec8(nm, ny) )                                        ! KK
  a8(:,:) = 0.0d0                                                               ! KK
  a8(1:n, 1:n) = (a(1:n,1:n) + transpose(a(1:n,1:n)))*0.5d0                     ! KK

  eivec8 = 0.0d0
  eival8 = 0.0d0
  CALL Eigen_sx(n, n, a8, nm, eival8, eivec8, nm, m_forward=32, m_backward=128) ! KK
 !CALL rs(n,n,a8,eival8,imode,eivec8,wrk1,wrk2,ierr)                            ! KK
! IF( ierr/=0 ) THEN                                                            ! KK
!   WRITE(6,'(A,I)') '!!! ERROR (mtx_eigen): rs error code is ',ierr            ! KK
!   STOP 2                                                                      ! KK
! END IF                                                                        ! KK

  do kk = 1, n
    if( ieee_support_nan(eival8(kk)) ) then                                   ! KK
      if( ieee_is_nan(eival8(kk)) ) then                                      ! KK
        write(6, *) "1st EigenExa failed.", kk, eival8(kk)                    ! KK
        eivec8 = 0.0d0                                                        ! KK
        eival8 = 0.0d0                                                        ! KK
        a8     = 0.0d0                                                        ! KK
        a8(1:n, 1:n) = a(1:n,1:n)                                             ! KK
        CALL Eigen_sx(n,n,a8,nm,eival8,eivec8,nm,m_forward=32,m_backward=128) ! KK

        do k = 1, n                                                                   ! KK
          if( ieee_support_nan(eival8(k)) ) then                                      ! KK
            if( ieee_is_nan(eival8(k)) ) then                                         ! KK
              write(6, *) "2nd EigenExa failed.", k, eival8(k)                        ! KK
              open(10, file='cov_matrix.bin', form='unformatted', &         ! KK
                       access='direct', recl=8*n*n)                         ! KK
              write(10, rec=1) a(1:n, 1:n)                                  ! KK
              close(10)                                                     ! KK
              allocate(a8_tmp(n,n), eivec8_tmp(n,n))                                  ! KK
              do j = 1, n                                                             ! KK
!               write(6,*) j, eival8(j)
                do i = 1, n                                                           ! KK
                  a8_tmp(i,j) = (a(i,j) + a(j,i))*0.5d0                               ! KK
                end do                                                                ! KK
              end do                                                                  ! KK
              eivec8_tmp = 0.0d0                                                      ! KK
              CALL rs(n,n,a8_tmp,eival8,imode,eivec8_tmp,wrk1,wrk2,ierr)              ! KK
              eivec8(1:n, 1:n) = eivec8_tmp(1:n, 1:n)                                 ! KK
              deallocate(a8_tmp, eivec8_tmp)                                          ! KK
              IF( ierr/=0 ) THEN                                                      ! KK
                WRITE(6,'(A,I)') '!!! ERROR (mtx_eigen): rs error code is ',ierr      ! KK
                STOP 2                                                                ! KK
              END IF                                                                  ! KK
