...
!!      12th Conference on Mesoscale Processes, 6-9 August 2007, Waterville Valley conference &
!!      event center, Waterville Valley, NH, USA.
!! @li  Ogura, Y., and H. R. Cho, 1973:
...
!! which is operated by the University Corporation for Atmospheric Research (UCAR).
!! NCAR and UCAR make no proprietary claims, either statutory or otherwise,
!! to this version and release of WRF and consider WRF to be in the public domain
!! for use by any person or entity for any purpose without any fee or charge.
!! UCAR requests that any WRF user include this notice on any partial or full
!! copies of WRF. WRF is provided on an "AS IS" basis and any warranties,
!! either express or implied, including but not limited to implied warranties
!! of non-infringement, originality, merchantability and fitness for a particular
!! purpose, are disclaimed. In no event shall UCAR be liable for any damages,
!! whatsoever, whether direct, indirect, consequential or special, that arise
...
contains
  ...
  subroutine ATMOS_PHY_CP_kf_tendency( &
       ...
       cldfrac_dp,     &
       cldfrac_sh,     &
       flux_qr_total,  &        ! ### for SPRINTARS
       flux_qs_total   )        ! ### for SPRINTARS
    use scale_prc, only: &
       PRC_abort
    use scale_file_history, only: &
       FILE_HISTORY_query, &
    ...
    real(RP), intent(inout) :: DENS_t (KA,IA,JA)       !< tendency DENS [kg/m3/s]
    real(RP), intent(inout) :: RHOT_t (KA,IA,JA)       !< tendency RHOT [K*kg/m3/s]
    real(RP), intent(inout) :: RHOQV_t(KA,IA,JA)       !< tendency rho*QV [kg/kg/s]
    ...
    real(RP), intent(inout),optional :: flux_qr_total(KA,IA,JA) !< rain flux ### for SPRINTARS
    real(RP), intent(inout),optional :: flux_qs_total(KA,IA,JA) !< snow flux ### for SPRINTARS
    ...
    real(RP) :: qv_d(KA)                !< downdraft water vapor
    real(RP) :: rain_flux               !< rain precpitation flux
    real(RP) :: snow_flux               !< snow precpitation flux
    real(RP) :: prec_engi               !< precipitation ingernal energy

    ! update variables
    real(RP) :: theta_nw(KA)            !< new PT
    ...
    real(RP) :: dQV, dQC, dQI, dQR, dQS !< change of hydrometeors

    real(RP) :: tder_total(KA)  ! ### for SPRINTARS
    real(RP) :: tder_ref        ! ### for SPRINTARS
    real(RP) :: ddinc_total     ! ### for SPRINTARS
    integer  :: k_ldt_total     ! ### for SPRINTARS
    integer  :: k_ldb_total     ! ### for SPRINTARS
    ...
    !$omp parallel do default(none) schedule(dynamic) collapse(2) reduction(.or.:error) &
    ...
    !$omp         tder_total,tder_ref,ddinc_total,k_ldt_total,k_ldb_total) &
    ...
    !$omp        flux_qr_total,flux_qs_total)
    !$acc kernels
    !$acc loop collapse(2) reduction(.or.:error) independent &
    !$acc private(RHOD,tempv,qv_d,qc,qi,PSAT,QSAT,QV,rh, &
    !$acc         dens_nw,theta_nw,qv_nw,qc_nw,qi_nw,qr_nw,qs_nw, &
    !$acc         flux_qr,flux_qs,theta_eu,theta_ee,theta_d,umfnewdold,qvdet,qcdet,qidet, &
    !$acc         umf,umflcl,cape,presmix,upent,updet,temp_u,qv_u, &
    !$acc         dpthmx,zmix,rain_flux,snow_flux,prec_engi,time_advec, &
    !$acc         k_lcl,k_lc,k_lfs,k_pbl,k_top,k_let,k_ml, &
    !$acc         nic,deltap,cldfrac_KF,ems,emsd,wspd,dmf,downent,downdet, &
    !$acc         dQv,dQC,dQR,dQI,dQS, &
    !$acc         tder_total, &
    !$acc         work,work2,iwork)
    do j = JS, JE
    do i = IS, IE
       ...
       ! check convection
       if ( nca(i,j) .lt. 0.5_RP * KF_DTSEC ) then
       ...
       if (I_convflag(i,j) /= 2) then ! convection allowed I_convflag=0 or 1
          ...
          if( present(flux_qr_total) .and. present(flux_qs_total) ) then ! ### for SPRINTARS
             call CP_kf_downdraft ( &
                  ...
                  k_lfs,                                         & ! [OUT]
                  tder_total(:),                                 & ! [OUT] ### for SPRINTARS
                  tder_ref,                                      & ! [OUT] ### for SPRINTARS
                  ddinc_total,                                   & ! [OUT] ### for SPRINTARS
                  k_ldt_total,                                   & ! [OUT] ### for SPRINTARS
                  k_ldb_total                                    ) ! [OUT] ### for SPRINTARS
             ...
             call CP_kf_compensational ( &
                  ...
                  error,                                                        & ! [OUT]
                  flux_qr_total(:,i,j), flux_qs_total(:,i,j),                   & ! [OUT] ### for SPRINTARS
                  tder_total(:),                                                & ! [IN]  ### for SPRINTARS
                  tder_ref,                                                     & ! [IN]  ### for SPRINTARS
                  ddinc_total,                                                  & ! [IN]  ### for SPRINTARS
                  k_ldt_total,                                                  & ! [IN]  ### for SPRINTARS
                  k_ldb_total                                                   ) ! [IN]  ### for SPRINTARS
          else
             call CP_kf_downdraft ( &
                  KA, KS, KE,                                    & ! [IN]
                  I_convflag(i,j),                               & ! [IN]
       ...
       if (I_convflag(i,j) == 2) then ! no convection
          ...
          nca(i,j)             = -100.0_RP
          if( present(flux_qr_total) ) flux_qr_total(:,i,j) = 0.0_RP ! ### for SPRINTARS
          if( present(flux_qs_total) ) flux_qs_total(:,i,j) = 0.0_RP ! ### for SPRINTARS
...
!OCL SERIAL
  subroutine CP_kf_trigger ( &
...
#ifdef _OPENACC
    real(RP), intent(out) :: work(KA,8)
    integer,  intent(out) :: iwork(KA,1)
#endif
    ...
    integer,  intent(out) :: k_lc               !< index of LC
    integer,  intent(out) :: k_pbl              !< index of PBL
    integer,  intent(out) :: k_top              !< index of cloud top hight
    integer,  intent(out) :: k_let              !< index of LET
...
#ifdef _OPENACC
#define qrout_t(k) work(k,1)
#define qsout_t(k) work(k,2)
#else
    real(RP) :: qrout_t(KA)     !< rain
    real(RP) :: qsout_t(KA)     !< snow
#endif
...
!          w_cz = 0.02_RP*zlcl/2.e3_RP!
       ...
       if (temp_lcl + dtvv + dtrh <= temp_env + EPS*1e3_RP ) then ! kf triggerfunc dtrh is used @ NHM trigger func
          ...
          elseif (cloudhight > d_min .and. cape > 1._RP) then ! deep convection
             ...
             !> shallow convection after determine
             !! remember this layer (by virture of non-zero CLDHGT) as potential shallow-cloud layer
             !! determine shallow convection is after checked all potentially USL layer
             !< when no deep convection is found (this is determined first if block in do loop)
             I_convflag = 1
    ...
    if (k_let == k_top) then
       ...
       do kk = k_let+1,k_top
          ...
          if (kk >= k_let+2) then
             flux_qr(kk) = umf(kk-1)*qrout_t(kk)
             flux_qs(kk) = umf(kk-1)*qsout_t(kk)
          end if
          !
       end do
...
!OCL SERIAL
  subroutine CP_kf_updraft ( &
    ...
    integer,  intent(in) :: k_pbl               !< index of USL layer
    real(RP), intent(in) :: dz_kf(KA), z_kf(KA) !< delataz and z height
    real(RP), intent(in) :: w_lcl               !< vertical velocity of LCL layer
    real(RP), intent(in) :: temp_lcl            !< temperature of LCL layer
    real(RP), intent(in) :: tempv_lcl           !< vertural temperature of LCL layer
    real(RP), intent(in) :: pres_lcl            !< pressure of LCL layer
    real(RP), intent(in) :: qv_mix              !< water vapor of USL layer
    real(RP), intent(in) :: qv(KA)              !< water vapor
    real(RP), intent(in) :: temp(KA)            !< temperature
    real(RP), intent(in) :: tempv_env           !< environment vertual temperature
    real(RP), intent(in) :: zlcl                !< z[m]@LCL will be intent in
    real(RP), intent(in) :: pres(KA)            !< preassure [hPa]
    real(RP), intent(in) :: deltap(KA)          !< dp [pa]
    real(RP), intent(in) :: deltax              !< deltax
    real(RP), intent(in) :: radius              !< cloud radius
    ...
    integer  :: k_lclm1                         !< k_lcl -1
#ifdef _OPENACC
#define tempv_u(k)   work(k,1)
#define tempvq_uu(k) work(k,2)
#else
    real(RP) :: tempv_u(KA)                     !< updraft vertual temperature internalwork
    real(RP) :: tempvq_uu(KA)                   !< temperature vertial for updraft ,qv, qc, qi
#endif
    real(RP) :: denslcl                         !< density @LCL
    real(RP) :: ee1,ud1, ee2,ud2                !< entrainment and detrainment calc valiables
#ifdef _OPENACC
#define f_eq(k) work(k,3)
#else
    real(RP) :: f_eq(KA)                        !< CP_kf_prof5 variable
#endif
...
#ifdef _OPENACC
#define tempv_ud(k) work(k,4)
#else
    real(RP) :: tempv_ud(KA)                    !< virtual temperature  [K]
#endif
    real(RP) :: wtw                             !< w**2
...
#define wu(k) work(k,5)
    ...
    cape       = 0._RP
    do kk = KS, KE
       tempv_ud(kk)   = temp(kk) *( 1.0_RP + EPSTvap * qv(kk) ) ! vertual temperature
    end do
...
!                               temp_u(kkp1), theta_eu(kkp1), qv_u(kkp1), qi(kkp1) ) ! [OUT]
       ...
       if (kk == k_lclm1) then !! lcl layer exist  between kk and kk+1 layer then use interporate value
          boeff  = (tempv_lcl + tempv_u(kkp1))/(tempv_env + tempv_ud(kkp1)) - 1._RP
          boterm = 2._RP*(z_kf(kkp1) - zlcl)*GRAV*boeff/1.5_RP
          dztmp  = z_kf(kkp1) - zlcl
       else
          boeff  = (tempv_u(kk) + tempv_u(kkp1))/(tempv_ud(kk) + tempv_ud(kkp1)) - 1._RP
       ...
       !> CP_kf_precipitation_OC1973 or CP_kf_precipitation_Kessler
       !! in Ogura and Cho (1973), 'RATE' is nessesary but this rate is parameter in subroutine "CP_kf_precipitation_OC1973"
       ...
       ! calc cape
       tempvq_uu(kkp1) = temp_u(kkp1) * ( 1.0_RP + EPSTvap * qv_u(kkp1) - qc(kkp1) - qi(kkp1) )
       if (kk == k_lclm1) then!! lcl layer exist  between kk and kk+1 then use interporate value
          dilbe = ((tempv_lcl + tempvq_uu(kkp1))/(tempv_env + tempv_ud(kkp1)) - 1._RP)*dztmp
       else
          dilbe = ((tempvq_uu(kk) + tempvq_uu(kkp1))/(tempv_ud(kk) + tempv_ud(kkp1)) - 1._RP)*dztmp
       end if
       ...
       if(tempvq_uu(kkp1) <= tempv_ud(kkp1)) then ! if entrain and detrain
          ...
          ! TU95 in old coad
          if ( temp_u95 > tempv_ud(kkp1)) then ! few mix but bouyant then ! if95
             ...
             if (abs(temp_u10 - tempvq_uu(kkp1)) < 1.e-3_RP ) then !if10%
                ee2        = 1._RP ! all entrain
                ud2        = 0._RP
                f_eq(kkp1) = 1._RP
             else
                f_eq(kkp1) = (tempv_ud(kkp1) - tempvq_uu(kkp1))*f_mix1 &
                     &    /(temp_u10 - tempvq_uu(kkp1))
...
!OCL SERIAL
  subroutine CP_kf_downdraft ( &
       ...
       prec_engi,                              &
       k_lfs,                                  &
       tder_total,                             & ! ### for SPRINTARS
       tder_ref,                               & ! ### for SPRINTARS
       ddinc_total,                            & ! ### for SPRINTARS
       k_ldt_total,                            & ! ### for SPRINTARS
       k_ldb_total                             ) ! ### for SPRINTARS
    !$acc routine seq
    use scale_const,only :&
         PRE00   => CONST_PRE00,  &
         Rdry    => CONST_Rdry,   &
    ...
    real(RP), intent(in) :: theta_ee(KA)        !< environment equivalent theta
    real(RP), intent(in) :: umf(KA)             !< upward mass flux
    real(RP), intent(in) :: temp_u(KA)          !< updraft temperature
    real(RP), intent(in) :: flux_qr(KA)         !< rain fall
    ...
    real(RP), intent(out) :: rain_flux          !< rain precpitation flux
    real(RP), intent(out) :: snow_flux          !< snow precpitation flux
    real(RP), intent(out) :: prec_engi          !< precipiation internal energy
    integer,  intent(out) :: k_lfs              !< LFS layer index
    real(RP), intent(out),optional :: tder_total(KA)  ! ### for SPRINTARS
    real(RP), intent(out),optional :: tder_ref        ! ### for SPRINTARS
    real(RP), intent(out),optional :: ddinc_total     ! ### for SPRINTARS
    integer,  intent(out),optional :: k_ldt_total     ! ### for SPRINTARS
    integer,  intent(out),optional :: k_ldb_total     ! ### for SPRINTARS

    integer  :: kk, kkp1                        !< loop index
    ...
    real(RP) :: dssdt                           !< tempolaly variable for humidity adjustment
    real(RP) :: dtmp                            !< tempolaly variable for humidity adjustment
    real(RP) :: qsrh                            !< tempolaly variable for humidity adjustment
    real(RP) :: RL                              !< tempolaly variable for humidity adjustment
    real(RP) :: T1rh                            !< tempolaly variable for humidity adjustment
    real(RP) :: dpthtmp                         !< temporaly depth of downdraft source layer
    real(RP) :: dpthdet                         !< downdraft detrainment  depth (Pa)
#ifdef _OPENACC
#define temp_d(k)   work(k,1)
#define tempv_d(k)  work(k,2)
#define theta_ed(k) work(k,3)
#define qvsd(k)     work(k,4)
#else
...
#ifdef _OPENACC
#define iexn_d(k) work(k,5)
#else
    real(RP) :: iexn_d(KA)                      !< inverse of exner function
#endif
    real(RP) :: f_dmf                           !< factor of dmf Kain(2004) eq.11
    real(RP) :: dq                              !< heat
    real(RP) :: es                              !< saturate vapor pressure
    real(RP) :: ddinc                           !< downdraft ??atode
    real(RP) :: total_prec                      !< rain or snow
    real(RP) :: total_rain                      !< rain
    ...
    prec_engi  = 0._RP
    k_lfs      = 0
    if( present(tder_total) ) then ! ### for SPRINTARS
       tder_total(:) = 0.0_RP      ! ### for SPRINTARS
       ddinc_total = 0.0_RP        ! ### for SPRINTARS
       k_ldt_total = 0             ! ### for SPRINTARS
       k_ldb_total = 0             ! ### for SPRINTARS
    endif
    ...
    if(I_convflag == 1) then ! shallow convection
       ...
       if(pres(k_dstart) - pres(k_lfs) > 50.e2_RP) then   ! LFS > 50mb(minimum of downdraft source layer)
          ...
          call CP_kf_calciexn( pres(k_lfs), qvs_tmp, & ! [IN]
                               iexn_d(k_lfs)         ) ! [OUT]
          theta_d(k_lfs) = temp_d(k_lfs) * iexn_d(k_lfs)
          ...
          if( present(k_ldt_total) ) k_ldt_total = k_ldt  ! ### for SPRINTARS
          if( present(k_ldb_total) ) k_ldb_total = k_ldb  ! ### for SPRINTARS
          if( (pres(k_ldb)-pres(k_lfs)) > 50.e2_RP ) then ! minimum downdraft depth !
             do kk = k_ldt,k_ldb,-1
                ...
                if( present(tder_total) ) then
                   tder_total(kk) = (qvsd(kk) - qv_d(kk))*downdet(kk) ! ### for SPRINTARS
                endif
                qv_d(kk)    = qvsd(kk)
                call CP_kf_calciexn( pres(kk), qv_d(kk), & ! [IN]
                                     iexn_d(kk)          ) ! [OUT]
                theta_d(kk) = temp_d(kk)*iexn_d(kk)
             end do
          end if
       end if ! LFS>50mb
    end if ! down devap
    if( present(tder_ref) ) tder_ref = tder ! ### for SPRINTARS
    !!
    ...
    if (tder < 1._RP) then ! dmf modify
       ...
       if( present(ddinc_total) ) ddinc_total = ddinc ! ### for SPRINTARS
       do kk = k_ldb,k_lfs
          dmf(kk)     = dmf(kk)*ddinc
...
!OCL SERIAL
  subroutine CP_kf_compensational (&
       ...
       error,                               &
       flux_qr_total, flux_qs_total,        & ! ### for SPRINTARS
       tder_total,                          & ! ### for SPRINTARS
       tder_ref,                            & ! ### for SPRINTARS
       ddinc_total,                         & ! ### for SPRINTARS
       k_ldt_total,                         & ! ### for SPRINTARS
    ...
    integer,  intent(in)    :: k_pbl              !< index at pbl
    integer,  intent(in)    :: k_ml               !< index at melting layer
    integer,  intent(in)    :: k_lfs              !< index at lfs
    ...
    real(RP), intent(in)    :: cape               !< estimated cape
    real(RP), intent(in)    :: temp_u(KA)         !< temp of updraft
    real(RP), intent(in)    :: qvdet(KA)          !< detrainment qv
    ...
    real(RP), intent(in)    :: flux_qr(KA)        !< rain flux
    real(RP), intent(in)    :: flux_qs(KA)        !< snow flux
    ...
    real(RP), intent(in)    :: prec_engi          !< precipitation internal energy
    real(RP), intent(in)    :: KF_DTSEC           !< time step
    real(RP), intent(in)    :: RHOD(KA)
    integer,  intent(in)    :: i, j

#ifdef _OPENACC
    real(RP), intent(out)   :: work(KA,24)
    real(RP), intent(out)   :: work2(0:KA,7)
#endif
    ...
    real(RP), intent(inout) :: rain_flux          !< rain precipitation
    real(RP), intent(inout) :: snow_flux          !< snow precipitation
    ...
    real(RP), intent(out)   :: theta_nw(KA)       !< theta after timestep
    real(RP), intent(out)   :: qv_g(KA)           !< qv after timestep
    real(RP), intent(out)   :: qc_nw(KA)          !< qc after timestep
    real(RP), intent(out)   :: qi_nw(KA)          !< qi after timestep
    real(RP), intent(out)   :: qr_nw(KA)          !< qr after timestep
    real(RP), intent(out)   :: qs_nw(KA)          !< qs after timestep
    real(RP), intent(out)   :: sflx_rain          !< convective rain rate
    real(RP), intent(out)   :: sflx_snow          !< convective rain rate
    real(RP), intent(out)   :: sflx_engi          !< convective rain rate
    ...
    logical,  intent(out)   :: error
    ! ### for SPRINTARS
    real(RP), intent(out), optional  :: flux_qr_total(KA)  !< rain flux
    real(RP), intent(out), optional  :: flux_qs_total(KA)  !< snow flux
    real(RP), intent(in) , optional  :: tder_total(KA)     !< evaporation
    real(RP), intent(in) , optional  :: tder_ref           !< evaporation
    real(RP), intent(in) , optional  :: ddinc_total        !
    integer,  intent(in) , optional  :: k_ldt_total        !
    integer,  intent(in) , optional  :: k_ldb_total        !
    real(RP)                         :: tder_sum           !
    integer                          :: k_ml_sum           !
    integer                          :: k_lcl_total        !
    real(RP)                         :: flux_prec_total1   !
    real(RP)                         :: flux_prec_total2   !
    ...
    integer  :: kk,kkp1                           !< index of do loop (temporaly vars)
    integer  :: k_lmax                            !< max of k_lcl,k_lfs (temporaly vars)
    integer  :: k_lcl, k_lclm1                    !< LCL and LCL-1 layer index used calucrat CAPE

#ifdef _OPENACC
#define umf2(k)     work(k,1)
#define dmf2(k)     work(k,2)
#define upent2(k)   work(k,3)
#define updet2(k)   work(k,4)
#define qcdet2(k)   work(k,5)
#define qidet2(k)   work(k,6)
#define downent2(k) work(k,7)
#define downdet2(k) work(k,8)
#else
    real(RP) :: umf2(KA), dmf2(KA)                !< UMF and DMF
    real(RP) :: upent2(KA), updet2(KA)            !< updraft entrainment and detrainment
    real(RP) :: qcdet2(KA), qidet2(KA)            !< updraft detrainment qc and qi
    real(RP) :: downent2(KA), downdet2(KA)        !< downdraft entrainment and detrainment
#endif
    real(RP) :: rain_flux2                        !< rain precpitation flux
    real(RP) :: snow_flux2                        !< snow precpitation flux
    real(RP) :: tkemax                            !< tkemax tuning prameter
    real(RP) :: z_lcl                             !< lcl layer hight
#ifdef _OPENACC
#define theta(k)    work(k,9)
#define theta_u(k)  work(k,10)
#define theta_eu(k) work(k,11)
#define theta_eg(k) work(k,12)
#define iexn_c(k)   work(k,13)
#else
    ...
    real(RP) :: iexn_c(KA)                        !< inverse of exner function
#endif
    real(RP) :: qv_env                            !< environment qv
    real(RP) :: qv_mix                            !< USL layer mean  qv
#ifdef _OPENACC
#define qv_gu(k)  work(k,14)
#define temp_g(k) work(k,15)
#else
    real(RP) :: qv_gu(KA)                         !< updraft qv (used calc CAPE)
    real(RP) :: temp_g(KA)                        !< temperature of new valuavles after timestep
#endif
...
#ifdef _OPENACC
#define temp_gu(k)   work(k,16)
#define tempv_g(k)   work(k,17)
#define tempvq_uc(k) work(k,18)
#else
    real(RP) :: temp_gu(KA)                       !< temporarly updraft temperature
    real(RP) :: tempv_g(KA)                       !< temporaly virtual
    real(RP) :: tempvq_uc(KA)                     !< temporaly virtual
#endif
    real(RP) :: es                                !< saturate vapor pressure
    real(RP) :: qvss                              !< saturate vapor pressure mixingratio
    real(RP) :: DQ, TDPT, DSSDT, emix, RL, TLOG   !< calc dew point temperature etc.
    real(RP) :: vconv
...
#ifdef _OPENACC
#define theta_g(k) work(k,19)
#define qv_nw(k)   work(k,20)
#else
...
#ifdef _OPENACC
#define domg_dp(k) work(k,22)
#else
    real(RP) :: domg_dp(KA)                       !< d omega/dp (temporaly vars)
#endif
    real(RP) :: absomgtc,absomg                   !< (temporaly vars)
    real(RP) :: f_dp                              !< (temporaly vars)

#ifdef _OPENACC
#define theta_fx(k) work2(k,2)
#define qv_fx(k)    work2(k,3)
#define qc_fx(k)    work2(k,4)
#define qi_fx(k)    work2(k,5)
#define qr_fx(k)    work2(k,6)
#define qs_fx(k)    work2(k,7)
#define rainfb(k)   work(k,23)
#define snowfb(k)   work(k,24)
#else
    ...
    k_lcl = k_lcl_bf
    if( present(k_ldt_total) ) k_lcl_total = k_lcl_bf                    ! ### for SPRINTARS
    ...
    do kk = KS,k_top
       call CP_kf_calciexn( pres(kk), qv(kk), & ! [IN]
                            iexn_c(kk)        ) ! [OUT]
       theta(kk) = temp_bf(kk) * iexn_c(kk)
       call CP_kf_calciexn( pres(kk), qvdet(kk), & ! [IN]
                            iexn_c(kk)           ) ! [OUT]
       theta_u(kk) = temp_u(kk) * iexn_c(kk)
    end do
...
!OCL NOEVAL
    ...
    do ncount = 1,10 ! iteration
       ...
       do kk = KS,k_top
          call CP_kf_calciexn( pres(kk), qv_g(kk), & ! [IN]
                               iexn_c(kk)          ) ! [OUT]
          temp_g(kk)  = theta_g(kk) / iexn_c(kk)
       ...
       do kk=k_lclm1,k_top-1 ! LTOPM1
          ...
          tempvq_uc(kkp1) = temp_gu(kkp1) * ( 1.0_RP + EPSTvap * qv_gu(kkp1) - qc(kkp1)- qi(kkp1) )
          if(kk == k_lclm1) then !  interporate
             dzz = z_kf(k_lcl) - z_lcl
             dilbe = ((tempv_lcl + tempvq_uc(kkp1))/(tempv_env + tempv_g(kkp1)) - 1._RP)*dzz
          else
             dzz = dz_kf(kk)
             dilbe = ((tempvq_uc(kk) + tempvq_uc(kkp1))/(tempv_g(kk) + tempv_g(kkp1)) - 1._RP)*dzz
          end if
    ...
    ! ### for SPRINTARS
    if( present(flux_qr_total) .and. present(flux_qs_total) ) then
       flux_qr_total(:) = 0.0_RP
       flux_qs_total(:) = 0.0_RP
       if ( k_ml == KS ) then
          k_ml_sum = KS-1
       else
          k_ml_sum = k_ml
       endif
       do kk = k_top-1, KS-1, -1 ! ### for SPRINTARS
          ! if ( kk < k_ml ) then  ! below melting layer
          !    if ( k_ldt_total > k_ldb_total ) then
          !       if ( tder_total(kk+1) < 1.0_RP / real(k_ldt_total-k_ldb_total,kind=RP) ) then
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qs_total(kk+1) + ( flux_qr(kk+1)                                  ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !          flux_qs_total(kk) = 0.0_RP
          !       else
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qs_total(kk+1) + ( flux_qr(kk+1) - tder_total(kk+1) * ddinc_total ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !          flux_qs_total(kk) = 0.0_RP
          !       endif
          !    else
          !       flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qs_total(kk+1) + ( flux_qr(kk+1) - tder_total(kk+1) * ddinc_total ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          ...
          !    if ( k_ldt_total > k_ldb_total ) then
          !       if ( tder_total(kk+1) < 1.0_RP / real(k_ldt_total-k_ldb_total,kind=RP) ) then
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + ( flux_qr(kk+1)                                  ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !          flux_qs_total(kk) = flux_qs_total(kk+1) + ( flux_qs(kk+1)                                  ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !       else
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + ( flux_qr(kk+1)                                  ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !          flux_qs_total(kk) = flux_qs_total(kk+1) + ( flux_qs(kk+1) - tder_total(kk+1) * ddinc_total ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !       endif
          !    else
          !       flux_qr_total(kk) = flux_qr_total(kk+1) + ( flux_qr(kk+1)                                  ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !       flux_qs_total(kk) = flux_qs_total(kk+1) + ( flux_qs(kk+1) - tder_total(kk+1) * ddinc_total ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          ...
          ! if ( tder_ref < 1.0_RP ) then
          !    if ( kk >= k_lcl_total ) then
          !       if ( kk < k_ml_sum ) then    ! below melting layer
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qs_total(kk+1) + ( flux_qr(kk+1) + flux_qs(kk+1) ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !          flux_qs_total(kk) = 0.0_RP
          !       else                         ! above melting layer
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qr(kk+1) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !          flux_qs_total(kk) = flux_qs_total(kk+1) + flux_qs(kk+1) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          ...
          !    else
          !       if ( kk < k_ml_sum ) then    ! below melting layer
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qs_total(kk+1)
          !          flux_qs_total(kk) = 0.0_RP
          !       else                         ! above melting layer
          !          flux_qr_total(kk) = flux_qr_total(kk+1)
          !          flux_qs_total(kk) = flux_qs_total(kk+1)
          !       endif
          !    endif
          ! else
          !    if ( kk >= k_lcl_total ) then
          !       if ( kk < k_ml_sum ) then    ! below melting layer
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qs_total(kk+1) + ( flux_qr(kk+1) + flux_qs(kk+1) - tder_total(kk+1) * ddinc_total ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !          flux_qs_total(kk) = 0.0_RP
          !       else                         ! above melting layer
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + ( flux_qr(kk+1)                                  ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !          flux_qs_total(kk) = flux_qs_total(kk+1) + ( flux_qs(kk+1) - tder_total(kk+1) * ddinc_total ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          ...
          !    else
          !       if ( kk < k_ml_sum ) then    ! below melting layer
          !          flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qs_total(kk+1) - tder_total(kk+1) * ddinc_total * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !          flux_qs_total(kk) = 0.0_RP
          !       else                         ! above melting layer
          !          flux_qr_total(kk) = flux_qr_total(kk+1)
          !          flux_qs_total(kk) = flux_qs_total(kk+1) - tder_total(kk+1) * ddinc_total * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
          !       endif
          !    endif
          ! endif
          if ( tder_ref < 1.0_RP ) then
             ...
             if ( kk >= k_lcl_total ) then
                if ( kk < k_ml_sum ) then    ! below melting layer
                   flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qs_total(kk+1) + ( flux_qs(kk+1) + flux_qr(kk+1) - tder_total(kk+1) * ddinc_total ) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
                   flux_qs_total(kk) = 0.0_RP
                else                         ! above melting layer
                   flux_qr_total(kk) = flux_qr_total(kk+1) + flux_qr(kk+1) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
                   flux_qs_total(kk) = flux_qs_total(kk+1) + flux_qs(kk+1) * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
                   flux_prec_total1  = flux_qr_total(kk) + flux_qs_total(kk)
                   flux_prec_total2  = flux_prec_total1 - tder_total(kk+1) * ddinc_total  * aincfin / (deltax**2) * ( 1.0_RP - fbfrc )
                   flux_prec_total2  = max( flux_prec_total2, 0.0_RP )
                   if ( flux_prec_total1 > 0.0_RP ) then
                      flux_qr_total(kk) = flux_prec_total2 * flux_qr_total(kk) / flux_prec_total1
                      flux_qs_total(kk) = flux_prec_total2 * flux_qs_total(kk) / flux_prec_total1
                   else
                      flux_qr_total(kk) = 0.0_RP
                      flux_qs_total(kk) = 0.0_RP
                   endif
                endif
             else
          ...
          flux_qr_total(kk) = max( flux_qr_total(kk), 0.0_RP )
          flux_qs_total(kk) = max( flux_qs_total(kk), 0.0_RP )
       enddo
    endif
    ...
    !< calc new theta (SCALE theta) then return tendency
    if (WARMRAIN) then
       !!
       !...IF ICE PHASE IS NOT ALLOWED, MELT ALL FROZEN HYDROMETEORS...
       !!
       do kk = KS,KE
          theta_nw(kk)  = theta_nw(kk) - EMELT * ( qi_nw(kk) + qs_nw(kk) ) * RHOD(kk) / ( CPdry + ( CPvap - CPdry ) * qv_g(kk) ) * iexn_c(kk)
...
!!$       end if
  ...
  subroutine CP_kf_precipitation_OC1973( &
    ...
    real(RP), intent(in)    :: BOTERM   !< bouyancy term
    real(RP), intent(in)    :: ENTERM   !< entrainment term
    real(RP), intent(inout) :: QLQOUT   !< q-liquid which is goint out
    real(RP), intent(inout) :: QICOUT   !< q-ice which is goint out
    real(RP), intent(inout) :: WTW      !< estimated vertical velocity
    real(RP), intent(inout) :: QLIQ     !< q-liquid
    real(RP), intent(inout) :: QICE     !< q-ice
    real(RP), intent(inout) :: QNEWLQ   !< newly calculated q-liquid
    real(RP), intent(inout) :: QNEWIC   !< newly calculated q-ice
  ...
  !------------------------------------------------------------------------------
  !> CP_kf_precipitation_Kessler
  !! calculate precipitain fallout by Kessler type auto conversion
  !<
...
!    real(RP), intent(inout) :: TU, THTEU, QU, QICE
  ...
  !------------------------------------------------------------------------------
  !> CP_kf_prof5
  !! calculate gaussian type mixing profile
