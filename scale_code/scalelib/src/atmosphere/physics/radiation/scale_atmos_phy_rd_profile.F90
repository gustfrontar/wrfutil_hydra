...
contains
  ...
  subroutine PROFILE_read_user( &
    ...
    open( unit   = fid,         &
       ...
       do k = 1, USER_klim
          read(fid,*,iostat=ierr) USER_z(k), USER_pres(k), USER_temp(k), USER_qv(k), USER_o3(k), USER_co2(k)
          if ( ierr /= 0 ) exit
       enddo
       USER_kmax = k - 1
    close(fid)
    ...
    do k = 1, USER_kmax
       work(k) = USER_co2(k) / 44.0_RP * Mdry / PPM ! [kg/kg->PPM]
    enddo
