...
! namelist variables
  NAMELIST / PARAM_ATMOS_PHY_MP_SDM / &
    ...
    LOG_NEWLINE
    LOG_INFO("ATMOS_PHY_MP_sdn_setup",*) 'Setup'
...
!       if( mype == PRC_master )  write(*,*) "sdm_zupper was set to minval(REAL_FZ(KE)) because zupper > minval(REAL_FZ(KE))"
  ...
  subroutine ATMOS_PHY_MP_sdm_adjustment(  &
    ...
    if( sd_first ) then
      sd_first = .false.
      LOG_INFO("ATMOS_PHY_MP_sdm_adjustment",*) 'S.D.: setup'
      if( sd_rest_flg_in ) then
         ...
         call sdm_sd2qcqr(DENS,QHYD_sdm(:,:,:,I_QHC_sdm),QHYD_sdm(:,:,:,I_QHR_sdm),          &
                       zph_crs,                   &
         ...
         if( sdm_cold ) then
            call sdm_sd2qiqsqg(DENS,QHYD_sdm(:,:,:,I_QHI_sdm),QHYD_sdm(:,:,:,I_QHS_sdm),QHYD_sdm(:,:,:,I_QHG_sdm),        &
                      zph_crs,                                               &
                      sdnum_s2c,sdn_s2c,sdliqice_s2c, sdx_s2c,sdy_s2c,                     &
    ...
    if( (mod(sdm_dmpvar,10)==1) .and. sdm_dmpitva>0.0_RP .and. &
       ...
       LOG_INFO("ATMOS_PHY_MP_sdm_adjustment",*) 'Output Super Droplet Data in ASCII'
    ...
    if( ((mod(sdm_dmpvar,100))/10>=1) .and. sdm_dmpitvb>0.0_RP .and. &
       ...
       LOG_INFO("ATMOS_PHY_MP_sdm_adjustment",*) 'Output Super Droplet Data in NetCDF'
       !! Evaluate diagnostic variables
       !!! z
       call sdm_rk2z(sdnum_s2c,sdx_s2c,sdy_s2c,sdrk_s2c,sdz_s2c,sdri_s2c,sdrj_s2c)
       !!! terminal velocity vz
    ...
    if( ((mod(sdm_dmpvar,1000))/100>=1) .and. sdm_dmpitvl>0.0_RP .and. &
       ...
       LOG_INFO("ATMOS_PHY_MP_sdm_adjustment",*) ' *** Output Selected Super Droplet Data in NetCDF'
...
!!$    call FILE_HISTORY_in( rhoa_sdm(:,:,:), 'RAERO', 'aerosol mass conc.', 'kg/m3', dt)
...
!    enddo
  ...
  subroutine ATMOS_PHY_MP_sdm_qtrc2qhyd( &
    ...
    if ( .not. allocated(QHYD_sdm) ) then
       ! for scale_init
       !$omp workshare
       Qe(:,:,:,:) = 0.0_RP
       !$omp end workshare
