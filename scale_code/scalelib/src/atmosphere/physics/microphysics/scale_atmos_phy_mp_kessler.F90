...
contains
  ...
  subroutine ATMOS_PHY_MP_kessler_adjustment( &
    ...
    integer  :: k, i, j
    !---------------------------------------------------------------------------
    !$acc data copyin(DENS,PRES) &
    !$acc      copy(TEMP,QTRC,CPtot,CVtot) &
    !$acc      copyout(RHOE_t,EVAPORATE) &
    !$acc      create(QTRC_dummy,RHOE_d_sat,QC_t_sat)
...
!OCL XFILL
  ...
  subroutine MP_kessler( &
    ...
    !$acc data copy(QTRC0,TEMP0,CPtot0,CVtot0) copyin(DENS0,PRES0) copyout(RHOE_t)
    ...
    do j = JS, JE
    do i = IS, IE
    !$acc loop private(dens,temp,qsatl)
    do k = KS, KE
...
!    real(RP) :: qhydro
    ...
    do i  = IS, IE
    do k  = KS, KE
       cldfrac(k,i,j) = 0.5_RP + sign(0.5_RP,QTRC(k,i,j,I_hyd_QC) + &
                        QTRC(k,i,j,I_hyd_QR)-mask_criterion)
    enddo
    enddo
    enddo
    !$acc end kernels
