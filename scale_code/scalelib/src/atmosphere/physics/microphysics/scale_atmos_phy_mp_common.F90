...
!OCL SERIAL
  subroutine ATMOS_PHY_MP_precipitation_upwind( &
    ...
    real(RP), intent(out),optional :: mflx_qr (KA) ! ### for SPRINTARS
    real(RP), intent(out),optional :: mflx_qs (KA) ! ### for SPRINTARS
...
#define RHOCV_pu(k) RHOCV_pu
    ...
    mflx_qr(:) = 0.0_RP   ! ### for SPRINTARS
    mflx_qs(:) = 0.0_RP   ! ### for SPRINTARS
...
#endif
       ! ### for SPRINTARS
       if ( present(mflx_qs) .and. present(mflx_qr) ) then 
          if ( iq > QLA ) then ! ice water
#ifdef _OPENACC
             mflx_qs(k-1) = mflx_qs(k-1) + qflx0
#else
             do k = KS-1, KE-1
                mflx_qs(k) = mflx_qs(k) + qflx(k)
             enddo
#endif
          else
#ifdef _OPENACC
             mflx_qr(k-1) = mflx_qr(k-1) + qflx0
#else
             do k = KS-1, KE-1
                mflx_qr(k) = mflx_qr(k) + qflx(k)
             enddo
#endif
          endif
       endif
