!> module common / FEM communication / 3D
!!
!! @par Description
...
!<
#include "scalelib.h"
module scale_comm_fem_meshfield_cubedom3d
...
!----------------------------
  ...
  contains
    subroutine revert_hori(revert, ori, Ne_h1D, NeZ)
      ...
      integer :: i_, p1_
      !-----------------------------------------------------------------------------
      !$omp parallel do private(n,k,i,p3,p1, i_,p1_)
      do n=1, var_num
      do k=1, NeZ
      do i=1, Ne_h1D
        i_ = Ne_h1D - i + 1
...
end module scale_comm_fem_meshfield_cubedom3d