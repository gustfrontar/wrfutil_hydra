...
module scale_comm_cartesC_nest
  ...
  logical,  public :: ONLINE_BOUNDARY_USE_CHEM = .false.
  logical,  public :: ONLINE_BOUNDARY_USE_AEROSOL = .false.
  ...
  integer,  public :: ONLINE_RECV_QS_MP = 0     !< first index of MP's tracer received from the parent domain
  integer,  public :: ONLINE_RECV_QE_MP = 0     !< last  index of MP's tracer received from the parent domain
  integer,  public :: ONLINE_RECV_QS_CHEM = 0   !< first index of Chem's tracer received from the parent domain
  integer,  public :: ONLINE_RECV_QE_CHEM = 0   !< last  index of Chem's tracer received from the parent domain
  integer,  public :: ONLINE_RECV_QS_AERO = 0   !< first index of Aerosol's tracer received from the parent domain
  integer,  public :: ONLINE_RECV_QE_AERO = 0   !< last  index of Aerosol's tracer received from the parent domain
  integer,  public :: ONLINE_SEND_QS_MP = 0     !< first index of MP's tracer send to the daughter domain
  integer,  public :: ONLINE_SEND_QE_MP = 0     !< last  index of MP's tracer send to the daughter domain
  integer,  public :: ONLINE_SEND_QS_CHEM = 0   !< first index of Chem's tracer send to the daughter domain
  integer,  public :: ONLINE_SEND_QE_CHEM = 0   !< last  index of Chem's tracer send to the daughter domain
  integer,  public :: ONLINE_SEND_QS_AERO = 0   !< first index of Aerosol's tracer send to the daughter domain
  integer,  public :: ONLINE_SEND_QE_AERO = 0   !< last  index of Aerosol's tracer send to the daughter domain
...
contains
  ...
  subroutine COMM_CARTESC_NEST_setup ( &
    ...
    integer,          intent(in) :: QA_MP, QA_CH, QA_AE
    ...
    namelist / PARAM_COMM_CARTESC_NEST / &
       ...
       ONLINE_BOUNDARY_USE_CHEM, &
       ONLINE_BOUNDARY_USE_AEROSOL, &
    ...
    if( flag_parent ) then ! must do first before daughter processes
       ...
       call COMM_CARTESC_NEST_parentsize( HANDLING_NUM, QA_MP, QA_CH, QA_AE )
       ...
       allocate( org_QTRC(KA,IA,JA,max(ONLINE_SEND_QA,1)) )
  ...
  subroutine COMM_CARTESC_NEST_parentsize( &
    ...
    real(RP) :: buffer
    integer, parameter :: ileng = 12
    integer  :: datapack(ileng)

    integer :: QA_OTHERSIDE
    integer :: QA_MP_OTHERSIDE
    integer :: QA_CH_OTHERSIDE
    integer :: QA_AE_OTHERSIDE
    ...
    if ( COMM_CARTESC_NEST_Filiation( INTERCOMM_ID(HANDLE) ) > 0 ) then
       ...
       datapack(10) = QA_MP
       if( ONLINE_BOUNDARY_USE_CHEM ) then
          datapack(11) = QA_CH
       else
          datapack(11) = 0
       end if
       if( ONLINE_BOUNDARY_USE_AEROSOL ) then
          datapack(12) = QA_AE
       else
          datapack(12) = 0
       end if
       ...
       if ( MP_TYPE_OTHERSIDE == "DRY" .or. MP_TYPE == "DRY" ) then
          ONLINE_SEND_QA = 0
          ONLINE_SEND_QS_MP = 0
          ONLINE_SEND_QE_MP = 0
          ...
          ONLINE_SEND_QE_MP = 1
          ONLINE_SEND_DIAGQHYD = .false.
       else if ( MP_TYPE == MP_TYPE_OTHERSIDE ) then
          ONLINE_SEND_QA = QA_MP
          ONLINE_SEND_QS_MP = 1
          ONLINE_SEND_QE_MP = QA_MP
          ONLINE_SEND_DIAGQHYD = .false.
       else
          ...
          ONLINE_SEND_QS_MP = 1
          ONLINE_SEND_QE_MP = N_HYD + 1
          ONLINE_SEND_DIAGQHYD = .true.
       endif

       if( ONLINE_BOUNDARY_USE_CHEM ) then
          ONLINE_SEND_QS_CHEM = ONLINE_SEND_QA+1
          ONLINE_SEND_QE_CHEM = ONLINE_SEND_QA+QA_CH
          ONLINE_SEND_QA = ONLINE_SEND_QA+QA_CH
       else
          ONLINE_SEND_QS_CHEM = 0
          ONLINE_SEND_QE_CHEM = 0
       endif

       if( ONLINE_BOUNDARY_USE_AEROSOL ) then
          ONLINE_SEND_QS_AERO = ONLINE_SEND_QA+1
          ONLINE_SEND_QE_AERO = ONLINE_SEND_QA+QA_AE
          ONLINE_SEND_QA = ONLINE_SEND_QA+QA_AE
       else
          ONLINE_SEND_QS_AERO = 0
          ONLINE_SEND_QE_AERO = 0
       endif


       if ( PRC_IsMaster ) then
          ! from parent to daughter
          call MPI_ISEND(ONLINE_SEND_QA, 1, MPI_INTEGER, PRC_myrank, tag, PRC_INTERCOMM_CHILD, ireq1, ierr1)
          call MPI_WAIT(ireq1, istatus, ierr1)
       end if
    ...
    elseif( COMM_CARTESC_NEST_Filiation( INTERCOMM_ID(HANDLE) ) < 0 ) then
       ...
       QA_MP_OTHERSIDE              = datapack(10)
       QA_CH_OTHERSIDE              = datapack(11)
       QA_AE_OTHERSIDE              = datapack(12)
       ONLINE_PARENT_DTSEC          = buffer

       if ( MP_TYPE == "DRY" .or. MP_TYPE_OTHERSIDE == "DRY" ) then
          ONLINE_RECV_QA = 0
          ONLINE_RECV_QS_MP = 0
          ONLINE_RECV_QE_MP = 0
          ONLINE_RECV_DIAGQHYD = .false.
       else if ( MP_TYPE == "QV" .or. MP_TYPE == "QV" ) then
          ONLINE_RECV_QA = 1
          ONLINE_RECV_QS_MP = 1
          ONLINE_RECV_QE_MP = 1
          ONLINE_RECV_DIAGQHYD = .false.
          ...
          ONLINE_RECV_QS_MP = 1
          ONLINE_RECV_QE_MP = N_HYD+1
          ONLINE_RECV_DIAGQHYD = .true.
       endif

       if( ONLINE_BOUNDARY_USE_CHEM ) then
          if ( QA_CH == QA_CH_OTHERSIDE ) then
             ONLINE_RECV_QS_CHEM = ONLINE_RECV_QA+1
             ONLINE_RECV_QE_CHEM = ONLINE_RECV_QA+QA_CH
             ONLINE_RECV_QA = ONLINE_RECV_QA+QA_CH
          else
             LOG_ERROR("COMM_CARTESC_NEST_parentsize",*) "Number of variables for CHEM is not consisteng"
             call PRC_abort
          end if
       else
          ONLINE_RECV_QS_CHEM = 0
          ONLINE_RECV_QE_CHEM = 0
       endif

       if( ONLINE_BOUNDARY_USE_AEROSOL ) then
          if ( QA_AE == QA_AE_OTHERSIDE ) then
             ONLINE_RECV_QS_AERO = ONLINE_RECV_QA+1
             ONLINE_RECV_QE_AERO = ONLINE_RECV_QA+QA_AE
             ONLINE_RECV_QA = ONLINE_RECV_QA+QA_AE
          else
             LOG_ERROR("COMM_CARTESC_NEST_parentsize",*) "Number of variables for AEROSOL is not consisteng"
             call PRC_abort
          end if
       else
          ONLINE_RECV_QS_AERO = 0
          ONLINE_RECV_QE_AERO = 0
       endif
 
       if ( PRC_IsMaster ) then
          ! from parent to daughter
          call MPI_IRECV(QA_OTHERSIDE, 1, MPI_INTEGER, PRC_myrank, tag, PRC_INTERCOMM_PARENT, ireq1, ierr1)
          call MPI_WAIT(ireq1, istatus, ierr1)
          if ( QA_OTHERSIDE .ne. ONLINE_RECV_QA ) then
             LOG_ERROR("COMM_CARTESC_NEST_parentsize",*) "Number of QA is not consisteng", QA_OTHERSIDE, ONLINE_RECV_QA
             call PRC_abort
          end if
       end if

    else
       LOG_ERROR("COMM_CARTESC_NEST_parentsize",*) 'internal error'
       call PRC_abort
    endif
  ...
  subroutine COMM_CARTESC_NEST_importgrid_nestdown( &
    ...
    if ( COMM_CARTESC_NEST_Filiation( INTERCOMM_ID(HANDLE) ) > 0 ) then
       ...
       do n = 1, NUM_YP
          ...
          target_rank = COMM_CARTESC_NEST_TILE_LIST_YP(n)
    ...
    elseif( COMM_CARTESC_NEST_Filiation( INTERCOMM_ID(HANDLE) ) < 0 ) then
       ...
       do n = 1, COMM_CARTESC_NEST_TILE_ALL
          ! receive data from multiple parent tiles
          target_rank = COMM_CARTESC_NEST_TILE_LIST_d(n,PRC_myrank+1)

          xloc = mod( n-1, dom_info(I_PARENT)%tile_num_x ) + 1
          yloc = int( real(n-1) / real(dom_info(I_PARENT)%tile_num_x) ) + 1
          ...
          tag   = tagbase + tag_cz
          call MPI_IRECV(recvbuf_3D(:,:,:,1), ileng, COMM_datatype, target_rank, tag, PRC_INTERCOMM_PARENT, ireq_d(rq), ierr)

          rq = rq + 1
          ileng = PARENT_KA * PARENT_IA * PARENT_JA
          tag   = tagbase + tag_fz
          call MPI_IRECV(recvbuf_3D(:,:,:,2), ileng, COMM_datatype, target_rank, tag, PRC_INTERCOMM_PARENT, ireq_d(rq), ierr)
          ...
          !$omp parallel do
          do j = 1, PARENT_JE - PARENT_JS + 1
          do i = 1, PARENT_IE - PARENT_IS + 1
             buffer_ref_LON  (i+xs-1,j+ys-1) = recvbuf_2D(i+PARENT_IS-1,j+PARENT_JS-1,tag_lon  )
             buffer_ref_LAT  (i+xs-1,j+ys-1) = recvbuf_2D(i+PARENT_IS-1,j+PARENT_JS-1,tag_lat  )
             buffer_ref_LONUY(i+xs-1,j+ys-1) = recvbuf_2D(i+PARENT_IS-1,j+PARENT_JS-1,tag_lonuy)
             buffer_ref_LATUY(i+xs-1,j+ys-1) = recvbuf_2D(i+PARENT_IS-1,j+PARENT_JS-1,tag_latuy)
             buffer_ref_LONXV(i+xs-1,j+ys-1) = recvbuf_2D(i+PARENT_IS-1,j+PARENT_JS-1,tag_lonxv)
             buffer_ref_LATXV(i+xs-1,j+ys-1) = recvbuf_2D(i+PARENT_IS-1,j+PARENT_JS-1,tag_latxv)
          enddo
          enddo
          ...
          do i = 1, PARENT_IE - PARENT_IS + 1
          do k = 1, PARENT_KA
             buffer_ref_CZ(k,i+xs-1,j+ys-1) = recvbuf_3D(k,i+PARENT_IS-1,j+PARENT_JS-1,1)
             buffer_ref_FZ(k,i+xs-1,j+ys-1) = recvbuf_3D(k,i+PARENT_IS-1,j+PARENT_JS-1,2)
          enddo
          enddo
          enddo
       enddo
...
!OCL XFILL
       ...
       if ( .NOT. ONLINE_DAUGHTER_NO_ROTATE ) then
          ...
          !$omp parallel do collapse(2) private(u_on_map,v_on_map)
          do j = 1, JA
          do i = 1, IA
          do k = 1, KA
