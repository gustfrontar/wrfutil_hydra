!> module common / FEM element / hexahedron
!!
!! @par Description
...
!<
#include "scalelib.h"
module scale_fem_element_hexahedral
...
!OCL SERIAL  
  subroutine construct_Element(elem)
    ...
    use scale_matrix, only: MATRIX_inv
    ...
    use scale_fem_element_quadrilateral, only: QuadrilateralElement
    ...
    do f=1, elem%Nfaces_h
      if (elem%IsLumpedMatrix()) then
        ...
        MassEdge_h(:,:) = MATRIX_inv(matmul(V2D_h, transpose(V2D_h)))
      end if
    ...
    do f=1, elem%Nfaces_v
      if (elem%IsLumpedMatrix()) then
        ...
        MassEdge_v(:,:) = MATRIX_inv(matmul(V2D_v, transpose(V2D_v)))
      end if
...
end module scale_fem_element_hexahedral