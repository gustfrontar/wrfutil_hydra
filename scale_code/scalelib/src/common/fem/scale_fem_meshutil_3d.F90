!> module common / FEM mesh / utility 3D
!!
!! @par Description
!!           A module useful for generating 3D mesh in FEM
!!
!! @author Yuta Kawai, Team SCALE
!!
!<
#include "scalelib.h"
module scale_fem_meshutil_3d
  !-----------------------------------------------------------------------------
  !
  !++ used modules
  !
...
!OCL SERIAL
  subroutine MeshUtil3D_genConnectivity( EToE, EToF, &
    ...
    use scale_sort, only: SORT_quicksort
    implicit none
    ...
    integer(kind=DP) :: face_ids(Ne*Nfaces)
    integer :: k
    integer :: f
    integer :: n
    integer :: n1, n2
    ...
    integer(kind=DP) :: sort_indx(Ne*Nfaces)
    integer(kind=DP) :: sorted_faceid(Ne*Nfaces)
    ...
    !- sort row
    call SORT_quicksort( int(Ne*Nfaces, kind=DP), sorted_faceid, sort_indx )
...
end module scale_fem_meshutil_3d