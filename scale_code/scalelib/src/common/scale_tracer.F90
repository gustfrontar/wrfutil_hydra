...
module scale_tracer
  ...
  integer,                public :: TRACER_REF  (QA_MAX) !> reference in advection

  !$acc declare create(QA, TRACER_CV, TRACER_CP, TRACER_R, TRACER_ENGI0, TRACER_ADVC, TRACER_MASS, TRACER_REF)
...
contains
  ...
  subroutine TRACER_regist(  &
    ...
    integer,          intent(in), optional :: REF  (NQ)
    ...
    if ( present(REF) ) then
       ...
       do n = 1, NQ
          if ( REF(n) < 0 ) then
             REF_(n) = n + REF(n)
             if ( REF_(n) < id_ref ) then
                LOG_ERROR("TRACER_regist",*) 'The order of tracer is invalid in terms of REF!'
                call PRC_abort
             end if
             id_ref = REF_(n)
             if ( .not. ADVC_(id_ref) ) then
                LOG_ERROR("TRACER_regist",*) 'The reference tracer must be advectable!'
                call PRC_abort
             end if
             REF_(n) = REF_(n) + QA
          else
    ...
    do n = 1, NQ
       ...
       LOG_INFO("TRACER_regist",'(1x,A,I3,A,A,A,F6.1,A,F6.1,A,ES10.3,A,L1,A,L1,A,I3)') &
    ...
    !$acc update device(QA, TRACER_CV(QS:QA), TRACER_CP(QS:QA), TRACER_R(QS:QA), TRACER_ENGI0(QS:QA), TRACER_ADVC(QS:QA), TRACER_MASS(QS:QA), TRACER_REF(QS:QA))
