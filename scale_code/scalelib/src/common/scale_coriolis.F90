...
contains
  ...
  subroutine CORIOLIS_setup( &
    ...
    call CORIOLIS_calc_f( IA, JA, CY, LAT, & ! [in]
       CORIOLIS_f                          ) ! [out]
...
!!
  subroutine CORIOLIS_calc_f( IA, JA, CY, LAT, &
    cori_f )
    ...
    real(RP), intent(in) :: CY(JA)
    real(RP), intent(in) :: LAT(IA,JA)    
    real(RP), intent(out) :: cori_f(IA,JA)
    ...
    case ( 'PLANE' )
      ...
      do j = 1, JA
      do i = 1, IA
         cori_f(i,j) = CORIOLIS_f0 + CORIOLIS_beta * ( CY(j) - CORIOLIS_y0 )
      end do
      end do
    case ( 'SPHERE' )
      !omp parallel do collapse(2)
      do j = 1, JA
      do i = 1, IA
         cori_f(i,j) = 2.0_RP * OHM * sin( LAT(i,j) )
      end do
      end do
    case default
      LOG_ERROR("CORIOLIS_calc_f",*) 'Coriolis type is invalid: ', trim(coriolis_type)
      LOG_ERROR_CONT(*) 'The type must be PLANE or SPHERE'
      call PRC_abort
    end select

    return
  end subroutine CORIOLIS_calc_f
