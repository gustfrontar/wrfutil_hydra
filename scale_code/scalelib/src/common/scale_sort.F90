...
module scale_sort
  ...
  interface SORT_quicksort
     module procedure SORT_quicksort_without_idx
     module procedure SORT_quicksort_with_idx
  end interface SORT_quicksort

  interface SORT_heapsort
     module procedure SORT_heapsort_without_idx
     module procedure SORT_heapsort_with_idx
  end interface SORT_heapsort
...
!OCL SERIAL
  ...
  recursive subroutine SORT_quicksort_without_idx(n, array)
    implicit none
    integer(DP), intent(in) :: n
    ...
    return
  end subroutine SORT_quicksort_without_idx

  recursive subroutine SORT_quicksort_with_idx(n, array, idx)
    implicit none
    integer(DP), intent(in) :: n
    integer(DP), intent(inout) :: array(n)
    integer(DP), intent(inout) :: idx(n)
    integer(DP) :: i, j, k, k1, kn2, kn, tmp, tmp_idx
    integer, parameter :: threshold = 4
    ...
    if(i - 1 > threshold) then
       call SORT_quicksort(i - 1, array(1:(i - 1)), idx(1:(i - 1)))
    else if(i - 1 > 1) then
       call SORT_heapsort(i - 1, array(1:(i - 1)), idx(1:(i - 1)))
    end if
    if(n - j > threshold) then
       call SORT_quicksort(n - j, array((j + 1):n), idx((j + 1):n))
    else if(n - j > 1) then
       call SORT_heapsort(n - j, array((j + 1):n), idx((j + 1):n))
    end if

    return
  end subroutine SORT_quicksort_with_idx

  subroutine SORT_heapsort_without_idx(n, array)
    implicit none
    integer(DP), intent(in) :: n
    integer(DP), intent(inout) :: array(n)
    ...
    return
  end subroutine SORT_heapsort_without_idx

  subroutine SORT_heapsort_with_idx(n, array, idx)
    implicit none
    integer(DP), intent(in) :: n
    integer(DP), intent(inout) :: array(n)
    integer(DP), intent(inout) :: idx(n)
    ...
    integer(DP) :: i, j, k, l
    integer(DP) :: t, ti ! temporal values
    ...
    return
  end subroutine SORT_heapsort_with_idx
