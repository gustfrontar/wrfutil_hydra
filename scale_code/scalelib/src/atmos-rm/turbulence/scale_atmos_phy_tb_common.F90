...
contains
  ...
  subroutine ATMOS_PHY_TB_calc_strain_tensor( &
   ...
   ! momentum -> velocity
    ...
    !$omp shared(JS,JE,ISn1,IEp1,KS,KE,MOMY,DENS,VELY_ZX) &
...
!OCL LOOP_NOINTERCHANGE
       ...
       if ( twoD ) then
          i = IIS
          !$omp parallel do default(none)                                            &
          !$omp shared(JJS,JJE,i,KS,KE,S11_C,S22_C,S33_C,S31_C,S12_C,S23_C,S2) &
          !$omp private(j,k) OMP_SCHEDULE_ collapse(2)
          !$acc kernels
          do j = JJS-1, JJE+1
          do k = KS, KE
#ifdef DEBUG
          ...
          call CHECK( __LINE__, S23_C(k,i,j) )
#endif
             S2(k,i,j) = 2.0_RP * ( S22_C(k,i,j)**2 + S33_C(k,i,j)**2 ) &
                       + 4.0_RP * ( S23_C(k,i,j)**2 )
          enddo
          enddo
          !$acc end kernels
       else
