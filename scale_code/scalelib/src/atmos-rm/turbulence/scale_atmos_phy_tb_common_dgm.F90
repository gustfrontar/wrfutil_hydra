...
!-------------------------------------------------------------------------------
#include "scalelib.h"
module scale_atmos_phy_tb_common_dgm
  !-----------------------------------------------------------------------------
  !
  !++ Used modules
  !
  ...
  !++ Public procedures
  !
  public :: atm_phy_tb_common_dgm_setup
  public :: atm_phy_tb_common_dgm_Final

  public :: atm_phy_tb_common_dgm_get_varinfo
  public :: atm_phy_tb_common_dgm_calc_lambda

  public :: atm_phy_tb_common_dgm_cal_grad_qtrc
  public :: atm_phy_tb_common_dgm_cal_tend
  public :: atm_phy_tb_common_dgm_cal_tend_qtrc
  ...
  type(DYN_BNDInfo), allocatable, public :: bnd_info(:)    
...
contains
!OCL SERIAL
  subroutine atm_phy_tb_common_dgm_setup( mesh )
    ...
    do ldomID=1, mesh%LOCAL_MESH_NUM
      lcmesh3D =>  mesh%lcmesh_list(ldomID)

      allocate( bnd_info(ldomID)%is_bound(lcmesh3D%refElem3D%NfpTot,lcmesh3D%Ne) )
      call dyn_bnd%Inquire_bound_flag(  bnd_info(ldomID)%is_bound, & ! (out)
        ldomID, lcmesh3D%VMapM, lcmesh3D%VMapP, lcmesh3D%VMapB,    & ! (in)
        lcmesh3D, lcmesh3D%refElem3D                               ) ! (in)
    enddo

    return
  end subroutine atm_phy_tb_common_dgm_setup

!OCL SERIAL
  subroutine atm_phy_tb_common_dgm_Final()
    implicit none
    integer :: ldomid
    !--------------------------------------------------------------------

    do ldomid=1, size(bnd_info)
      deallocate( bnd_info(ldomid)%is_bound )
    enddo
    deallocate( bnd_info )

    return
  end subroutine atm_phy_tb_common_dgm_Final

!OCL SERIAL
  subroutine atm_phy_tb_common_dgm_get_varinfo( &
    auxvar_info, diagvar_info, tend_info        )
    use scale_fem_variableinfo, only: VariableInfo
    implicit none
    ...
    return
  end subroutine atm_phy_tb_common_dgm_get_varinfo

!OCL SERIAL  
  subroutine atm_phy_tb_common_dgm_calc_lambda( lambda, & ! (out)
    ...
    do ke=lmesh%NeS, lmesh%NeE
      ...
      he = ( vol / real( elem%Nnode_h1D**2 * elem%Nnode_v, kind=RP ) )**OneOverThree

      FZ = lmesh%zlev(elem%Colmask(elem%Nnode_v,1),ke) - lmesh%zlev(elem%Colmask(1,1),ke)      
      elem_aspect_eff = fact( FZ/real(elem%Nnode_v, kind=RP), sqrt(vol/FZ)/real(elem%Nnode_h1D, kind=RP), sqrt(vol/FZ)/real(elem%Nnode_h1D, kind=RP) )
    ...
    return
  end subroutine atm_phy_tb_common_dgm_calc_lambda


!> Calculate parameterized diffusive mass flux of tracer with turbulent model
!!
!OCL SERIAL  
  subroutine atm_phy_tb_common_dgm_cal_grad_qtrc( &
    DFQ1, DFQ2, DFQ3,                                           & ! (out)
    ...
    return
  end subroutine atm_phy_tb_common_dgm_cal_grad_qtrc

!> Calculate tendecies with turbulent model
!!
!OCL SERIAL  
  subroutine atm_phy_tb_common_dgm_cal_tend( &
    ...
    return
  end subroutine atm_phy_tb_common_dgm_cal_tend

!> Calculate tendecies of tracer density with turbulent model
!!
!OCL SERIAL  
  subroutine atm_phy_tb_common_dgm_cal_tend_qtrc( &
    RHOQ_t,                                          & ! (out)
    DFQ1, DFQ2, DFQ3,                                & ! (in)
    ...
    return
  end subroutine atm_phy_tb_common_dgm_cal_tend_qtrc
...
end module scale_atmos_phy_tb_common_dgm
