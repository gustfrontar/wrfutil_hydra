...
!-------------------------------------------------------------------------------
#include "scalelib.h"
module scale_atmos_dyn_dgm_hydrostatic
  !-----------------------------------------------------------------------------
  !
  !++ Used modules
  !
...
#else
      LOG_ERROR('matmul_pinv_v',*) 'Binary not compiled specifying SCALE_MATHLIB_LIBS! STOP.'
      call PRC_abort     
#endif
...
!OCL SERIAL
    subroutine get_PmatD_LU( pmatDlu_, pmatD_, pmatDlu_ipiv_, N)
      ...
      pmatDlu_(:,:) = pmatD_(:,:)
      call MATRIX_LU(pmatDlu_, pmatDlu_ipiv_)
      return
...
!OCL SERIAL
    subroutine get_PmatD_inv( pmatDinv_, pmatD_, pmatDlu_ipiv_, N)
      ...
      pmatDinv_(:,:) = MATRIX_inv( pmatD_ )
      return
    end subroutine get_PmatD_inv    
  end subroutine construct_pmatInv

end module scale_atmos_dyn_dgm_hydrostatic