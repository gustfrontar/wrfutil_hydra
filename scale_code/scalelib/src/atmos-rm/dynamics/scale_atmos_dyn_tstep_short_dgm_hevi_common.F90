...
!!
!! @par Description
!!          Common subroutines for HEVI DGM scheme
!!
...
!-------------------------------------------------------------------------------
#include "scalelib.h"
module scale_atmos_dyn_tstep_short_dgm_hevi_common
  !-----------------------------------------------------------------------------
  !
  !++ used modules
  !
  ...
  !++ Public procedure
  !
  public :: atmos_dyn_tstep_short_dgm_hevi_common_gen_vmap
  public :: atmos_dyn_tstep_short_dgm_hevi_common_eval_Ax
  public :: atmos_dyn_tstep_short_dgm_hevi_common_eval_Ax_uv
  public :: atmos_dyn_tstep_short_dgm_hevi_common_construct_matbnd
  public :: atmos_dyn_tstep_short_dgm_hevi_common_construct_matbnd_uv
...
!OCL SERIAL
subroutine atmos_dyn_tstep_short_dgm_hevi_common_gen_vmap( &
    vmapM, vmapP, & ! (out)
    lmesh, elem   ) ! (in)
    implicit none
    ...
    return
  end subroutine atmos_dyn_tstep_short_dgm_hevi_common_gen_vmap

!OCL SERIAL  
  subroutine atmos_dyn_tstep_short_dgm_hevi_common_eval_Ax( &
...
!      call sparsemat_matmul(Dz, MOMZ(:)**2 / ( DENS_hyd(:,ke_z,ke_xy) + DDENS(:) ) + DPRES(:), Fz) ! [<- MOMZ x MOMZ / DENS + DPRES ]
    ...
    return
  end subroutine atmos_dyn_tstep_short_dgm_hevi_common_eval_Ax

!OCL SERIAL  
  subroutine atmos_dyn_tstep_short_dgm_hevi_common_eval_Ax_uv( &
    MOMX_t, MOMY_t,                                          & ! (out)
    alph,                                                    & ! (out)
    PROG_VARS, PROG_VARS0,                                   & ! (in)
    DDENS00, MOMX00, MOMY00, MOMZ00, DRHOT00,                & ! (in)
    ...
    return
  end subroutine atmos_dyn_tstep_short_dgm_hevi_common_eval_Ax_uv

!OCL SERIAL  
  subroutine atmos_dyn_tstep_short_dgm_hevi_common_construct_matbnd( &
    PmatBnd,                                & ! (out)
    kl, ku, nz_1D,                          & ! (in)
    ...
    do ke_z=1, lmesh%NeZ
      ...
      do f1=1, 2
        ...
        if (bc_flag) then
          ...
          ! PmatD(:,pv1,MOMZ_VID_LC,MOMZ_VID_LC) = PmatD(:,pv1,MOMZ_VID_LC,MOMZ_VID_LC) - tmp1(:) * 2.0_RP * W0(pv1,ke_z,ij) [ <- d_MOMZ ( MOMZ x MOMZ / DENS ) ]
          ! PmatD(:,pv1,MOMZ_VID_LC,DENS_VID_LC) = PmatD(:,pv1,MOMZ_VID_LC,DENS_VID_LC) + tmp1(:) * W0(pv1,ke_z,ij)**2       [ <- d_DENS ( MOMZ x MOMZ / DENS ) ]
          ...
          if (f1 == 1) then
            ...
            ! PmatL(:,pv2,MOMZ_VID_LC,MOMZ_VID_LC) = PmatL(:,pv2,MOMZ_VID_LC,MOMZ_VID_LC) &     [ <- d_MOMZ ( MOMZ x MOMZ / DENS ) ]
            !                                      + tmp1(:) * 2.0_RP * W0(pv2,ke_z2,ij)
            ! PmatL(:,pv2,MOMZ_VID_LC,DENS_VID_LC) = - tmp1(:) * W0(pv2,ke_z2,ij) * W0(pv2,ke_z2,ij) [ <- d_DENS ( MOMZ x MOMZ / DENS ) ]      
            ...
            ! PmatU(:,pv2,MOMZ_VID_LC,MOMZ_VID_LC) = PmatU(:,pv2,MOMZ_VID_LC,MOMZ_VID_LC ) &    [ <- d_MOMZ ( MOMZ x MOMZ / DENS ) ]
            !                                     + tmp1(:) * 2.0_RP * W0(pv2,ke_z2,ij)
            ! PmatU(:,pv2,MOMZ_VID_LC,DENS_VID_LC) = - tmp1(:) * W0(pv2,ke_z2,ij) * W0(pv2,ke_z2,ij) [ <- d_DENS ( MOMZ x MOMZ / DENS ) ]  
    ...
    return
  end subroutine atmos_dyn_tstep_short_dgm_hevi_common_construct_matbnd

!OCL SERIAL  
  subroutine atmos_dyn_tstep_short_dgm_hevi_common_construct_matbnd_uv( &
    PmatBnd_uv,                             & ! (out)
    ...
    return
  end subroutine atmos_dyn_tstep_short_dgm_hevi_common_construct_matbnd_uv
...
end module scale_atmos_dyn_tstep_short_dgm_hevi_common