!-------------------------------------------------------------------------------
!> module Atmosphere / Dynamics driver
!!
!! @par Description
...
module scale_atmos_dyn
  ...
  character(len=3) :: spatial_scheme  ! FVM or DGM
...
contains
  ...
  subroutine ATMOS_DYN_setup( &
    ...
    use scale_atmos_dyn_dgm_operator, only: &
       dg_mesh3D => mesh3D
    use scale_atmos_dyn_tstep_large_dgm_heve, only: &
       ATMOS_DYN_Tstep_large_dgm_heve_regist_dynvar
    use scale_atmos_dyn_tinteg_short_rkdg, only: &
       ATMOS_DYN_Tinteg_short_rkdg_setup
    use scale_atmos_dyn_tinteg_tracer_rkdg, only: &
       ATMOS_DYN_Tinteg_tracer_rkdg_setup
    ...
    real(RP),          intent(in)    :: DTSEC_dyn
    logical, optional, intent(in)    :: none
    type(AtmosDynDGM_vars), intent(inout), optional :: dg_dyn_vars
    ...
    if ( .NOT. DYN_NONE ) then
      spatial_scheme = DYN_Tstep_Large_TYPE(1:3)
       ...
       case ('DGM')
         call ATMOS_DYN_Tstep_large_dgm_heve_regist_dynvar( dg_dyn_vars )
       end select
       ...
       case ('DGM')
         call ATMOS_DYN_Tinteg_short_rkdg_setup( DYN_Tinteg_Short_TYPE, DYN_Tstep_Short_TYPE, DTSEC_dyn ) ! [IN]
         call ATMOS_DYN_Tinteg_tracer_rkdg_setup( DYN_Tinteg_Tracer_TYPE, DTSEC ) ! [IN]
       end select
  ...
  subroutine ATMOS_DYN_finalize
    ...
    use scale_atmos_dyn_dgm_vars, only: &
       ATMOS_DYN_DGM_vars_finalize
    use scale_atmos_dyn_tinteg_short_rkdg, only: &
       ATMOS_DYN_Tinteg_short_rkdg_finalize
    use scale_atmos_dyn_tinteg_tracer_rkdg, only: &
       ATMOS_DYN_Tinteg_tracer_rkdg_finalize
    use scale_spnudge, only: &
       SPNUDGE_finalize
    ...
    if ( .NOT. DYN_NONE ) then
      ...
      case( 'DGM' )
        call ATMOS_DYN_Tinteg_short_rkdg_finalize
        call ATMOS_DYN_Tinteg_tracer_rkdg_finalize
      end select
