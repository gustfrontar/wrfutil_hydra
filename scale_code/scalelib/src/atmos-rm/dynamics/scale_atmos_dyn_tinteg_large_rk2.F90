...
!! This module provides two types of 2nd order and 2 stage Runge=Kutta method 
!!
!<
!-------------------------------------------------------------------------------
#include "scalelib.h"
module scale_atmos_dyn_tinteg_large_rk2
  !-----------------------------------------------------------------------------
  !
  !++ used modules
  !
  ...
  !++ Public procedure
  !
  public :: ATMOS_DYN_Tinteg_large_rk2_setup
  public :: ATMOS_DYN_Tinteg_large_rk2
...
contains
  ...
  !-----------------------------------------------------------------------------
  !> Setup
  subroutine ATMOS_DYN_Tinteg_large_rk2_setup( &
       tinteg_type )
    use scale_prc, only: &
       PRC_abort
    implicit none
    ...
    if ( tinteg_type /= 'RK2' ) then
       LOG_ERROR("ATMOS_DYN_Tinteg_large_rk3_setup",*) 'TINTEG_LARGE_TYPE is not RK2. Check!'
       call PRC_abort
    end if

    return
  end subroutine ATMOS_DYN_Tinteg_large_rk2_setup

  !-----------------------------------------------------------------------------
  !> RK3
  subroutine ATMOS_DYN_tinteg_large_rk2( &
    ...
    call PROF_rapstart("DYN_Large_RK2_Prep", 3)
...
!OCL XFILL
       ...
       call PROF_rapend  ("DYN_Large_RK2_Prep", 3)

       !------------------------------------------------------------------------
       ! Start RK
       !------------------------------------------------------------------------

       call PROF_rapstart("DYN_Large_RK2", 3)

       do n = 1, 2

          dtrk = DTL / real( 3-n, kind=DP )
          first = n == 1
          last = n == 2
         ...
         !  end if
          ...
          call ATMOS_DYN_tstep_large( &
               ...
               ND_COEF, ND_COEF_Q, ND_LAPLACIAN_NUM, ND_SFC_FACT, ND_USE_RS, & ! (in)
       ...
       call PROF_rapend  ("DYN_Large_RK2", 3)

    return
  end subroutine ATMOS_DYN_tinteg_large_rk2

end module scale_atmos_dyn_tinteg_large_rk2
