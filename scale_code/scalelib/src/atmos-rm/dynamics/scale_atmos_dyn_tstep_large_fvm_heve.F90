...
contains
  ...
  subroutine ATMOS_DYN_Tstep_large_fvm_heve_setup( &
    ...
    ! reference
    allocate( ref_save(QA) )
    do iq = 1, QA
       if ( TRACER_REF(iq) > 0 ) then
          ref_save(TRACER_REF(iq)) = .true.
       end if
    end do
    do iq = 1, QA
       if ( ref_save(iq) ) then
          allocate( qflx_ref(KA,IA,JA,3) )
          !$acc enter data create(qflx_ref)
          exit
       end if
    end do
    ref_id = -1
  ...
  subroutine ATMOS_DYN_Tstep_large_fvm_heve_finalize
    ...
    deallocate( ref_save )
    if ( allocated(qflx_ref) ) then
       !$acc exit data delete(qflx_ref)
       deallocate( qflx_ref )
    end if
