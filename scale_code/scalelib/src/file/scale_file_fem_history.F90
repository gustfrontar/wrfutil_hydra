!-------------------------------------------------------------------------------
!> module File / FEM field data / History
!!
!! @par Description
...
module scale_file_fem_history
  ...
  use scale_file_history, only: &
    ...
    FILE_HISTORY_truncate_1D_default, &
    FILE_HISTORY_truncate_2D_default, &
    FILE_HISTORY_truncate_3D_default, &
    FILE_HISTORY_put,         &
    FILE_HISTORY_write,       &
  ...
  use scale_fem_element_base, only: elementbase1D, elementbase2D, elementbase3D
  use scale_fem_mesh_base1d, only: MeshBase1D
  ...
  use scale_fem_localmesh_1d, only: LocalMesh1D
...
!OCL SERIAL
  subroutine FILE_HISTORY_meshfield_setup( &
    ...
    logical, intent(in), optional :: skip_hist_setup
...
!OCL SERIAL
  subroutine FILE_HISTORY_meshfield_put1D(hstid, field1d)
    use scale_file_fem_common, only: &
    ...
    call FILE_HISTORY_put( hstid, buf, truncate=FILE_HISTORY_truncate_1D_default )
...
!OCL SERIAL
  subroutine FILE_HISTORY_meshfield_in1D( field1d, desc, standard_name )
    ...
    call history_in_regvar( hstid, do_put,        & ! (out)
      field1d, desc, 1, standard_name, 'FEM_XYZ'  ) ! (in)
...
!OCL SERIAL
  subroutine FILE_HISTORY_meshfield_put2D(hstid, field2d)
    ...
    call FILE_HISTORY_put( hstid, buf, FILE_HISTORY_truncate_2D_default )
...
!OCL SERIAL
  subroutine FILE_HISTORY_meshfield_in2D( field2d, desc, standard_name )
    ...
    call history_in_regvar( hstid, do_put,      & ! (out)
      field2d, desc, 2, standard_name, 'FEM_XY' ) ! (in)
...
!OCL SERIAL
  subroutine FILE_HISTORY_meshfield_put3D(hstid, field3d)
    ...
    call FILE_HISTORY_put( hstid, buf, FILE_HISTORY_truncate_3D_default )
...
!OCL SERIAL
  subroutine FILE_HISTORY_meshfield_in3D( field3d, desc, standard_name )
    ...
    call history_in_regvar( hstid, do_put,       & ! (out)
      field3d, desc, 3, standard_name, 'FEM_XYZ' ) ! (in)
...
!OCL SERIAL
  subroutine set_dim_axis1D()
    ...
    use scale_fem_mesh_base1d, only: &
...
end module scale_file_fem_history