...
module scale_landuse
  ...
  real(RP), public, allocatable :: LANDUSE_fact_lake_SPRINTARS (:,:) !< lake  factor ! ### for SPRINTARS
...
contains
  ...
  subroutine LANDUSE_setup( &
    ...
    ! ### for SPRINTARS
    allocate( LANDUSE_fact_lake_SPRINTARS (IA,JA) )
    LANDUSE_fact_lake_SPRINTARS (:,:) = 0.0_RP
    !$acc enter data copyin(LANDUSE_fact_lake_SPRINTARS)
...
!       LANDUSE_frac_lake (:,:) = 0.25_RP
  ...
  subroutine LANDUSE_read( &
    ...
    if ( LANDUSE_IN_BASENAME /= '' ) then
       ...
       call FILE_CARTESC_read( fid, 'FRAC_LAKE_abs', 'XY', LANDUSE_fact_lake_SPRINTARS (:,:) ) ! ### for SPRINTARS
...
!!$       end if
  ...
  subroutine LANDUSE_finalize
    ...
    !$acc exit data delete(LANDUSE_fact_lake_SPRINTARS)
    deallocate( LANDUSE_fact_lake_SPRINTARS )
