...
contains
    ...
    if ( do_read ) then
       ...
       if ( NM_FILE /= "" ) then
          ...
          do n = 1, nvars
             ...
             if ( ierr /= 0 ) exit
             ! set params
             call vars_atmos%put(item, vinfo(name=name, zstg=zstg, xstg=xstg, ystg=ystg, fact=fact, offset=offset, ignore=ignore))
          end do
    ...
    if ( do_read ) then
       ...
       do iq = 1, QA
          ...
          if ( vars_atmos%has_key( TRACER_NAME(iq) ) ) then
             ...
             type is ( vinfo )
                if ( v%name .ne. "" .and. (.not. v%ignore) ) then
  ...
  subroutine ParentAtmosInputNetCDF( &
    ...
    logical, intent(in)  :: same_mp_type
    logical, intent(in)  :: use_file_qhyd
    ...
    if ( same_mp_type ) then
       ...
       do iq = QS_MP, QE_MP
          if ( (.not. use_file_qhyd) .and. TRACER_NAME(iq) .ne. "QV" ) cycle
          call read3d( KA_org, KS_org+2, KE_org, IA_org, IS_org, IE_org, JA_org, JS_org, JE_org, &
               qtrc_org(:,:,:,iq), vars_atmos%get(TRACER_NAME(iq)), &
               it, nfiles_atm, fid_atm, fids_atm, SCALE_tile_atm, SCALE_DOMID_atm, &
               exist = exist )
...
!       call vars_land%put("SNOW_TEMP",  vinfo("TSNAV"))
       ...
       if ( NM_FILE /= "" ) then
          ...
          do n = 1, nvars
             ...
             if ( ierr /= 0 ) exit
             ! set params
             call vars_land%put(item, vinfo(name=name, zstg=zstg, xstg=xstg, ystg=ystg, fact=fact, offset=offset, ignore=ignore))
          end do
    ...
    if ( do_read ) then
       ...
       if ( NM_FILE /= "" ) then
          ...
          do n = 1, nvars
             ...
             if ( ierr /= 0 ) exit
             ! set params
             call vars_ocean%put(item, vinfo(name=name, zstg=zstg, xstg=xstg, ystg=ystg, fact=fact, offset=offset, ignore=ignore))
          end do
  ...
  subroutine ParentOceanInputNetCDF( &
    ...
    call read3d( 1, 1, 1, IA_org, IS_org, IE_org, JA_org, JS_org, JE_org, &
         tw_org(:,:), vars_ocean%get("OCEAN_TEMP"), &
         it, nfiles_ocn, fid_ocn, fids_ocn, SCALE_tile_ocn, SCALE_DOMID_ocn, &
         exist = exist )
    if ( .not. exist ) then
       call read2d( IA_org, IS_org, IE_org, JA_org, JS_org, JE_org, &
    ...
    if ( .not. exist ) then
       LOG_ERROR("ParentOceanInputNetCDF",*) "Either OCEAN_TEMP or OCEAN_SFC_TEMP must be exist"
       call PRC_abort
    end if
    sst_org(:,:) = tw_org(:,:)
  ...
  subroutine read3d( &
    ...
    select type(var)
    type is (vinfo)
       if ( var%name == "" .or. var%ignore ) then
          if ( present(exist) ) then
             exist = .false.
          else
             LOG_ERROR("read3d",*) 'data is not found '
