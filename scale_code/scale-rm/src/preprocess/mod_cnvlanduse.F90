...
module mod_cnvlanduse
  ...
  logical, public :: CNVLANDUSE_UseOEMAP  = .false.   ! OpenEarthMap
...
!       CNVLANDUSE_UseJIBIS
    ...
    if ( CNVLANDUSE_UseGLCCv2 ) then
       ...
       if ( CNVLANDUSE_UseOEMAP ) then
          LOG_INFO("CNVLANDUSE_setup",*) 'Use OpenEarthMap landuse data for a specific city'
          LOG_INFO("CNVLANDUSE_setup",*) 'Overwrite the city region'
       endif
    ...
    elseif ( CNVLANDUSE_UseOEMAP ) then
       CNVLANDUSE_DoNothing = .false.
       LOG_INFO("CNVLANDUSE_setup",*) 'Use OpenEarthMap landuse data for a specific city'
    endif
...
!OCL XFILL
       ...
       if ( CNVLANDUSE_UseOEMAP ) then
          call CNVLANDUSE_OEMAP( PFT_weight(:,:,:) ) ! [INOUT]
       endif
  ...
  !-----------------------------------------------------------------------------
  !> Convert from OenEarthMap landuse
  subroutine CNVLANDUSE_OEMAP( PFT_weight )
    use scale_prc, only: &
       PRC_abort
    use scale_const, only: &
       UNDEF2 => CONST_UNDEF2, &
    ...
    character(len=H_LONG) :: OEMAP_IN_DIR       = '.'     !< directory contains OEMAP files (GrADS format)
    character(len=H_LONG) :: OEMAP_IN_CATALOGUE = ''      !< metadata files for OEMAP

    namelist / PARAM_CNVLANDUSE_OEMAP / &
       OEMAP_IN_DIR,       &
       OEMAP_IN_CATALOGUE

    ! OEMAP data
    integer,  parameter :: categ_nmax = 8
    real(RP), parameter :: OEMAP_DLAT = 4.40902E-6_RP
    real(RP), parameter :: OEMAP_DLON = 5.364418030E-6_RP

    integer  :: lookuptable(1:8)
    data lookuptable /  1, & !  1 Bareland          ->  1 Dessert
                        2, & !  2 Rangeland         ->  2 Grassland
                        0, & !  3 Developed space   ->  7 Dryland Cropland and Pasture
                        0, & !  4 Road              ->  0 Urban and Built-up Land
                       11, & !  5 Tree              -> 11 Deciduous Broadleaf Forest
                       -1, & !  6 Water             -> -1 Sea Surface
                       10, & !  7 Agriculture land  -> 10 Paddy
                        0  / !  8 Building          ->  0 Urban and Built-up Land
    ...
    !--- read namelist
    rewind(IO_FID_CONF)
    read(IO_FID_CONF,nml=PARAM_CNVLANDUSE_OEMAP,iostat=ierr)
    if( ierr < 0 ) then !--- missing
       LOG_INFO("CNVLANDUSE_OEMAP",*) 'Not found namelist. Default used.'
    elseif( ierr > 0 ) then !--- fatal error
       LOG_ERROR("CNVLANDUSE_OEMAP",*) 'Not appropriate names in namelist PARAM_CNVLANDUSE_OEMAP. Check!'
       call PRC_abort
    endif
    LOG_NML(PARAM_CNVLANDUSE_OEMAP)


    TILE_DLAT = OEMAP_DLAT * D2R
    TILE_DLON = OEMAP_DLON * D2R

    ! catalogue file
    fname = trim(OEMAP_IN_DIR)//'/'//trim(OEMAP_IN_CATALOGUE)
    ...
    return
  end subroutine CNVLANDUSE_OEMAP
