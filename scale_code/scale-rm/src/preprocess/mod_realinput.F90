...
module mod_realinput
  ...
  logical,                private :: USE_FILE_QHYD              = .true.  ! use hydrometeor data from files
...
!                                   INIT_LANDWATER_RATIO_EACH(:),          &
  ...
  subroutine ParentAtmosSetup( &
    ...
    logical,          intent(in)  :: same_mp_type_in
    logical,          intent(in)  :: use_file_density_in ! use density data from files
    logical,          intent(in)  :: use_file_qhyd_in    ! use density data from files
 ...
 !-----------------------------------------------------------------------------
  ...
  subroutine ParentAtmosInput( &
    ...
    logical,          intent(in) :: same_mp_type_in   ! Is microphysics type same between outer and inner model
    logical,          intent(in) :: use_file_density_in
    logical,          intent(in) :: use_file_qhyd_in
    ...
    if ( do_read_atmos ) then
       ...
       if ( mixing_ratio ) then
          if ( use_file_qhyd_in ) then
             ...
             do k = k0, KA_org
                if( qv_org(k,i,j) > UNDEF ) then
                   qv_org(k,i,j) = qv_org(k,i,j) / ( 1.0_RP + qv_org(k,i,j) )
                else
                   qv_org(k,i,j) = UNDEF
                end if
             end do
       ...
       if ( (.not. use_file_qhyd_in) .or. (.not. same_mptype) ) then
          if ( qnum_flag ) then
       ...
       if ( nodens .and. use_file_density_in ) then
