...
module mod_atmos_bnd_driver
  ...
  logical,               private :: ATMOS_BOUNDARY_USE_AEROSOL  = .false. ! read from file?
  ...
  subroutine ATMOS_BOUNDARY_driver_setup
    ...
    if ( ATMOS_BOUNDARY_ONLINE ) then
       ...
       if ( ONLINE_IAM_DAUGHTER ) then
          ...
          ATMOS_BOUNDARY_USE_CHEM = ONLINE_BOUNDARY_USE_CHEM
          ATMOS_BOUNDARY_USE_AEROSOL = ONLINE_BOUNDARY_USE_AEROSOL
       endif
    endif
    ...
    if( ATMOS_BOUNDARY_USE_AEROSOL ) then
       do iq = QS_AE, QE_AE
          BND_QA = BND_QA + 1
          BND_IQ(iq) = BND_QA
       end do
    ...
    LOG_INFO_CONT(*) 'Is AEROSOL used in atmospheric boundary?       : ', ATMOS_BOUNDARY_USE_AEROSOL
    LOG_NEWLINE
...
!!$    enddo
  ...
  !-----------------------------------------------------------------------------
  !> set boundary data from initial value
  ...
  subroutine ATMOS_BOUNDARY_driver_send
    ...
    if ( do_parent_process ) then !online [parent]
       ...
       call ATMOS_BOUNDARY_update_online_parent( DENS,MOMZ,MOMX,MOMY,RHOT,QTRC,QV,Qe )
  ...
  subroutine ATMOS_BOUNDARY_send( &
    ...
    ! works
    real(RP) :: Q(KA,IA,JA,ONLINE_SEND_QA)
    ...
    ! send array for hydrometeor
    if ( ONLINE_SEND_DIAGQHYD ) then
       !$omp workshare
       !$acc kernels
       Q(:,:,:,ONLINE_SEND_QS_MP) = QV(:,:,:)
       !$acc end kernels
       !$omp end workshare
       !$omp parallel do private(ip)
       !$acc kernels
       do iq = ONLINE_SEND_QS_MP+1, ONLINE_SEND_QE_MP
          ip = iq-ONLINE_SEND_QS_MP
          Q(:,:,:,iq) = Qe(:,:,:,ip)
       end do
       !$acc end kernels
    else
       !$omp parallel do private(ip)
       !$acc kernels
       do iq = ONLINE_SEND_QS_MP, ONLINE_SEND_QE_MP
          ip = QS_MP+iq-ONLINE_SEND_QS_MP
          Q(:,:,:,iq) = QTRC(:,:,:,ip)
       enddo
       !$acc end kernels
    end if

    ! send array for chem's tracer
    if ( ONLINE_BOUNDARY_USE_CHEM ) then
       !$omp parallel do private(ip)
       !$acc kernels
       do iq = ONLINE_SEND_QS_CHEM, ONLINE_SEND_QE_CHEM
           ip = QS_CH+iq-ONLINE_SEND_QS_CHEM
           Q(:,:,:,iq) = QTRC(:,:,:,ip)
       enddo
       !$acc end kernels
    endif

    ! send array for aerosol's tracer
    if ( ONLINE_BOUNDARY_USE_AEROSOL ) then
       !$omp parallel do private(ip)
       !$acc kernels
       do iq = ONLINE_SEND_QS_AERO, ONLINE_SEND_QE_AERO
           ip = QS_AE+iq-ONLINE_SEND_QS_AERO
           Q(:,:,:,iq) = QTRC(:,:,:,ip)
       enddo
       !$acc end kernels
    endif
  ...
  subroutine ATMOS_BOUNDARY_recv
    ...
    ! works
    real(RP) :: Q(KA,IA,JA,ONLINE_RECV_QA)
    integer :: iq, ip
    !---------------------------------------------------------------------------
    ...
    if ( ONLINE_RECV_DIAGQHYD ) then
       ...
       !$omp parallel do private(ip)
       !$acc kernels
       do iq = ONLINE_RECV_QS_MP, ONLINE_RECV_QE_MP
          ip = QS_MP+iq-ONLINE_RECV_QS_MP
          QTRC_ref(:,:,:,ip) = Q(:,:,:,iq)
       end do
       !$acc end kernels
    end if
    ...
    if( ONLINE_BOUNDARY_USE_CHEM ) then
       !$omp parallel do private(ip)
       !$acc kernels
       do iq = ONLINE_RECV_QS_CHEM, ONLINE_RECV_QE_CHEM
          ip = QS_CH+iq-ONLINE_RECV_QS_CHEM
          QTRC_ref(:,:,:,ip) = Q(:,:,:,iq)
       enddo
       !$acc end kernels
    endif
    ...
    if( ONLINE_BOUNDARY_USE_AEROSOL ) then
       !$omp parallel do private(ip)
       !$acc kernels
       do iq = ONLINE_RECV_QS_AERO, ONLINE_RECV_QE_AERO
          ip = QS_AE+iq-ONLINE_RECV_QS_AERO
          QTRC_ref(:,:,:,ip) = Q(:,:,:,iq)
       enddo
       !$acc end kernels
    endif
