...
contains
  ...
  subroutine ATMOS_PHY_CP_driver_calc_tendency( update_flag )
    ...
    use mod_atmos_phy_cp_vars, only: &
       ...
       flux_qr_total  => ATMOS_PHY_CP_flux_qr_total,  &  ! rain flux for SPRINTARS
       flux_qs_total  => ATMOS_PHY_CP_flux_qs_total      ! snow flux for SPRINTARS
...
!       !$acc update device(w0mean)
    ...
    if ( update_flag ) then ! update
       ...
       select case ( ATMOS_PHY_CP_TYPE )
       case ( 'KF' )
          if(  ATMOS_PHY_AE_TYPE == 'SPRINTARS' ) then ! ### for SPRINTARS
!          !$acc update host(DENS,U,V,RHOT,TEMP,PRES,QDRY,QV,DENS_t_CP,RHOT_t_CP,RHOQV_t_CP,RHOHYD_t_CP,kf_nca,SFLX_rain,SFLX_snow,SFLX_ENGI,cloudtop,cloudbase,cldfrac_dp,cldfrac_sh)
             call ATMOS_PHY_CP_kf_tendency( KA, KS, KE, IA, IS, IE, JA, JS, JE, &
                                            ...
                                            flux_qr_total(1:KA,:,:),                  & ! [OUT] ### for SPRINTARS
                                            flux_qs_total(1:KA,:,:)                   ) ! [OUT] ### for SPRINTARS
!             !$acc update device(DENS_t_CP,RHOT_t_CP,RHOQV_t_CP,RHOHYD_t_CP,kf_nca,SFLX_rain,SFLX_snow,SFLX_ENGI,cloudtop,cloudbase,cldfrac_dp,cldfrac_sh,lux_qr_total,lux_qs_total)
          else
!             !$acc update host(DENS,U,V,RHOT,TEMP,PRES,QDRY,QV,DENS_t_CP,RHOT_t_CP,RHOQV_t_CP,RHOHYD_t_CP,kf_nca,SFLX_rain,SFLX_snow,SFLX_ENGI,cloudtop,cloudbase,cldfrac_dp,cldfrac_sh)
             call ATMOS_PHY_CP_kf_tendency( KA, KS, KE, IA, IS, IE, JA, JS, JE, &
                                            DENS(:,:,:),                              & ! [IN]
...
!             !$acc update device(DENS_t_CP,RHOT_t_CP,RHOQV_t_CP,RHOHYD_t_CP,kf_nca,SFLX_rain,SFLX_snow,SFLX_ENGI,cloudtop,cloudbase,cldfrac_dp,cldfrac_sh)
       ...
       case ( 'KF-JMAPPLIB' )
          ...
          if(  ATMOS_PHY_AE_TYPE == 'SPRINTARS' ) then ! ### for SPRINTARS
             LOG_ERROR("ATMOS_PHY_CP_driver",*) 'For Current version, ATMOS_PHY_AE_TYPE (', trim(ATMOS_PHY_AE_TYPE), &
                                                ') cannot be used for ATMOS_PHY_CP_TYPE = KF-JMAPPLIB !!!'
             call PRC_abort
          endif
