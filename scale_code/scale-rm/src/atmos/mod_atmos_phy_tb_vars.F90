...
module mod_atmos_phy_tb_vars
  ...
  type(AtmosPhyTbDGM_vars), public :: tb_dg_vars
...
contains
  ...
  subroutine ATMOS_PHY_TB_vars_setup
    ...
    if ( ATMOS_sw_dyn_DGM ) then
      call tb_dg_vars%Init( dg_mesh3d )
!      if ( FILE_HISTORY_CARTESC_initialized ) call tb_dg_vars%Setup_hist()
    end if
