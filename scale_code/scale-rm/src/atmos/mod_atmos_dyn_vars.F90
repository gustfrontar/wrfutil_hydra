...
module mod_atmos_dyn_vars
  ...
  character(len=H_LONG),  public :: ATMOS_DYN_DGM_RESTART_IN_BASENAME       = ''                  !< Basename of the input  file
  ...
  character(len=H_LONG),  public :: ATMOS_DYN_DGM_RESTART_OUT_BASENAME      = ''                  !< Basename of the output file
  ...
  type(AtmosDynDGM_vars), public :: dyn_dg_vars
...
contains
  ...
  subroutine ATMOS_DYN_vars_setup
    ...
    use scale_file_history_cartesC, only: &
       FILE_HISTORY_CARTESC_initialized
    implicit none

    namelist / PARAM_ATMOS_DYN_VARS / &
       ATMOS_DYN_RESTART_IN_BASENAME,           &
       ATMOS_DYN_DGM_RESTART_IN_BASENAME,       &
       ...
       ATMOS_DYN_DGM_RESTART_OUT_BASENAME,      &
    ...
    if ( ATMOS_sw_dyn_DGM ) then
      call dyn_dg_vars%Init( dg_mesh3D )
      call dyn_dg_vars%Setup_restart( &
         ATMOS_DYN_DGM_RESTART_IN_BASENAME, ATMOS_DYN_RESTART_IN_POSTFIX_TIMELABEL,   &
         ATMOS_DYN_DGM_RESTART_OUT_BASENAME, ATMOS_DYN_RESTART_OUT_POSTFIX_TIMELABEL, &
         ATMOS_DYN_RESTART_OUT_DTYPE, ATMOS_DYN_RESTART_OUT_TITLE )

      if ( FILE_HISTORY_CARTESC_initialized ) call dyn_dg_vars%Setup_hist()
    end if
  ...
  subroutine ATMOS_DYN_vars_restart_open
    ...
    if ( ATMOS_sw_dyn_DGM ) then
      LOG_INFO("ATMOS_DYN_vars_restart_open",*) 'DGM '
      call dyn_dg_vars%Open_restart()
    endif
  ...
  subroutine ATMOS_DYN_vars_restart_read
    ...
    if ( ATMOS_sw_dyn_DGM ) call dyn_dg_vars%Read_restart()
  ...
  subroutine ATMOS_DYN_vars_restart_create
    ...
    if ( ATMOS_sw_dyn_DGM ) then
      call dyn_dg_vars%Create_restart()
    endif
  ...
  subroutine ATMOS_DYN_vars_restart_enddef
    ...
    if ( ATMOS_sw_dyn_DGM ) call dyn_dg_vars%Enddef_restart()
  ...
  subroutine ATMOS_DYN_vars_restart_close
    ...
    if ( ATMOS_sw_dyn_DGM ) call dyn_dg_vars%Close_restart()
  ...
  subroutine ATMOS_DYN_vars_restart_def_var
    ...
    if ( ATMOS_sw_dyn_DGM ) call dyn_dg_vars%Defvar_restart()
  ...
  subroutine ATMOS_DYN_vars_restart_write
    ...
    if ( ATMOS_sw_dyn_DGM ) call dyn_dg_vars%Write_restart()
  ...
  subroutine ATMOS_DYN_vars_history
   implicit none
   !---------------------------------------------------------------------------
   if ( ATMOS_sw_dyn_DGM ) then
      call dyn_dg_vars%Put_hist()
   end if

   return
  end subroutine ATMOS_DYN_vars_history
