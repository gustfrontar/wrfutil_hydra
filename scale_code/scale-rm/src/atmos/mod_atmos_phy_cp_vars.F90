...
module mod_atmos_phy_cp_vars
  ...
  ! for SPRINTARS
  real(RP), public, allocatable :: ATMOS_PHY_CP_flux_qr_total(:,:,:)  ! rain flux for SPRINTARS
  real(RP), public, allocatable :: ATMOS_PHY_CP_flux_qs_total(:,:,:)  ! snow flux for SPRINTARS
  !-----------------------------------------------------------------------------
  !
  !++ Private procedure
  !
...
contains
  ...
  subroutine ATMOS_PHY_CP_vars_setup
    ...
    ! ### for SPRINTARS
    if( ATMOS_PHY_AE_TYPE == 'SPRINTARS' ) then
       allocate( ATMOS_PHY_CP_flux_qr_total(0:KA,IA,JA) )
       allocate( ATMOS_PHY_CP_flux_qs_total(0:KA,IA,JA) )
       ATMOS_PHY_CP_flux_qr_total(:,:,:) = 0.0_RP
       ATMOS_PHY_CP_flux_qs_total(:,:,:) = 0.0_RP
       !$acc enter data copyin(ATMOS_PHY_CP_flux_qr_total,ATMOS_PHY_CP_flux_qs_total)
    endif
  ...
  subroutine ATMOS_PHY_CP_vars_finalize
    ...
    ! ### for SPRINTARS
    if( ATMOS_PHY_AE_TYPE == 'SPRINTARS' ) then
       !$acc exit data delete(ATMOS_PHY_CP_flux_qr_total,ATMOS_PHY_CP_flux_qs_total)
       deallocate( ATMOS_PHY_CP_flux_qr_total )
       deallocate( ATMOS_PHY_CP_flux_qs_total )
    endif
