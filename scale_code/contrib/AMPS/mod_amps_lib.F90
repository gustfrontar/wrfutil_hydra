module mod_amps_lib
  ...
  real(RP), parameter, private :: p00 = 100000.0_RP
  real(RP), parameter, private :: r  = 287.04_RP
  real(RP), parameter, private :: rm = 461.50_RP
  real(RP), parameter, private :: cp = 1004.64_RP
...
!!!  type (random_genvar),intent(in) :: rdsd0
  ...
  ! tendencies output
  real(MP_KIND), intent(inout) :: dmtendl(11,8,L),dcontendl(11,8,L),dbintendl(3,2,mxnbin,L)
...
END SUBROUTINE ifc_cloud_micro
      ...
      use maxdims
      use com_amps
      use mod_amps_utility, only: read_AMPSTASK &
              ,RDCETB &
              ,RDAPTB &
              ,RDSTTB &
...
!
      ...
      if (nbr /= 40 .and. nbr /= 80) then
         write(*,*) "# of bins for liquid is not 40 or 80", nbr
         call MPI_abort(MPI_COMM_WORLD, -1, ierr)
      endif
      if (nbi /= 10 .and. nbi /= 20 .and. nbi /= 40) then
         write(*,*) "# of bins for ice is not 10 or 20 or 40", nbi
         call MPI_abort(MPI_COMM_WORLD, -1, ierr)
      endif
      if (nbr == 40) then
...
!     calculation of bin boundaries
         if(size(binbr).lt.nbr) then
            if ( IsMaster ) then
              write(fid_alog,*) "# of bins for liquid is not enough in binbr",size(binbr) &
                   ,nbr
            end if
            write(*,*) "# of bins for liquid is not enough in binbr", size(binbr), nbr
            call MPI_abort(MPI_COMM_WORLD, -1, ierr)
         endif
         if(size(binbi).lt.nbi) then
            if ( IsMaster ) then
              write(fid_alog,*) "# of bins for ice is not enough in binbi",size(binbi) &
                   ,nbi
            end if
            write(*,*) "# of bins for ice is not enough in binbi", size(binbi), nbi
            call MPI_abort(MPI_COMM_WORLD, -1, ierr)
         endif
...
!sheba         write(fid_alog,*) "minmass_r: ",minmass_r
           ...
           do i=1,nbin_h+1
              write(fid_alog,'(I5,3ES15.6)') i,binbr(i),dsrat_h,(binbr(i)/coef3)**(1.0/3.0)
           end do
           do i=nbin_h+2,nbr+1
              binbr(i)=binbr(i-1)*dsrat
              write(fid_alog,'(I5,3ES15.6)') i,binbr(i),dsrat,(binbr(i)/coef3)**(1.0/3.0)
...
!     for ice category
         ...
         elseif(nbi==10) then
            ...
            if ( IsMaster ) then
              write(fid_alog,*) "Standard setup for binb chosen for ice."
            end if
            write(*,*) "# of bins for ice is not 40, 20, 10", nbi
            call MPI_abort(MPI_COMM_WORLD, -1, ierr)
            binbi(1) = minmass_s
            do i=2,nbi+1
...
!           if((binbr(i)/coef3)**(1.0/3.0).gt.14.0e-4) then
         ...
         if ( IsMaster ) then
           write(fid_alog,*) "Split bin for liquid: ",isplit_bin_liq
         end if
...
!     find smallest and largest bin numbers necessary for breakup calculation
         ...
         if(ibinr_frg0==0.or.ibinr_frg1==0) then
            if ( IsMaster ) then
              write(fid_alog,*)  &
                  "ibinr_frg0.or.ibinr_frg1 is 0. check the bin boundary"
            end if
            write(*,*) "ibinr_frg0.or.ibinr_frg1 is 0. check the bin boundary"
            call MPI_abort(MPI_COMM_WORLD, -1, ierr)
         end if
         if ( IsMaster ) then
...
!-------------------------------------------------------------------
      ...
      use par_amps
      use com_amps
      use mod_amps_utility, only: get_inact_tropic2,cdfnor
      implicit none
...
!           uniform distribution
      ...
      do ica=1,nca
        ...
        sd=ap_sig_cp(ica)
        zn=0.0d+0
        write(fid_alog,*) "AP contact parameter mean and sd:",ica,mean,sd
        if(mean<=0.0.or.sd<=0.0) then
          write(fid_alog,*) "Need to specify contact parameter variables"
          stop
        endif
        ...
        write(fid_alog,*) "AP cdf of 0 and 180 deg",ica,cdf_cp_0(ica),cdf_cp_180m0(ica)
      enddo
...
!   assume the ratio of each distributions.
      r1=66.6/(66.6+133.0+3.1)
      r2=133.0/(66.6+133.0+3.1)
      r3=3.1/(66.6+133.0+3.1)

      write(fid_alog,*) "CCN setup"
      do i=1,nz
         if(z(i)<=z2) then 
            qapv(acon_q,1,1,i)=log10(c1)+(log10(c2)-log10(c1)) &
                         /(z2-z1)*(z(i)-z1)
            qapv(acon_q,1,1,i)=10.0**qapv(acon_q,1,1,i)
         ...
         write(fid_alog,254) z(i),qapv(acon_q,1,1,i),qapv(acon_q,1,3,i) &
              ,qapv(acon_q,1,4,i) &
...
!         si=es/esi*(sw+1.0)-1.0_RP
         ...
         write(fid_alog,255) z(k),qapv(acon_q,1,2,k),til-273.16_RP,si
      end do
...
!           lognormal distribution
            coef_ap(ica)=4.188790205*den_apt(ica) &
                 *exp(4.5*ap_lnsig(ica)**2.0+3.0*log(ap_mean(ica)))
            write(fid_alog,*) "AP lognormal",ica,coef_ap(ica)
         elseif(dtype_a(ica)==4) then
!           uniform distribution
            coef_ap(ica)=4.188790205*den_apt(ica) &
                 *ap_mean(ica)**3.0
            write(fid_alog,*) "AP uniform",ica,coef_ap(ica)
         end if
      end do
...
!    only one category here
      ...
      do i=1,nz
         ...
         write(fid_alog,254) z(i),qapv(acon_q,1,1,i)
...
!         si=es/esi*(sw+1.0)-1.0
         ...
         qapv(acon_q,1,2,k)=get_inact_tropic2(si,til)
         write(fid_alog,255) z(k),qapv(acon_q,1,2,k),til-273.16,si,es,esi
      end do
...
!     is given by M=N (4*PI/3)*den * exp(9*lnsig**2/2+3*ln(am))
      ...
      use par_amps
      use com_amps
      use mod_amps_utility, only: get_inact_pamarcmip,cdfnor
      implicit none
...
!    only one category here
      ...
      do i=1,nz
         ...
         write(fid_alog,254) z(i),qapv(acon_q,1,1,i), qapv(acon_q,1,3,i), qapv(acon_q,1,4,i)
...
!     set background concentration and height
      ...
      do k=1,nz 
         ...
         write(fid_alog,255) z(k),qapv(acon_q,1,2,k)!, til
      end do
...
!!!      c1=0.29e-3  ! Fridlind et al (2011)
      c1=0.16e-3  ! Fridlind et al (2011)
      z1=1000.0
!     set background concentration and height
!GCSS      c2=1.7e-3
!!!      c2=0.29e-3 ! Fridlind et al (2011)
      c2=0.16e-3 ! Fridlind et al (2011)
      z2=2000.0
...
!sheba         QAP01D(acon_q,1,2,i)=a*exp(-b*Z(i))
         ...
         write(fid_alog,255) z(i),qapv(acon_q,1,2,i)
      end do
...
!     activated IN cocnetration
              ...
              write(fid_alog,*) "qapv check",ica,k,qapv(acon_q,1,ica,k)&
      ...
      use par_amps
      use maxdims
      use mod_amps_utility, only : cdfnor
      implicit none
...
!     set up the mean mass for a AP particle
         ...
         write(fid_alog,*) "ap_mass ini",ica,ap_mass(ica)
...
!GCSS      c1=1.7e-3
!!!      c1=0.29e-3  ! Fridlind et al (2011)
      c1=0.15e-3  ! Fridlind et al (2011)
      z1=1000.0
!     set background concentration and height
!GCSS      c2=1.7e-3
!!!      c2=0.29e-3 ! Fridlind et al (2011)
      c2=0.15e-3 ! Fridlind et al (2011)
      z2=2000.0
!sheba      b=log(c1/c2)/(z2-z1)
!sheba      a=c1*(c1/c2)**(z1/(z2-z1))
      write(fid_alog,*) "IN setup 1"
...
!tmp       srat_r, sadd_r, minmass_r, 1,1.0,1.0,1.0,1.0,binbr)
  ...
  if(size(bu_tmass)<i1d_pair_max) then
    if ( IsMaster ) then
      write(fid_alog,*) "Error.  Increase the bu_tmass array"
    end if
    write(*,*) "Error.  Increase the bu_tmass array"
    call MPI_abort(MPI_COMM_WORLD, -1, ierr)
  endif
  if(size(bu_fd)/2<kk_max) then
    if ( IsMaster ) then
      write(fid_alog,*) "Error.  Increase the bu_fd array"
    end if
    write(*,*) "Error.  Increase the bu_fd array"
    call MPI_abort(MPI_COMM_WORLD, -1, ierr)
  endif
...
!    io_print = IO_get_available_fid()
    ...
    do jj=jmin_bk,ii-1
       ...
       do k=1,g%n_bin
          ...
          write(io_print,'(5es15.6)') g%ms(ii,1)%len,g%ms(jj,1)%len,d2,bu_fd(1,kk),bu_fd(2,kk)
       end do
    end do
    close(io_print)
  end if
...
subroutine cal_thetail(thetail,NGRIDS,qr,qi,pt,T)
  ...
  real(MP_KIND),parameter :: p00=1.0e+5

  real(MP_KIND) :: pit,theta

  ! this is the easiest parcel model
  do i=1,NGRIDS
     pit=cp*(pt(i)/p00)**(Ra/cp)
     theta=T(i)*cp/pit
...
!org  fid_alog=mpi_getfid(111)
  ...
  fid_alog = 9998
    !ierr = IO_CNF_open(fname_ampslog, &  ! [IN]
    !                   is_master=.true.) ! [IN]
  open(fid_alog,file=trim(fname_ampslog),form='formatted',iostat=ierr)
  if ( ierr /= 0 ) then
     write(*,*) "AMPS log file cannot be open, check!"
     call MPI_abort(MPI_COMM_WORLD, -1, ierr)
  end if
