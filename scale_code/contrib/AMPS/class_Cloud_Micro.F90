...
! >>> 2014/10 T. Hashino added for KiD
    ...
    real(MP_KIND), intent(inout) :: dmtendl(10,8,LL), dcontendl(10,8,LL), dbintendl(3,2,mxnbin,LL)
! <<< 2014/10 T. Hashino added for KiD
    real(PS), dimension(lbin) :: &
                  dM_auto_liq,dM_accr_liq &
                 ,dM_auto_ice,dM_accr_ice &
                 ,dM_auto_rim,dM_accr_rim 
! >>> 2014/10 T. Hashino added for KiD
...
!        call PROF_rapstart('amps_update_diagnose',3)
        ...
        !cptime(5)=cptime(5)+toc(s1,r1)
        if( em /= 0 ) then
          write(fid_alog,*) "diag_pq_vap > error!"
          call MPI_abort(MPI_COMM_WORLD, -1, ierr)
        endif
!        call PROF_rapend('amps_update_diagnose',3)
...
!!c             M_v = max( M_v + give_M - need_M, min_rvapor*CM%air%TV(j)%den)
    ...
    if(any(ierror>0)) then
      do j=1,CM%L
        if(ierror(j)==1) then
          write(fid_alog,*) "need and give_M are not 0", j,need_M(j),give_M(j)
        endif
      enddo
      stop
    endif
...
!!!c          call check_length(CM%solid_hydro)
             ...
             if( need_M > 0.0 .or. give_M > 0.0 ) then
                write(fid_alog,*) "need and give_M are not 0", j,need_M,give_M
                stop
             end if
       ...
       do n=1,CM%L
          ...
          if( CM%flagp_a/=0 ) then
             ...
             if(inos>0.or.inor>0) then
                ...
                write(fid_alog,*) "check_total_apmass > something is wrong, inor or inos is not 0"
                write(fid_alog,*) inor,inos
                stop
             end if

             if(mapt_a2+mapt_r2+mapt_s2<mlmt) then
                write(fid_alog,*) "mass of AP is too low"
                stop
             else
                T0=0.0_PS
                do i=1,NACAT
...
!!!              write(fid_alog,*) "xs1,xs13",XS(imt_q,i,NSCAT,n),XS(imat_q,i,NSCAT,n)
        ...
        if(any(ierror>0)) then
          do n=1,CM%L
            if(ierror(n)>0) then
              write(fid_alog,*) "mass of AP is too low",n
            endif
          enddo
          stop
        endif
