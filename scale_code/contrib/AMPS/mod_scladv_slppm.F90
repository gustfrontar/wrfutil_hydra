...
subroutine cal_ppmcoef(ns,n2,n2max,maxgrds,xx,ngrid,CPPM)
  implicit none
  integer,intent(in) :: ns,n2,n2max,maxgrds,ngrid
  real(MP_KIND),dimension(n2) :: xx ! n2max -> n2
  real(MP_KIND),dimension(n2) :: dx
  real(MP_KIND),dimension(11,n2max,maxgrds) :: CPPM ! how n2max -> n2
  integer :: i,j
  ...
  do i=2,n2-1
     ...
     ! for a(i-1)
     CPPM(1,i,ngrid)=-(dx(i)+2.0*dx(i+1))/(dx(i-1)+dx(i))
     ! for a(i)
     CPPM(2,i,ngrid)=-(2.0*dx(i-1)+dx(i))/(dx(i+1)+dx(i))+(dx(i)+2.0*dx(i+1))/(dx(i-1)+dx(i))
     ! for a(i+1)
     CPPM(3,i,ngrid)=(2.0*dx(i-1)+dx(i))/(dx(i+1)+dx(i))
     do j=1,3
        CPPM(j,i,ngrid)=CPPM(j,i,ngrid)*dx(i)/(dx(i-1)+dx(i)+dx(i+1))
     end do
     
     !! for da2, these have unit of L^(-2)
     ! for a(i-1)
     CPPM(4,i,ngrid)=1.0/(dx(i)+dx(i-1))/(dx(i-1)+dx(i)+dx(i+1))
     ! for a(i)
     CPPM(5,i,ngrid)=(-1.0/(dx(i+1)+dx(i))-1.0/(dx(i)+dx(i-1)))/(dx(i-1)+dx(i)+dx(i+1))
     ! for a(i+1)
     CPPM(6,i,ngrid)=1.0/(dx(i+1)+dx(i))/(dx(i-1)+dx(i)+dx(i+1))
     
  end do

  ! for periodic condition
  ! for i=n2
!!c  CPPM(1,n2,ngrid)=-(dx(n2)+2.0*dx(ns))/(dx(n2-1)+dx(n2))
!!c  ! for a(n2)
!!c  CPPM(2,n2,ngrid)=-(2.0*dx(n2-1)+dx(n2))/(dx(ns)+dx(n2))+(dx(n2)+2.0*dx(ns))/(dx(n2-1)+dx(n2))
!!c  ! for a(ns)
!!c  CPPM(3,n2,ngrid)=(2.0*dx(n2-1)+dx(n2))/(dx(ns)+dx(n2))
!!c  do j=1,3
!!c     CPPM(j,n2,ngrid)=CPPM(j,n2,ngrid)*dx(n2)/(dx(n2-1)+dx(n2)+dx(ns))
!!c  end do
!!c  !! for da2
!!c  ! for a(n2-1)
!!c  CPPM(4,n2,ngrid)=1.0/(dx(n2)+dx(n2-1))/(dx(n2-1)+dx(n2)+dx(ns))
!!c  ! for a(n2)
!!c  CPPM(5,n2,ngrid)=(-1.0/(dx(ns)+dx(n2))-1.0/(dx(n2)+dx(n2-1)))/(dx(n2-1)+dx(n2)+dx(ns))
!!c  ! for a(ns)
!!c  CPPM(6,n2,ngrid)=1.0/(dx(ns)+dx(n2))/(dx(n2-1)+dx(n2)+dx(ns))
!!c  
!!c  
!!c  ! for i=ns-1
!!c  !! for da1
!!c  ! for a(i-1)
!!c  CPPM(1,i,ngrid)=-(dx(ns-1)+2.0*dx(ns))/(dx(n2-2)+dx(ns-1))
!!c  ! for a(ns-1)
!!c  CPPM(2,i,ngrid)=-(2.0*dx(n2-2)+dx(ns-1))/(dx(ns)+dx(ns-1))+(dx(ns-1)+2.0*dx(ns))/(dx(n2-2)+dx(ns-1))
!!c  ! for a(ns)
!!c  CPPM(3,i,ngrid)=(2.0*dx(n2-2)+dx(ns-1))/(dx(ns)+dx(ns-1))
!!c  do j=1,3
!!c     CPPM(j,i,ngrid)=CPPM(j,i,ngrid)*dx(ns-1)/(dx(n2-2)+dx(ns-1)+dx(ns))
!!c  end do
!!c  
!!c  !! for da2
!!c  ! for a(i-1)
!!c  CPPM(4,i,ngrid)=1.0/(dx(ns-1)+dx(n2-2))/(dx(n2-2)+dx(ns-1)+dx(ns))
!!c  ! for a(ns-1)
!!c  CPPM(5,i,ngrid)=(-1.0/(dx(ns)+dx(ns-1))-1.0/(dx(ns-1)+dx(n2-2)))/(dx(n2-2)+dx(ns-1)+dx(ns))
!!c  ! for a(ns)
!!c  CPPM(6,i,ngrid)=1.0/(dx(ns)+dx(ns-1))/(dx(n2-2)+dx(ns-1)+dx(ns))
  ...
  do i=ns,n2-2
     ...
     CPPM(7,i,ngrid)=1.0-dx(i)/(dx(i)+dx(i+1))+&
     ...
     CPPM(8,i,ngrid)=dx(i)/(dx(i)+dx(i+1))+&
          ( (dx(i-1)+dx(i))/(2.0*dx(i)+dx(i+1))-(dx(i+2)+dx(i+1))/(2.0*dx(i+1)+dx(i)) )*&
          (1.0)*2.0*dx(i+1)*dx(i)/(dx(i)+dx(i+1))/&
          (dx(i-1)+dx(i)+dx(i+1)+dx(i+2))
     ...
     CPPM(9,i,ngrid)=dx(i+1)*(dx(i+1)+dx(i+2))/(dx(i)+2.0*dx(i+1))/&
            (dx(i-1)+dx(i)+dx(i+1)+dx(i+2))            
     ! for da(i+1), dimensionless
     CPPM(10,i,ngrid)=(-1.0)*dx(i)*(dx(i-1)+dx(i))/(2.0*dx(i)+dx(i+1))/&
          (dx(i-1)+dx(i)+dx(i+1)+dx(i+2))
...
!!c            (dx(i-1)+dx(i)+dx(i+1)+dx(i+2))
     ...
     ! dimension is L^2
     CPPM(11,i,ngrid)=-1.0/(0.5*dx(i+1)+dx(i)+0.5*dx(i-1))*&
                ( (0.5*dx(i)+0.5*dx(i-1))**3.0+(0.5*dx(i+1)+0.5*dx(i))**3.0)

  end do
  CPPM(7,n2-1,ngrid)=CPPM(7,n2-2,ngrid)
  CPPM(8,n2-1,ngrid)=CPPM(8,n2-2,ngrid)
  CPPM(9,n2-1,ngrid)=CPPM(9,n2-2,ngrid)
  CPPM(10,n2-1,ngrid)=CPPM(10,n2-2,ngrid)
  CPPM(11,n2-1,ngrid)=CPPM(11,n2-2,ngrid)
  ! for periodic condition
  ! for i=n2-1
  ! for a(i)
!!c  CPPM(7,n2-1,ngrid)=1.0-dx(n2-1)/(dx(n2-1)+dx(n2))+&
!!c       ((dx(n2-2)+dx(n2-1))/(2.0*dx(n2-1)+dx(n2))-(dx(ns)+dx(n2))/(2.0*dx(n2)+dx(n2-1)))*&
!!c       (-1.0)*2.0*dx(n2)*dx(n2-1)/(dx(n2-1)+dx(n2))/&
!!c       (dx(n2-2)+dx(n2-1)+dx(n2)+dx(ns))
!!c    
!!c  ! for a(n2)
!!c  CPPM(8,n2-1,ngrid)=dx(n2-1)/(dx(n2-1)+dx(n2))+&
!!c       ((dx(n2-2)+dx(n2-1))/(2.0*dx(n2-1)+dx(n2))-(dx(ns)+dx(n2))/(2.0*dx(n2)+dx(n2-1)))*&
!!c       (1.0)*2.0*dx(n2)*dx(n2-1)/(dx(n2-1)+dx(n2))/&
!!c         (dx(n2-2)+dx(n2-1)+dx(n2)+dx(ns))
!!c  
!!c  
!!c  ! for da(n2-1)
!!c  CPPM(9,n2-1,ngrid)=dx(n2)*(dx(n2)+dx(ns))/(dx(n2-1)+2.0*dx(n2))/&
!!c       (dx(n2-2)+dx(n2-1)+dx(n2)+dx(ns))            
!!c  ! for da(n2)
!!c  CPPM(10,n2-1)=(-1.0)*dx(n2-1)*(dx(n2-2)+dx(n2-1))/(2.0*dx(n2-1)+dx(n2))/&
!!c       (dx(n2-2)+dx(n2-1)+dx(n2)+dx(ns))
!!c  
!!c  ! for i=ns-1
!!c  ! for a(i)
!!c  CPPM(7,ns-1,ngrid)=1.0-dx(ns-1)/(dx(ns-1)+dx(ns))+&
!!c       ((dx(n2)+dx(ns-1))/(2.0*dx(ns-1)+dx(ns))-(dx(ns+1)+dx(ns))/(2.0*dx(ns)+dx(ns-1)))*&
!!c       (-1.0)*2.0*dx(ns)*dx(ns-1)/(dx(ns-1)+dx(ns))/&
!!c       (dx(n2)+dx(ns-1)+dx(ns)+dx(ns+1))
!!c  
!!c  ! for a(ns)
!!c  CPPM(8,ns-1,ngrid)=dx(ns-1)/(dx(ns-1)+dx(ns))+&
!!c       ((dx(n2)+dx(ns-1))/(2.0*dx(ns-1)+dx(ns))-(dx(ns+1)+dx(ns))/(2.0*dx(ns)+dx(ns-1)))*&
!!c       (1.0)*2.0*dx(i+1)*dx(ns-1)/(dx(ns-1)+dx(i+1))/&
!!c       (dx(n2)+dx(ns-1)+dx(i+1)+dx(ns+1))
!!c  
!!c  
!!c  ! for da(ns-1)
!!c  CPPM(9,ns-1,ngrid)=dx(i+1)*(dx(i+1)+dx(ns+1))/(dx(ns-1)+2.0*dx(i+1))/&
!!c       (dx(n2)+dx(ns-1)+dx(i+1)+dx(ns+1))            
!!c  ! for da(i+1)
!!c  CPPM(10,ns-1,ngrid)=(-1.0)*dx(ns-1)*(dx(n2)+dx(ns-1))/(2.0*dx(ns-1)+dx(i+1))/&
!!c       (dx(n2)+dx(ns-1)+dx(i+1)+dx(ns+1))
...
subroutine cal_ppmcoef_eta(n1,n2,n3,n2max,n3max,ngrid,maxgrds,&
     zz,dz1,k1eta,CPPME)
  implicit none
  integer,intent(in) :: n1,n2,n3,n2max,n3max,maxgrds,ngrid
  real(MP_KIND),dimension(n1) :: zz
  real(MP_KIND) :: dz1(n2,n3),dzkp1,dzkp2,dzkm1
  integer :: k1eta(n2,n3)
  real(MP_KIND),dimension(11,n2max,n3max,maxgrds) :: CPPME ! how about n2max -> n2 n3max -> n3
  integer :: i,j,k
  

!!c  open(1,file='check_eta.dat') 
!!c  write(1,*) dz1(2,200)
!!c  do k=k1eta(2,200)+1,n1
!!c     write(1,'(15ES15.6)') zz(k)-zz(k-1)
!!c  end do
!!c  close(1)

  do j=1,n3
     do i=1,n2
        dzkp1=zz(k1eta(i,j)+1)-zz(k1eta(i,j))
        dzkp2=zz(k1eta(i,j)+2)-zz(k1eta(i,j)+1)
        dzkm1=dz1(i,j)
        ! calculate the difference of grid interfaces
        ! coefficients for calculation of slope
        !! for da1
        ! for a(i-1)
        CPPME(1,i,j,ngrid)=-(dz1(i,j)+2.0*dzkp1)/(dzkm1+dz1(i,j))
        ! for a(i)
        CPPME(2,i,j,ngrid)=-(2.0*dzkm1+dz1(i,j))/(dzkp1+dz1(i,j))+(dz1(i,j)+2.0*dzkp1)/(dzkm1+dz1(i,j))
        ! for a(i+1)
        CPPME(3,i,j,ngrid)=(2.0*dzkm1+dz1(i,j))/(dzkp1+dz1(i,j))
        do k=1,3
           CPPME(k,i,j,ngrid)=CPPME(k,i,j,ngrid)*dz1(i,j)/(dzkm1+dz1(i,j)+dzkp1)
        end do
        
        !! for da2
        ! for a(i-1)
        CPPME(4,i,j,ngrid)=1.0/(dz1(i,j)+dzkm1)/(dzkm1+dz1(i,j)+dzkp1)
        ! for a(i)
        CPPME(5,i,j,ngrid)=(-1.0/(dzkp1+dz1(i,j))-1.0/(dz1(i,j)+dzkm1))/(dzkm1+dz1(i,j)+dzkp1)
        ! for a(i+1)
        CPPME(6,i,j,ngrid)=1.0/(dzkp1+dz1(i,j))/(dzkm1+dz1(i,j)+dzkp1)
        
        ! for a(i)
        CPPME(7,i,j,ngrid)=1.0-dz1(i,j)/(dz1(i,j)+dzkp1)+&
             ((dzkm1+dz1(i,j))/(2.0*dz1(i,j)+dzkp1)-(dzkp2+dzkp1)/(2.0*dzkp1+dz1(i,j)))*&
             (-1.0)*2.0*dzkp1*dz1(i,j)/(dz1(i,j)+dzkp1)/&
             (dzkm1+dz1(i,j)+dzkp1+dzkp2)
        
        ! for a(i+1)
        CPPME(8,i,j,ngrid)=dz1(i,j)/(dz1(i,j)+dzkp1)+&
             ((dzkm1+dz1(i,j))/(2.0*dz1(i,j)+dzkp1)-(dzkp2+dzkp1)/(2.0*dzkp1+dz1(i,j)))*&
             (1.0)*2.0*dzkp1*dz1(i,j)/(dz1(i,j)+dzkp1)/&
             (dzkm1+dz1(i,j)+dzkp1+dzkp2)
        ...
        ! for da(i)
        CPPME(9,i,j,ngrid)=dzkp1*(dzkp1+dzkp2)/(dz1(i,j)+2.0*dzkp1)/&
             (dzkm1+dz1(i,j)+dzkp1+dzkp2)            
        ! for da(i+1)
        CPPME(10,i,j,ngrid)=(-1.0)*dz1(i,j)*(dzkm1+dz1(i,j))/(2.0*dz1(i,j)+dzkp1)/&
             (dzkm1+dz1(i,j)+dzkp1+dzkp2)
     
        CPPME(11,i,j,ngrid)=-1.0/(0.5*dzkp1+dz1(i,j)+0.5*dzkm1)*&
             ( (0.5*dz1(i,j)+0.5*dzkm1)**3.0+(0.5*dzkp1+0.5*dz1(i,j))**3.0)
     end do
  end do
...
subroutine cal_aLRa6_z(i,j,ns,ne,k1eta,n1,n1mx,n2mx,n3mx,lbin,istp,CPPM,CPPME,a,aLR,a6)
  ...
  real(PS) :: aLR(n1,2)
  real(RP) :: CPPM(11,n1mx),CPPME(11,n2mx,n3mx)
...
!!c  do n=1,n3mx
!!c     write(1,'(15ES15.6)') (CPPME(m,2,n),m=1,11)
!!c  end do
!!c  close(1)
!!c  stop
  ...
  ! calculate the slopes
  if(ns<=k1eta+1) then
     da(k1eta)=CPPME(1,i,j)*a(k1eta-1)+CPPME(2,i,j)*a(k1eta)+CPPME(3,i,j)*a(k1eta+1)
     ...
     da2(k1eta)=CPPME(4,i,j)*a(k1eta-1)+CPPME(5,i,j)*a(k1eta)+CPPME(6,i,j)*a(k1eta+1)
  end if
  ...
  ! interpolation at the left and right interfaces of cell i
  if(ns<=k1eta+1) then
     aLR(k1eta,2)=CPPME(7,i,j)*a(k1eta)+CPPME(8,i,j)*a(k1eta+1)+CPPME(9,i,j)*da(k1eta)+CPPME(10,i,j)*da(k1eta+1)
  ...
  if(istp==1) then
     if(ns==k1eta) then
        if(-da2(k1eta+1)*da2(k1eta-1)>0.0.or.abs(a(k1eta+1)-a(k1eta-1))-eps*min(abs(a(k1eta+1)),abs(a(k1eta-1)))>0.0) then
           ita_w=CPPME(11,i,j)*(da2(k1eta+1)-da2(k1eta-1))/(a(k1eta+1)-a(k1eta-1))
        else
           ita_w=0.0
        end if
        ita=max(0.0_PS,min(ita_1*(ita_w-ita_2),1.0_PS))
...
!tmp         enddo
!tmp
!tmp         write(*,*) CPPME(7,i,j),CPPME(8,i,j),CPPME(9,i,j),CPPME(10,i,j)
...
!!c             (aLR(ka,2)-0.5*s2*(aLR(ka,2)-aLR(ka,1)-(1.0-2.0*s2/3.0)*a6(ka)))*s1&
             den(ka)*(aLR(ka,2)-0.5*s2*(aLR(ka,2)-aLR(ka,1)-(1.0-2.0*s2/3.0)*a6(ka)))*s1&
             )/(yisum+s1)
     end if
...
!!c        flux=flux+a(n)*dz(n)
     ...
     if(kdep-1>=1) then
        ...
        write(*,*) "something wrong in vdy_scladv_slppm flux_z",kdep
        stop
     end if
     s2=s1/dz(ka)
     if(yisum+s1>0.0) then
...
!!c              yisum=yisum+dvol(n)
     ...
     if(jdep-1>=1) then
        ...
        write(*,*) "something wrong in vdy_scladv_slppm flux_y",jdep
        stop
     end if
     s2=s1/dy
     if(yisum+s1>0.0) then
...
!!c7a              xisum=xisum+dx*coslu
     ...
     if(idep-1>=1) then
        ...
        write(*,*) "something wrong in vdy_scladv_slppm flux_x",idep
        stop
     end if
     s2=s1/dx
!!c7a     s2=s1/(dx*coslu)
