...
!     private
  ...
  real(PS), parameter              :: R_u = 8.31436e+07 ! chiarui, origin 8.3143
  real(PS), parameter              :: Rdvchiarui = 287.04_RP/461.50_RP ! origin 0.622_PS
  real(PS), parameter              :: M_a = 18.016_PS/Rdvchiarui

  ! gas constant for vapor in  ergs/deg/g
  real (PS), parameter             :: R_v = 4.615e+06_PS
  real (PS), parameter             :: M_w = 18.016_PS ! chiarui, origin 18.0
...
!!c  end function Thermo_Var_
  ...
  function get_diffusivity(P,T) result (D_v)
    ...
    real (PS)               :: D_v
    real (PS), parameter:: T_0 = 273.15    ! K
    real (PS), parameter:: P_0 = 1013250.0 ! g/s^2/cm
    D_v = 0.211 * ((T/T_0)**1.94) * (P_0/P)
