...
!! @li      2021-03-04 (S.Shima) [fix] sd_ncol calculation for gfortran compatibility
!
!<
!-------------------------------------------------------------------------------
...
contains
  subroutine sdm_coales_cold(    &
                        KA,KS,KE, IA,IS,IE, JA,JS,JE, &
                        sdm_colkrnl,sdm_colbrwn,          &
    ...
    real(RP) :: nre_column
    real(RP) :: qd_scale(KA,IA,JA), qvsi_scale(KA,IA,JA), esi_scale(KA,IA,JA)
    ...
    !$omp do private(m) reduction(max:freq_max)
    do m=1,gnum
...
!OCL NORECURRENCE
    ...
    !$omp& private(m,sort_tag0m,sort_freqm,n,in1,in2,in3,in4,tc,tp)
    do m=1,gnum
...
!OCL NORECURRENCE
    !$omp  do schedule(static) &
    !$omp& private(m,sort_tag0m,sort_freqm,n,tc,tp,icptc,icptp) &
    !$omp& private(sd_li1,sd_r1,sd_vz1) &
    !$omp& private(sd_li2,sd_r2,sd_vz2) &
    !$omp& private(sd_re1,sd_rp1,sd_rho1,sd_eqr1,sd_maxD1,sd_phi1,sd_area_ce1,sd_area1) &
    !$omp& private(sd_re2,sd_rp2,sd_rho2,sd_eqr2,sd_maxD2,sd_phi2,sd_area_ce2,sd_area2) &
    !$omp& private(sumr,dvz,sd_rd,sd_t,tdeg,sd_dvisc,size_ratio,nre,n_kn,C_sc,n_st) &
    !$omp& private(n_mixFr,rim_eff_plate,nre_column,rim_eff_column,var_k2,var_k1) &
    !$omp& private(ri,rj,rk,i,j,k)
    do m=1,gnum
       if( sort_freq(m) <= 1 ) cycle
...
!!$                                              * dvz
             ...
             sd_maxD2 = 2.0_RP * max(sd_re2,sd_rp2) ! maximum dimension of the particle [m]
             ...
             if(sd_vz1>sd_vz2) then ! collector is droplet
                ...
                !###### Raynolds number
                nre = sd_rd * (2.0_RP*sd_r1) * sd_vz1 / sd_dvisc
                !###### Knudsen number
                n_kn = lambda_mfp/(2.0*sd_eqr2)
                !###### Cunningham slip correction factor
                C_sc = 1.0_RP + n_kn * (para_Asc + para_Bsc * exp(-para_Csc/n_kn) )
                !###### Stokes number
...
!!$                                              * dvz
             ...
             sd_maxD1 = 2.0_RP * max(sd_re1,sd_rp1) ! maximum dimension of the particle [m]
             ...
             if(sd_vz2>sd_vz1) then ! collector is droplet
                ...
                !###### Raynolds number
                nre = sd_rd * (2.0_RP*sd_r2) * sd_vz2 / sd_dvisc
                !###### Knudsen number
                ...
                !###### mixed Froude number
                n_mixFr = (sd_vz1 - sd_vz2) * sd_vz2 / ((sd_maxD1/2.0_RP) * grav_mks)
                call riming_efficiency_Beard_Grover_1974(c_rate(tc),size_ratio,nre,n_mixFr)

                !### Erfani and Mitchell (2017)
                sd_phi1 = sd_rp1/sd_re1
                if(sd_phi1<1.0_RP)then ! oblate
                   call riming_efficiency_Erfani_Mitchell_2017_plate(rim_eff_plate,nre,n_mixFr)
                   c_rate(tc) = sd_phi1*c_rate(tc) +(1.0_RP-sd_phi1)*rim_eff_plate
                else ! prolate
                   !###### Raynolds number based on column width
                   nre_column = sd_rd * (2.0_RP*sd_re1) * sd_vz1 / sd_dvisc
                   call riming_efficiency_Erfani_Mitchell_2017_column(rim_eff_column,nre_column,n_mixFr)
                   c_rate(tc) = (1.0_RP/sd_phi1)*c_rate(tc) +(1.0_RP-(1.0_RP/sd_phi1))*rim_eff_column
                end if
...
!!$             if(min(sd_rho1,sd_rho2)<10.0_RP)then
!!$                c_rate(tc) = 0.0_RP
!!$             end if
...
!             c_rate(tc) = c_rate(tc) * (1.0-tanh(-log(min(sdi%rho(icptc),sdi%rho(icptp))/100.0)))/2.0
             ...
             sd_area1 = ONE_PI * sd_re1 * (sd_maxD1/2.0_RP) * (sd_rho1/rhoi_mks)**var_k1

             var_k2 = exp( -var_k_coef*sd_rp2/sd_re2 )
             sd_area2 = ONE_PI * sd_re2 * (sd_maxD2/2.0_RP) * (sd_rho2/rhoi_mks)**var_k2
...
!OCL NORECURRENCE
       !$omp  do schedule(static) &
       !$omp& private(m,sort_tag0m,sort_freqm,n,tc,tp,icptc,icptp) &
       !$omp& private(i,j,k,s,k12)                &
       !$omp& private(sd_c1,sd_c2,sd_cc1,sd_cc2,sd_d1,sd_d2,sd_dia1,sd_dia2)   &
       !$omp& private(sd_g1,sd_g2,sd_lmd1,sd_lmd2,sd_m1,sd_m2,sd_rw1,sd_rw2)   &
       !$omp& private(sd_tmasl1,sd_tmasl2,sd_tvasl1,sd_tvasl2,sd_v1,sd_v2)     &
       !$omp& private(sd_li1,sd_li2)     &
       !$omp& private(lmd_crs,p_crs,t_crs,vis_crs)                      &
       !$omp& private(sumc,sumd,sumdia,sumg,ivvol,tdeg,dtmp)
       do m=1,gnum
          if( sort_freq(m) <= 1 ) cycle
          ...
          do n=1,sort_freqm/2
             ...
             if(sd_li1 .eq. STAT_LIQ) then
                ...
                sd_rw1 = (sdi%re(icptc)**2 * sdi%rp(icptc))**(1.0_RP/3.0_RP)
                ! equivalent radius
             ...
             if(sd_li2 .eq. STAT_LIQ) then
                ...
                sd_rw2 = (sdi%re(icptp)**2 * sdi%rp(icptp))**(1.0_RP/3.0_RP)
                ! equivalent radius
                ! just an idea. no rigorous justification to use this.
             end if
             ...
             dtmp = sqrt(8.0_RP*mass_air*1.E-3_RP/(ONE_PI*rrst*t_crs))
             ...
             sd_c1 = sqrt(dtmp/sd_m1)
             sd_c2 = sqrt(dtmp/sd_m2)
             ...
             sumd   = sd_d1   + sd_d2
             sumc   = sqrt( sd_c1*sd_c1 + sd_c2*sd_c2 )
             sumg   = sqrt( sd_g1*sd_g1 + sd_g2*sd_g2 )
...
!OCL NORECURRENCE
    ...
    do m=1,gnum
       ...
       do n=1,sort_freqm/2
          ...
          c_rate(tc) = c_rate(tc) * real( sd_nmax, kind=RP ) * real(ipremium, kind=RP )
       end do
    end do
    !$omp end do nowait
...
!OCL NORECURRENCE
    !$omp  do schedule(static) &
    !$omp& private(m,sort_tag0m,sort_freqm,n,tc,tp,icptc,icptp) &
    !$omp& private(k,s,frac) &
    !$omp& private(sd_n1,sd_r1,sd_rk1,sd_asl1,sd_m1,sd_li1)     &
    !$omp& private(sd_n2,sd_r2,       sd_asl2,sd_m2,sd_li2) &
    !$omp& private(sd_re1,sd_rp1,sd_rho1,sd_tf1,sd_mrime1,sd_nmono1,sd_vz1)     &
    !$omp& private(sd_re2,sd_rp2,sd_rho2,sd_tf2,sd_mrime2,sd_nmono2,sd_vz2)     &
    !$omp& private(sd_ncol,dtmp) &
    !$omp& private(ri,rj,rk,i,j,sd_t,sd_rd,sd_qv,qvsi_sd,esi_sd)
    do m=1,gnum
       if( sort_freq(m) <= 1 ) cycle
       ...
       do n=1,sort_freqm/2
          ...
          sd_ncol = int( min( int(sd_ncol,kind=DP), sd_n1/sd_n2) )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine sdm_outcome_riming(sd_ncol,sd_r1,sd_r2,    &
    ...
    real(RP), intent(in)  :: qvsi_sd   ! Saturation mixing ratio over ice of the grid contained the SD [kg/kg]
    real(RP), intent(in)  :: esi_sd    ! Saturation vapor pressure over ice of the grid contained the SD [kg/kg]

    real(RP) :: sd_m2_tmp
    real(RP) :: sd_re2_tmp,sd_rp2_tmp,sd_rho2_tmp,sd_v2_tmp, sd_mrime2_tmp
    integer(i2) :: sd_li2_tmp
    ...
    if( (sd_li1 .eq. STAT_ICE) .and. (sd_li2 .eq. STAT_LIQ) ) then ! riming
       if( sd_r2 > max(sd_re1,sd_rp1) )then ! droplet is larger than ice
          ...
          ! update re
          sd_re2_tmp = (sd_m2_tmp/sd_rho2_tmp/(F_THRD*ONE_PI))**(1.0_RP/3.0_RP)
          ! update rp
          ...
          call sdm_rho_riming_Heymsfield_Pflaum_1985(rho_rime,sd_r2,sd_re1,sd_rp1,sd_vz2,sd_vz1,sd_t,sd_rd,sd_qv,qvsi_sd,esi_sd)
!          if(sd_re1 > sd_rp1) then ! oblate
          ! mimick tumbling if the ice is quasi spherical (Jensen and Harrington, 2015)
          if((0.8_RP * sd_re1 > sd_rp1) .or. ((1.25_RP * sd_re1 > sd_rp1) .and. (sd_rp1 >= sd_re1))) then ! oblate and quasi spherical prolate
             sd_m2_tmp = F_THRD * ONE_PI * (sd_re1**2*sd_rp1*sd_rho1*real(sd_ncol,kind=RP) + sd_r2**3*rhow_mks)
       ...
       if( sd_r1 > max(sd_re2,sd_rp2) )then ! droplet is larger than ice
          ...
          call sdm_rho_riming_Heymsfield_Pflaum_1985(rho_rime,sd_r1,sd_re2,sd_rp2,sd_vz1,sd_vz2,sd_t,sd_rd,sd_qv,qvsi_sd,esi_sd)
!          if(sd_re2 > sd_rp2) then ! oblate
          ! mimick tumbling if the ice is quasi spherical (Jensen and Harrington, 2015)
          if((0.8_RP * sd_re2 > sd_rp2) .or. ((1.25_RP * sd_re2 > sd_rp2) .and. (sd_rp2 >= sd_re2))) then ! oblate and quasi spherical prolate
             sd_m2_tmp = F_THRD * ONE_PI * (sd_r1**3*rhow_mks*real(sd_ncol,kind=RP) + sd_re2**2*sd_rp2*sd_rho2)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine sdm_outcome_aggregation_shima2(                 &
    ...
    real(RP),parameter :: const_beta = 2.2_RP     ! power low of the mass of snow aggregate. (Schmitt and Hemsfield 2010)
    real(RP),parameter :: const_lambda = 3.0_RP   ! factor for oblate collector (Shima 2017)
    real(RP),parameter :: const_eta    = 1.5_RP   ! factor for prolate collector (Shima 2017)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine sdm_outcome_aggregation_Chen_Lamb_1994_mod(     &
    ...
    real(RP),parameter :: const_s = 0.6_RP ! parameter to determine the distance of two particles (Chen and Lamb 1994)
    ...
    const_theta_max = ONE_PI/4.0_RP ! maximum contact angle of two particles (Chen and Lamb 1994)

    sd_rho_ave = (sd_re1**2*sd_rp1*sd_rho1 + sd_re2**2*sd_rp2*sd_rho2)/(sd_re1**2*sd_rp1 + sd_re2**2*sd_rp2)

    if( max(sd_re1,sd_rp1) > max(sd_re2,sd_rp2) )then ! ice1 is bigger than ice2
       theta = (const_theta_max)*(1.0_RP-sd_rho_ave/rhoi_mks) ! contact angle
       height = sqrt(2.0_RP * ((sd_re2**2+sd_rp2**2) - abs(sd_re2**2-sd_rp2**2)*cos(2.0_RP*theta)) )
       width  = sqrt(2.0_RP * ((sd_re2**2+sd_rp2**2) + abs(sd_re2**2-sd_rp2**2)*cos(2.0_RP*theta)) )

       if(sd_re1 > sd_rp1) then ! collector is oblate
          distance_L = const_s * (sd_re1 + max(sd_re2,sd_rp2))
          excess_length = max( (distance_L+width/2.0_RP)-sd_re1, 0.0_RP )
          sd_rp2_tmp = sd_rp1 + height/2.0_RP
          sd_re2_tmp = sqrt( sd_re1*(sd_re1+excess_length/2.0_RP) )
          sd_m2_tmp   = F_THRD*ONE_PI*(sd_re1**2*sd_rp1*sd_rho1 + sd_re2**2*sd_rp2*sd_rho2)
          sd_v2_tmp   = F_THRD*ONE_PI*(sd_re2_tmp**2*sd_rp2_tmp)
          sd_rho2_tmp = sd_m2_tmp/sd_v2_tmp
       else ! collector is prolate
          distance_L = const_s * (sd_rp1 + max(sd_re2,sd_rp2))
          excess_length = max( (distance_L+width/2.0_RP)-sd_rp1, 0.0_RP )
          sd_rp2_tmp = sd_rp1 + excess_length/2.0_RP
          sd_re2_tmp = sqrt( sd_re1*(sd_re1+height/2.0_RP) )
          sd_m2_tmp   = F_THRD*ONE_PI*(sd_re1**2*sd_rp1*sd_rho1 + sd_re2**2*sd_rp2*sd_rho2)
          sd_v2_tmp   = F_THRD*ONE_PI*(sd_re2_tmp**2*sd_rp2_tmp)
          sd_rho2_tmp = sd_m2_tmp/sd_v2_tmp
       end if
       ...
       theta = (const_theta_max)*(1.0_RP-sd_rho_ave/rhoi_mks) ! contact angle
       height = sqrt(2.0_RP * ((sd_re1**2+sd_rp1**2) - abs(sd_re1**2-sd_rp1**2)*cos(2.0_RP*theta)) )
       width  = sqrt(2.0_RP * ((sd_re1**2+sd_rp1**2) + abs(sd_re1**2-sd_rp1**2)*cos(2.0_RP*theta)) )

       if(sd_re2 > sd_rp2) then ! collector is oblate
          distance_L = const_s * (sd_re2 + max(sd_re1,sd_rp1))
          excess_length = max( (distance_L+width/2.0_RP)-sd_re2, 0.0_RP )
          sd_rp2_tmp = sd_rp2 + height/2.0_RP
          sd_re2_tmp = sqrt( sd_re2*(sd_re2+excess_length/2.0_RP) )
          sd_m2_tmp   = F_THRD*ONE_PI*(sd_re1**2*sd_rp1*sd_rho1 + sd_re2**2*sd_rp2*sd_rho2)
          sd_v2_tmp   = F_THRD*ONE_PI*(sd_re2_tmp**2*sd_rp2_tmp)
          sd_rho2_tmp = sd_m2_tmp/sd_v2_tmp
       else ! collector is prolate
          distance_L = const_s * (sd_rp2 + max(sd_re1,sd_rp1))
          excess_length = max( (distance_L+width/2.0_RP)-sd_rp2, 0.0_RP )
          sd_rp2_tmp = sd_rp2 + excess_length/2.0_RP
          sd_re2_tmp = sqrt( sd_re2*(sd_re2+height/2.0_RP) )
          sd_m2_tmp   = F_THRD*ONE_PI*(sd_re1**2*sd_rp1*sd_rho1 + sd_re2**2*sd_rp2*sd_rho2)
          sd_v2_tmp   = F_THRD*ONE_PI*(sd_re2_tmp**2*sd_rp2_tmp)
          sd_rho2_tmp = sd_m2_tmp/sd_v2_tmp
       end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine riming_efficiency_Beard_Grover_1974(E_rime,p,nre,n_mixFr)
    ...
    real(RP),intent(out) :: E_rime

    real(RP),intent(in) :: p         ! (radius of small particle)/(radius of large droplet)
    real(RP),intent(in) :: nre       ! Reynolds number
    real(RP),intent(in) :: n_mixFr   ! mixed Froude number

    real(DP),parameter :: A0 = -0.1007_DP, &
                          A1 = -0.358_DP,  &
                          A2 =  0.0261_DP

    real(DP),parameter :: B0 =  0.1465_DP, &
                          B1 =  1.302_DP,  &
                          B2 = -0.607_DP,  &
                          B3 =  0.293_DP
    ...
    z = log(n_mixFr/K0)
    H = max(B0 + B1*z + B2*z**2 + B3*z**3,0.0_DP)
    yc0 = (2.0_DP/ONE_PI)*atan(H)
    E_rime = (yc0+p)**2/(1.0_DP+p)**2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  real(DP) :: nre_lmtd ! min(nre,120.0_DP)
  real(DP) :: n_mixFr_lmtd ! min(nre,35.0_DP)
  real(DP) :: K_thres, K_crit
  real(RP) :: E_rime1,E_rime2
  ...
  nre_lmtd = min(nre,120.0_RP)
  n_mixFr_lmtd = min(n_mixFr,35.0_RP)

  K_thres = -5.07e-10_DP*nre_lmtd**5 &
       &    +1.73e-7_DP* nre_lmtd**4 &
       &    -2.17e-5_DP* nre_lmtd**3 &
       &    +0.0013_DP*nre_lmtd**2 &
       &    -0.037_DP* nre_lmtd    &
       &    +0.8355_DP

  if     (nre_lmtd <=  10.0_DP)then
     K_crit = 1.250_DP*nre_lmtd**(-0.350_DP)
  else if(nre_lmtd <=  40.0_DP)then
     K_crit = 1.072_DP*nre_lmtd**(-0.301_DP)
  else if(nre_lmtd <= 120.0_DP)then
     K_crit = 0.356_DP*nre_lmtd**(-0.003_DP)
  end if

  if(n_mixFr_lmtd <= 0.35_DP) then
     E_rime = 0.787_DP*n_mixFr_lmtd**(0.988_DP)*max(0.263_DP*log(nre_lmtd)-0.264_DP, 0.0_DP)
  else if(n_mixFr_lmtd <= K_thres) then
     E_rime = (0.7475_DP*log10(n_mixFr_lmtd)+0.620_DP)*max(0.263_DP*log(nre_lmtd)-0.264_DP, 0.0_DP)
!!$  else
!!$     E_rime = sqrt( max(1.0d0-(log10(n_mixFr_lmtd/K_crit)-sqrt(5.0d0))**2/5.0d0, 0.0d0) )
!!$  end if
  else
     E_rime1 = (0.7475_DP*log10(K_thres)+0.620_DP)*max(0.263_DP*log(nre_lmtd)-0.264_DP, 0.0_DP)
     E_rime2 = sqrt( max(1.0_DP-(log10(n_mixFr_lmtd/K_crit)-sqrt(5.0_DP))**2/5.0_DP, 0.0_DP) )
...
subroutine riming_efficiency_Erfani_Mitchell_2017_column(E_rime,nre,n_mixFr)
  ...
  real(RP),intent(in) :: nre       ! Reynolds number (based on column width, not length)
  real(RP),intent(in) :: n_mixFr   ! mixed Froude number

  real(DP) :: nre_lmtd ! min(nre,20.0_DP)
  real(DP) :: n_mixFr_lmtd ! min(nre,20.0_DP)
  real(DP) :: K_thres, K_crit
  real(DP) :: para_r
  ...
  nre_lmtd = min(nre,20.0_RP)
  n_mixFr_lmtd = min(n_mixFr,20.0_RP)

  if(nre_lmtd <= 2.0_DP)then
     K_thres = 0.0251_DP*nre_lmtd**2 &
          &   -0.0144_DP*nre_lmtd    &
          &   +0.811_DP
  else
     K_thres =-0.0003_DP*nre_lmtd**3 &
          &   +0.0124_DP*nre_lmtd**2 &
          &   -0.1634_DP*nre_lmtd    &
          &   +1.0075_DP
  end if

  if(nre_lmtd <= 1.7_DP)then ! eq(19) of the paper is opposite
     para_r = 0.7422_DP*nre_lmtd**(0.2111_DP)
  else
     para_r = 0.8025_DP*nre_lmtd**(0.0604_DP)
  end if

  if(nre_lmtd <= 1.7_DP)then
     K_crit = 0.7797_DP*nre_lmtd**(-0.009_DP)
  else
     K_crit = 1.0916_DP*nre_lmtd**(-0.635_DP)
  end if

  if(n_mixFr_lmtd <= K_thres) then
     if(nre_lmtd <= 3.0_DP)then
        E_rime = 0.787_DP*n_mixFr_lmtd**(0.988_DP) &
             &  *(-0.0121_DP*nre_lmtd**2 + 0.1297_DP*nre_lmtd + 0.0598_DP)
     else
        E_rime = 0.787_DP*n_mixFr_lmtd**(0.988_DP) &
             &  *(-0.0005_DP*nre_lmtd**2 + 0.1028_DP*nre_lmtd + 0.0359_DP)
     end if
  else
     E_rime = para_r*sqrt( max(1.0_DP-(log10(n_mixFr_lmtd/K_crit)-sqrt(3.5_DP))**2/3.5_DP, 0.0_DP) )
  end if

end subroutine riming_efficiency_Erfani_Mitchell_2017_column
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine sdm_rho_riming_Heymsfield_Pflaum_1985(rho_rime,sd_r,sd_re,sd_rp,sd_vz_drop,sd_vz_ice,sd_t,sd_rd,sd_qv,qvsi_sd,esi_sd)
  use scale_const, only:  &
       rhow_mks => CONST_DWATR, &  ! density of liquid water [kg/m^3]
       t0 => CONST_TEM00           ! 0 degC in [K]
  use m_sdm_common, only: &
  ...
  !###### Raynolds number
  sd_maxD_ice = 2.0_RP * max(sd_re,sd_rp) ! maximum dimension of the ice particle [m]
  n_re = sd_rd * sd_maxD_ice * sd_vz_ice / sd_dvisc

  !###### Stokes number
  sd_eqr_ice = (sd_re**2 * sd_rp)**(O_THRD) ! equivalent radius of ice [m]
  n_st = 2.0_RP*sd_vz_ice*sd_r**2*rhow_mks/(9.0_RP*sd_dvisc*sd_eqr_ice)
  ...
  call sdm_ice_surface_temperature(temp_icesfc,sd_t,sd_qv,qvsi_sd,esi_sd) ! surface temp in [K]
  temp_icesfc = temp_icesfc - t0     !! [K] => [degC]
  temp_icesfc = min(temp_icesfc,-0.01_RP) ! limiter not to exceed 0 degC

  !################## evaluate the Y variable
  var_Y = -drop_radi_um*impact_vel/temp_icesfc

  !################## rho_rime in [g/cm^3]
  if( (temp_icesfc <= -5.0_RP) .or. (var_Y < 1.6_RP) )then
     rho_rime = 0.30_RP*var_Y**0.44_RP
  else
     var_Y = min(var_Y,3.5_RP)
     rho_rime = exp( -0.03115_RP - 1.7030_RP*var_Y + 0.9116_RP*var_Y**2 - 0.1224*var_Y**3 )
  end if
  rho_rime = max(rho_rime,0.1_RP)
  rho_rime = min(rho_rime,0.91_RP)
...
subroutine sdm_impact_velocity_Rasummsen_Heymsfield_1985(ratio_impact_vel,n_re,n_st)
  ...
  if     (n_re<( 10.0_RP+ 30.0_RP)*0.5_RP)then
     if  (n_st< 0.4_RP)then
        ratio_impact_vel = 0.0_RP
     else if(n_st<10.0_RP)then
        ratio_impact_vel = 0.1701_RP + 0.7246_RP*var_w + 0.2257_RP*var_w**2 - 1.13_RP*var_w**3 + 0.5756_RP*var_w**4
     else
        ratio_impact_vel = 0.57_RP
     end if
  else if(n_re<( 30.0_RP+100.0_RP)*0.5_RP)then
     if  (n_st< 0.1_RP)then
        ratio_impact_vel = 0.0_RP
     else if(n_st<10.0_RP)then
        ratio_impact_vel = 0.2927_RP + 0.5085_RP*var_w - 0.03453_RP*var_w**2 - 0.2184_RP*var_w**3 + 0.03595_RP*var_w**4
     else
        ratio_impact_vel = 0.59_RP
     end if
  else if(n_re<(100.0_RP+300.0_RP)*0.5_RP)then
     if  (n_st< 0.1_RP)then
        ratio_impact_vel = 0.0_RP
     else if(n_st<10.0_RP)then
        ratio_impact_vel = 0.3272_RP + 0.4907_RP*var_w - 0.09452_RP*var_w**2 - 0.1906_RP*var_w**3 + 0.07105_RP*var_w**4
     else
        ratio_impact_vel = 0.61_RP
     end if
  else
     if  (n_st< 0.1_RP)then
        ratio_impact_vel = 0.0_RP
     else if(n_st<10.0_RP)then
        ratio_impact_vel = 0.356_RP + 0.4738_RP*var_w - 0.1233_RP*var_w**2 - 0.1618_RP*var_w**3 + 0.08087_RP*var_w**4
     else
        ratio_impact_vel = 0.63_RP
     end if
  end if

  ratio_impact_vel = max(ratio_impact_vel,0.0_RP)

  return
end subroutine sdm_impact_velocity_Rasummsen_Heymsfield_1985
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine sdm_ice_surface_temperature(temp_icesfc,t_sd,qv_sd,qvsi_sd,esi_sd)
  use m_sdm_common, only: &
!!$       F_THRD, ONE_PI, &
