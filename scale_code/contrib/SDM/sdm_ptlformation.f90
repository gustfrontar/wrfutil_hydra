...
!!$    sdnum_s2c = nint(sdininum_s2c) + bufsiz
  ...
  ! Input variables
    ...
    ymin = GRID_FY(JS-1)
    ymax = GRID_FY(JE)
    call sdm_sdcreate(DENS, RHOT, QTRC,                   &
                         rng_s2c,                &
  ...
  !-----------------------------------------------------------------------------
  subroutine sdm_fillboundaries(DENS, RHOT, QTRC,                   &
                         sd_rng,             & 
  ...
  ! Input variables
    ...
    if(sd_num<sd_fmnum)then
       if( IO_L ) then
          write(IO_FID_LOG,*) "[SDM] Error: sd_num<sd_fmnum. Sizes of SD arrays are not consistent."
          call PRC_abort
       endif
    end if
    ...
    if( (sdm_wbc==2) .and. (.NOT. PRC_HAS_W ) ) then
       ...
       if( (available_sdnum-cnt) < nint(dtmp) ) then
          if( IO_L ) write(IO_FID_LOG,*) 'ERROR: Number of SDs not sufficient for filling the boundaries'
          if( IO_L ) write(IO_FID_LOG,*) 'Num of SDs needed:',nint(dtmp),', Remaining SDs: ', (available_sdnum-cnt)
          call PRC_abort
       end if
...
!!$          sd_vz(id_invd) = sd_fmvz(n)
    ...
    if( (sdm_ebc==2) .and. (.NOT. PRC_HAS_E ) ) then
       ...
       if( (available_sdnum-cnt) < nint(dtmp) ) then
          if( IO_L ) write(IO_FID_LOG,*) 'ERROR: Number of SDs not sufficinet for filling the boundaries'
          if( IO_L ) write(IO_FID_LOG,*) 'Num of SDs needed:',nint(dtmp),', Remaining SDs: ', (available_sdnum-cnt)
          call PRC_abort
       end if
...
!!$          sd_vz(id_invd) = sd_fmvz(n)
  ...
  !-----------------------------------------------------------------------------
  subroutine sdm_sdcreate(DENS, RHOT, QTRC,                   &
                         sd_rng,                &
                         xmin,xmax,ymin,ymax,zmin,zmax,sdm_dtcmph,sdm_calvar,   &
                         sdm_rdnc,sdm_aslset,                    &
                         sdm_inisdnc,sdm_sdnmlvol,               &
...
!!$       endif
    ...
    if( iexced<0 ) then
       write(*,*) "overflow of multiplicity"
       call PRC_abort
    end if
    ...
    call sdm_z2rk( sd_num,sd_x,sd_y,sd_z,sd_ri,sd_rj,sd_rk )
