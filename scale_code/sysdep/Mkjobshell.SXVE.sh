...
if [ ! ${SNOCONF} = "NONE" ]; then
   CONFLIST=(`echo ${SNOCONF} | tr -s ',' ' '`)
   ndata=${#CONFLIST[@]}
   for n in `seq 1 ${ndata}`
   do
      let i="n - 1"
      nn=`expr \( ${PROCLIST_SNO[i]} - 1 \) / 8 + 1`
      nnp=`expr ${PROCLIST_SNO[i]} / ${nn}`
      RUN_SNO=`echo -e "${RUN_SNO}\n"${MPIEXEC} -nn ${nn} -nnp ${nnp} ${BINDIR}/${SNONAME} ${CONFLIST[i]} "|| exit 1"`
   done
fi
