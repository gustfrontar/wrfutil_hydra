!-------------------------------------------------------------------------------
!> module Map projection
!!
!! @par Description
!!          Map projection module
!!
!! @author Team SCALE
!!
!! @par History
!! @li      2013-10-24 (H.Yashiro)  [new]
!!
!<
!-------------------------------------------------------------------------------
module scale_mapproj
  !-----------------------------------------------------------------------------
  !
  !++ used modules
  !
  use common
  use common_namelist
  use scale_const , only : CONST_PI , CONST_UNDEF , CONST_D2R , CONST_RADIUS
  !use scale_precision
  !use scale_stdio
  !use scale_prof
  !use scale_grid_index
  !-----------------------------------------------------------------------------
  implicit none
  private
  !-----------------------------------------------------------------------------
  !
  !++ Public procedure
  !
  public :: MPRJ_setup
  public :: MPRJ_xy2lonlat
  public :: MPRJ_lonlat2xy
  public :: MPRJ_mapfactor
  public :: MPRJ_rotcoef

  interface MPRJ_rotcoef
     module procedure MPRJ_rotcoef_0D
     module procedure MPRJ_rotcoef_2D
  end interface MPRJ_rotcoef

  !-----------------------------------------------------------------------------
  !
  !++ Public parameters & variables
  !
  !real(RP), public :: MPRJ_basepoint_lon = 135.221d0 ! position of base point (domain center) in real world [deg]
  !real(RP), public :: MPRJ_basepoint_lat =  34.653d0 ! position of base point (domain center) in real world [deg]

  !-----------------------------------------------------------------------------
  !
  !++ Private procedure
  !
  private :: MPRJ_None_setup
  private :: MPRJ_LambertConformal_setup
  private :: MPRJ_PolarStereographic_setup
  private :: MPRJ_Mercator_setup
  private :: MPRJ_EquidistantCylindrical_setup

  private :: MPRJ_None_xy2lonlat
  private :: MPRJ_LambertConformal_xy2lonlat
  private :: MPRJ_PolarStereographic_xy2lonlat
  private :: MPRJ_Mercator_xy2lonlat
  private :: MPRJ_EquidistantCylindrical_xy2lonlat

  private :: MPRJ_None_lonlat2xy
  private :: MPRJ_LambertConformal_lonlat2xy
  private :: MPRJ_PolarStereographic_lonlat2xy
  private :: MPRJ_Mercator_lonlat2xy
  private :: MPRJ_EquidistantCylindrical_lonlat2xy

  private :: MPRJ_None_mapfactor
  private :: MPRJ_LambertConformal_mapfactor
  private :: MPRJ_PolarStereographic_mapfactor
  private :: MPRJ_Mercator_mapfactor
  private :: MPRJ_EquidistantCylindrical_mapfactor

  private :: MPRJ_None_rotcoef_2D
  private :: MPRJ_LambertConformal_rotcoef_2D
  private :: MPRJ_PolarStereographic_rotcoef_2D
  private :: MPRJ_Mercator_rotcoef_2D
  private :: MPRJ_EquidistantCylindrical_rotcoef_2D

  private :: MPRJ_None_rotcoef_0D
  private :: MPRJ_LambertConformal_rotcoef_0D
  private :: MPRJ_PolarStereographic_rotcoef_0D
  private :: MPRJ_Mercator_rotcoef_0D
  private :: MPRJ_EquidistantCylindrical_rotcoef_0D

  !-----------------------------------------------------------------------------
  !
  !++ Private parameters & variables
  !
  !character(len=4), private :: MPRJ_type = 'NONE' !< map projection type ->
  !Moved to namelist JR
  !                                             ! 'NONE'
  !                                             ! 'LC'
  !                                             ! 'PS'
  !                                             ! 'MER'
  !                                             ! 'EC'

  real(r_size), private :: MPRJ_hemisphere         ! hemisphere flag: 1=north, -1=south

  !real(r_size), private :: MPRJ_basepoint_x        ! position of base point in the model [m]  -> Moved to namelist JR
  !real(r_size), private :: MPRJ_basepoint_y        ! position of base point in the model [m]  -> Moved to namelist JR

  real(r_size), private :: MPRJ_pole_x             ! position of north/south pole in the model [m]
  real(r_size), private :: MPRJ_pole_y             ! position of north/south pole in the model [m]
  real(r_size), private :: MPRJ_eq_x               ! position of equator at the base lon. in the model [m]
  real(r_size), private :: MPRJ_eq_y               ! position of equator at the base lon. in the model [m]

  !real(r_size), private :: MPRJ_rotation =  0.0d0 ! rotation factor (only for 'NONE' type) -> Moved to namelist JR

  !real(r_size), private :: MPRJ_LC_lat1  = 30.0d0 ! standard latitude1 for L.C. projection [deg]
  !real(r_size), private :: MPRJ_LC_lat2  = 60.0d0 ! standard latitude2 for L.C. projection [deg]
  real(r_size), private :: MPRJ_LC_c               ! conformal factor
  real(r_size), private :: MPRJ_LC_fact            ! pre-calc factor

  !real(r_size), private :: MPRJ_PS_lat             ! standard latitude1 for P.S. projection [deg]
  real(r_size), private :: MPRJ_PS_fact            ! pre-calc factor

  !real(r_size), private :: MPRJ_M_lat    =  0.0d0 ! standard latitude1 for Mer. projection [deg]
  real(r_size), private :: MPRJ_M_fact             ! pre-calc factor

  !real(r_size), private :: MPRJ_EC_lat   =  0.0d0 ! standard latitude1 for E.C. projection [deg]
  real(r_size), private :: MPRJ_EC_fact            ! pre-calc factor

  !real(r_size), private :: PI
  !real(r_size), private :: D2R
  !real(r_size), private :: RADIUS

  !-----------------------------------------------------------------------------
contains
  !-----------------------------------------------------------------------------
  !> Setup
  subroutine MPRJ_setup( DOMAIN_CENTER_X, DOMAIN_CENTER_Y )
  !  use scale_process, only: &
  !     PRC_MPIstop
    !use scale_const, only: &
    !   UNDEF     => CONST_UNDEF,  &
    !   PI_RP     => CONST_PI,     &
    !   D2R_RP    => CONST_D2R,    &
    !   RADIUS_RP => CONST_RADIUS
    implicit none

    real(r_size), intent(in) :: DOMAIN_CENTER_X !< center position of global domain [m]: x
    real(r_size), intent(in) :: DOMAIN_CENTER_Y !< center position of global domain [m]: y

    !namelist / PARAM_MAPPROJ / &
    !   MPRJ_basepoint_lon, &
    !   MPRJ_basepoint_lat, &
    !   MPRJ_basepoint_x,   &
    !   MPRJ_basepoint_y,   &
    !   MPRJ_type,          &
    !   MPRJ_rotation,      &
    !   MPRJ_LC_lat1,       &
    !   MPRJ_LC_lat2,       &
    !   MPRJ_PS_lat,        &
    !   MPRJ_M_lat,         &
    !   MPRJ_EC_lat

    integer :: ierr
    !---------------------------------------------------------------------------

    !if( IO_L ) write(IO_FID_LOG,*)
    !if( IO_L ) write(IO_FID_LOG,*) '++++++ Module[MAPPROJ] / Categ[ATMOS-RM GRID] / Origin[SCALElib]'

    !PI     = real(PI_RP,     r_size)
    !D2R    = real(D2R_RP,    r_size)
    !RADIUS = real(RADIUS_RP, r_size)

    MPRJ_basepoint_x = CONST_UNDEF
    MPRJ_basepoint_y = CONST_UNDEF
    MPRJ_PS_lat      = CONST_UNDEF

    MPRJ_basepoint_x = DOMAIN_CENTER_X
    MPRJ_basepoint_y = DOMAIN_CENTER_Y

    !--- read namelist
    !rewind(IO_FID_CONF)
    !read(IO_FID_CONF,nml=PARAM_MAPPROJ,iostat=ierr)

    !if( ierr < 0 ) then !--- missing
    !   if( IO_L ) write(IO_FID_LOG,*) '*** Not found namelist. Default used.'
    !elseif( ierr > 0 ) then !--- fatal error
    !   write(*,*) 'xxx Not appropriate names in namelist PARAM_MAPPROJ. Check!'
    !   call PRC_MPIstop
    !endif
    !if( IO_NML ) write(IO_FID_NML,nml=PARAM_MAPPROJ)

    if( MPRJ_PS_lat == CONST_UNDEF ) MPRJ_PS_lat = MPRJ_basepoint_lat

    !if( IO_L ) write(IO_FID_LOG,*)
    !if( IO_L ) write(IO_FID_LOG,*) '*** Map projection type : ', trim(MPRJ_type)
    select case(trim(MPRJ_type))
    case('NONE')
       !if( IO_L ) write(IO_FID_LOG,*) '*** => NO map projection'
       call MPRJ_None_setup
    case('LC')
       !if( IO_L ) write(IO_FID_LOG,*) '*** => Lambert Conformal projection'
       call MPRJ_LambertConformal_setup
    case('PS')
       !if( IO_L ) write(IO_FID_LOG,*) '*** => Polar Stereographic projection'
       call MPRJ_PolarStereographic_setup
    case('MER')
       !if( IO_L ) write(IO_FID_LOG,*) '*** => Mercator projection'
       call MPRJ_Mercator_setup
    case('EC')
       !if( IO_L ) write(IO_FID_LOG,*) '*** => Equidistant Cylindrical projection'
       call MPRJ_EquidistantCylindrical_setup
    case default
       write(*,*) 'xxx Unsupported MPRJ_type. STOP'
       !call PRC_MPIstop
       STOP
    endselect

    !if( IO_L ) write(IO_FID_LOG,'(1x,A,F15.3)') '*** Basepoint(x)    = ', MPRJ_basepoint_x
    !if( IO_L ) write(IO_FID_LOG,'(1x,A,F15.3)') '*** Basepoint(y)    = ', MPRJ_basepoint_y

    return
  end subroutine MPRJ_setup

  !-----------------------------------------------------------------------------
  !> (x,y) -> (lon,lat)
  subroutine MPRJ_xy2lonlat( &
       x,   &
       y,   &
       lon, &
       lat  )
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none

    real(r_size), intent(in)  :: x
    real(r_size), intent(in)  :: y
    real(r_size), intent(out) :: lon ! [rad]
    real(r_size), intent(out) :: lat ! [rad]
    !---------------------------------------------------------------------------

    select case(MPRJ_type)
    case('NONE')
       call MPRJ_None_xy2lonlat( x, y, lon, lat )
    case('LC')
       call MPRJ_LambertConformal_xy2lonlat( x, y, lon, lat )
    case('PS')
       call MPRJ_PolarStereographic_xy2lonlat( x, y, lon, lat )
    case('MER')
       call MPRJ_Mercator_xy2lonlat( x, y, lon, lat )
    case('EC')
       call MPRJ_EquidistantCylindrical_xy2lonlat( x, y, lon, lat )
    case default
       write(*,*) 'xxx Unsupported MPRJ_type. STOP'
       !call PRC_MPIstop
       STOP
    endselect

    return
  end subroutine MPRJ_xy2lonlat

  !-----------------------------------------------------------------------------
  !> (lon,lat) -> (x,y)
  subroutine MPRJ_lonlat2xy( &
       lon, &
       lat, &
       x,   &
       y    )
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none

    real(r_size), intent(in)  :: lon ! [rad]
    real(r_size), intent(in)  :: lat ! [rad]
    real(r_size), intent(out) :: x
    real(r_size), intent(out) :: y
    !---------------------------------------------------------------------------

            select case(MPRJ_type)
    case('NONE')
       call MPRJ_None_lonlat2xy( lon, lat, x, y )
    case('LC')
       call MPRJ_LambertConformal_lonlat2xy( lon, lat, x, y )
    case('PS')
       call MPRJ_PolarStereographic_lonlat2xy( lon, lat, x, y )
    case('MER')
       call MPRJ_Mercator_lonlat2xy( lon, lat, x, y )
    case('EC')
       call MPRJ_EquidistantCylindrical_lonlat2xy( lon, lat, x, y )
    case default
       write(*,*) 'xxx Unsupported MPRJ_type. STOP'
       !call PRC_MPIstop
       STOP
    endselect

    return
  end subroutine MPRJ_lonlat2xy

  !-----------------------------------------------------------------------------
  !> (x,y) -> (lon,lat)
  subroutine MPRJ_mapfactor( &
       lat, &
       m1,  &
       m2 , nlon , nlat  )
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(in)  :: lat(nlon,nlat) ! [rad]
    real(r_size), intent(out) :: m1 (nlon,nlat)
    real(r_size), intent(out) :: m2 (nlon,nlat)
    !---------------------------------------------------------------------------

    select case(MPRJ_type)
    case('NONE')
       call MPRJ_None_mapfactor( lat, m1, m2 , nlon , nlat)
    case('LC')
       call MPRJ_LambertConformal_mapfactor( lat, m1, m2 , nlon , nlat)
    case('PS')
       call MPRJ_PolarStereographic_mapfactor( lat, m1, m2 , nlon , nlat)
    case('MER')
       call MPRJ_Mercator_mapfactor( lat, m1, m2 , nlon , nlat )
    case('EC')
       call MPRJ_EquidistantCylindrical_mapfactor( lat, m1, m2 , nlon , nlat)
    case default
       write(*,*) 'xxx Unsupported MPRJ_type. STOP'
       !call PRC_MPIstop
       STOP
    endselect

    return
  end subroutine MPRJ_mapfactor

  !-----------------------------------------------------------------------------
  !> u(lat,lon) = cos u(x,y) - sin v(x,y)
  !> v(lat,lon) = sin u(x,y) + cos v(x,y)
  subroutine MPRJ_rotcoef_0D( &
       rotc, &
       lon,  &
       lat   )
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none

    real(r_size), intent(out) :: rotc(2) !< rotc(:,:,1)->cos, rotc(:,:,2)->sin
    real(r_size), intent(in)  :: lon   ! [rad]
    real(r_size), intent(in)  :: lat   ! [rad]
    !---------------------------------------------------------------------------

    select case(MPRJ_type)
    case('NONE')
       call MPRJ_None_rotcoef_0D( rotc )
    case('LC')
       call MPRJ_LambertConformal_rotcoef_0D( rotc, lon, lat )
    case('PS')
       call MPRJ_PolarStereographic_rotcoef_0D( rotc, lon, lat )
    case('MER')
       call MPRJ_Mercator_rotcoef_0D( rotc )
    case('EC')
       call MPRJ_EquidistantCylindrical_rotcoef_0D( rotc )
    case default
       write(*,*) 'xxx Unsupported MPRJ_type. STOP'
       !call PRC_MPIstop
       STOP
    endselect

    return
  end subroutine MPRJ_rotcoef_0D

  !-----------------------------------------------------------------------------
  !> u(lat,lon) = cos u(x,y) - sin v(x,y)
  !> v(lat,lon) = sin u(x,y) + cos v(x,y)
  subroutine MPRJ_rotcoef_2D( &
       rotc, &
       lon,  &
       lat, nlon , nlat   )
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(out) :: rotc(nlon,nlat,2) !< rotc(:,:,1)->cos, rotc(:,:,2)->sin
    real(r_size), intent(in)  :: lon (nlon,nlat) ! [rad]
    real(r_size), intent(in)  :: lat (nlon,nlat) ! [rad]
    !---------------------------------------------------------------------------

    select case(MPRJ_type)
    case('NONE')
       call MPRJ_None_rotcoef_2D( rotc , nlon , nlat)
    case('LC')
       call MPRJ_LambertConformal_rotcoef_2D( rotc, lon, lat , nlon , nlat)
    case('PS')
       call MPRJ_PolarStereographic_rotcoef_2D( rotc, lon, lat , nlon , nlat)
    case('MER')
       call MPRJ_Mercator_rotcoef_2D( rotc , nlon , nlat)
    case('EC')
       call MPRJ_EquidistantCylindrical_rotcoef_2D( rotc ,nlon , nlat)
    case default
       write(*,*) 'xxx Unsupported MPRJ_type. STOP'
       !call PRC_MPIstop
       STOP
    endselect

    return
  end subroutine MPRJ_rotcoef_2D

  !-----------------------------------------------------------------------------
  !> No projection
  subroutine MPRJ_None_setup
    implicit none
    !---------------------------------------------------------------------------

    !if( IO_L ) write(IO_FID_LOG,*)
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_rotation   = ', MPRJ_rotation

    return
  end subroutine MPRJ_None_setup

  !-----------------------------------------------------------------------------
  !> No projection, lon,lat are determined by gnomonic projection: (x,y) -> (lon,lat)
  subroutine MPRJ_None_xy2lonlat( &
       x,   &
       y,   &
       lon, &
       lat  )
    implicit none

    real(r_size), intent(in)  :: x
    real(r_size), intent(in)  :: y
    real(r_size), intent(out) :: lon ! [rad]
    real(r_size), intent(out) :: lat ! [rad]

    real(r_size) :: gno(2)
    real(r_size) :: rho, gmm
    !---------------------------------------------------------------------------

    gno(1) = ( (y-MPRJ_basepoint_y) * sin(MPRJ_rotation*CONST_D2R) &
             + (x-MPRJ_basepoint_x) * cos(MPRJ_rotation*CONST_D2R) ) / CONST_RADIUS
    gno(2) = ( (y-MPRJ_basepoint_y) * cos(MPRJ_rotation*CONST_D2R) &
             - (x-MPRJ_basepoint_x) * sin(MPRJ_rotation*CONST_D2R) ) / CONST_RADIUS

    rho = sqrt( gno(1) * gno(1) + gno(2) * gno(2) )
    gmm = atan( rho )

    if ( rho == 0.0d0 ) then
       lon = MPRJ_basepoint_lon * CONST_D2R
       lat = MPRJ_basepoint_lat * CONST_D2R
    else
       lon = MPRJ_basepoint_lon * CONST_D2R &
           + atan( gno(1)*sin(gmm) / ( rho   *cos(MPRJ_basepoint_lat*CONST_D2R)*cos(gmm) &
                                     - gno(2)*sin(MPRJ_basepoint_lat*CONST_D2R)*sin(gmm) ) )
       lat = asin(        sin(MPRJ_basepoint_lat*CONST_D2R)*cos(gmm)       &
                 + gno(2)*cos(MPRJ_basepoint_lat*CONST_D2R)*sin(gmm) / rho )
    endif

    return
  end subroutine MPRJ_None_xy2lonlat

  !-----------------------------------------------------------------------------
  !> No projection
  subroutine MPRJ_None_lonlat2xy( &
       lon, &
       lat, &
       x,   &
       y    )
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none

    real(r_size), intent(in)  :: lon ! [rad]
    real(r_size), intent(in)  :: lat ! [rad]
    real(r_size), intent(out) :: x
    real(r_size), intent(out) :: y

    real(r_size) :: lat_d, lat0_d, dlon
    real(r_size) :: gno(2)
    real(r_size) :: cos_gmm
    !---------------------------------------------------------------------------
    ! http://mathworld.wolfram.com/GnomonicProjection.html

    lat_d  = real(lat,kind=r_size)
    lat0_d = real(MPRJ_basepoint_lat*CONST_D2R,kind=r_size)

    dlon = lon - MPRJ_basepoint_lon * CONST_D2R

    cos_gmm = sin(lat0_d) * sin(lat_d) &
            + cos(lat0_d) * cos(lat_d) * cos(dlon)

    gno(1) = ( cos(lat_d)  * sin(dlon)  ) / cos_gmm
    gno(2) = ( cos(lat0_d) * sin(lat_d) &
             - sin(lat0_d) * cos(lat_d) * cos(dlon) ) / cos_gmm

    x = MPRJ_basepoint_x + ( gno(1) * cos(MPRJ_rotation*CONST_D2R) &
                           - gno(2) * sin(MPRJ_rotation*CONST_D2R) ) * CONST_RADIUS
    y = MPRJ_basepoint_y + ( gno(1) * sin(MPRJ_rotation*CONST_D2R) &
                           + gno(2) * cos(MPRJ_rotation*CONST_D2R) ) * CONST_RADIUS

    return
  end subroutine MPRJ_None_lonlat2xy

  !-----------------------------------------------------------------------------
  !> No projection: m1=m2=1
  subroutine MPRJ_None_mapfactor( &
       lat, &
       m1,  &
       m2, nlon , nlat   )
    implicit none
    integer     , intent(in)  :: nlon ,nlat
    real(r_size), intent(in)  :: lat(nlon,nlat) ! [rad]
    real(r_size), intent(out) :: m1 (nlon,nlat)
    real(r_size), intent(out) :: m2 (nlon,nlat)
    !---------------------------------------------------------------------------

    m1 = 1.0d0
    m2 = m1

    return
  end subroutine MPRJ_None_mapfactor

  !-----------------------------------------------------------------------------
  !> No projection:
  subroutine MPRJ_None_rotcoef_0D( &
       rotc )
    implicit none

    real(r_size), intent(out) :: rotc(2)
    !---------------------------------------------------------------------------

    rotc(1) = 1.0d0
    rotc(2) = 0.0d0

    return
  end subroutine MPRJ_None_rotcoef_0D

  !-----------------------------------------------------------------------------
  !> No projection:
  subroutine MPRJ_None_rotcoef_2D( &
       rotc , nlon , nlat)
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(out) :: rotc(nlon,nlat,2)
    !---------------------------------------------------------------------------

    rotc(:,:,1) = 1.0d0
    rotc(:,:,2) = 0.0d0

    return
  end subroutine MPRJ_None_rotcoef_2D

  !-----------------------------------------------------------------------------
  !> Lambert Conformal projection
  subroutine MPRJ_LambertConformal_setup
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none

    real(r_size) :: lat1rot, lat2rot
    real(r_size) :: dlon, latrot, dist
    !---------------------------------------------------------------------------

    if ( MPRJ_LC_lat1 >= MPRJ_LC_lat2 ) then
       write(*,*) 'xxx Please set MPRJ_LC_lat1 < MPRJ_LC_lat2 in degree. STOP'
       !call PRC_MPIstopa
       STOP
    endif

    ! check hemisphere: 1=north, -1=south
    MPRJ_hemisphere = sign(1.0d0,MPRJ_LC_lat1+MPRJ_LC_lat2)

    lat1rot = 0.5d0*CONST_PI - MPRJ_hemisphere * MPRJ_LC_lat1 * CONST_D2R
    lat2rot = 0.5d0*CONST_PI - MPRJ_hemisphere * MPRJ_LC_lat2 * CONST_D2R

    ! calc conformal factor c
    MPRJ_LC_c = ( log( sin(lat1rot)        ) - log( sin(lat2rot)        ) ) &
              / ( log( tan(0.5d0*lat1rot) ) - log( tan(0.5d0*lat2rot) ) )

    ! pre-calc factor
    MPRJ_LC_fact = sin(lat1rot) / MPRJ_LC_c / tan(0.5d0*lat1rot)**MPRJ_LC_c

    ! calc (x,y) at pole point
    dlon = 0.0d0

    latrot = 0.5d0*CONST_PI - MPRJ_hemisphere * MPRJ_basepoint_lat * CONST_D2R

    dist = MPRJ_LC_fact * CONST_RADIUS * tan(0.5d0*latrot)**MPRJ_LC_c

    MPRJ_pole_x = MPRJ_basepoint_x -                   dist * sin(MPRJ_LC_c*dlon)
    MPRJ_pole_y = MPRJ_basepoint_y + MPRJ_hemisphere * dist * cos(MPRJ_LC_c*dlon)

    !if( IO_L ) write(IO_FID_LOG,*)
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_LC_lat1    = ', MPRJ_LC_lat1
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_LC_lat2    = ', MPRJ_LC_lat2
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_hemisphere = ', MPRJ_hemisphere
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_LC_c       = ', MPRJ_LC_c
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_LC_fact    = ', MPRJ_LC_fact
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_pole_x     = ', MPRJ_pole_x
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_pole_y     = ', MPRJ_pole_y

    return
  end subroutine MPRJ_LambertConformal_setup

  !-----------------------------------------------------------------------------
  !> Lambert Conformal projection: (x,y) -> (lon,lat)
  subroutine MPRJ_LambertConformal_xy2lonlat( &
       x,   &
       y,   &
       lon, &
       lat  )
    implicit none

    real(r_size), intent(in)  :: x
    real(r_size), intent(in)  :: y
    real(r_size), intent(out) :: lon ! [rad]
    real(r_size), intent(out) :: lat ! [rad]

    real(r_size) :: xx, yy, dist
    !---------------------------------------------------------------------------

    xx =                    ( x - MPRJ_pole_x ) / CONST_RADIUS / MPRJ_LC_fact
    yy = -MPRJ_hemisphere * ( y - MPRJ_pole_y ) / CONST_RADIUS / MPRJ_LC_fact

    dist = sqrt( xx*xx + yy*yy )

    lon = MPRJ_basepoint_lon * CONST_D2R + atan2(xx,yy) / MPRJ_LC_c

    ! check hemisphere: 1=north, -1=south
    lat = MPRJ_hemisphere * ( 0.5d0*CONST_PI - 2.0d0*atan( dist**(1.0d0/MPRJ_LC_c) ) )

    return
  end subroutine MPRJ_LambertConformal_xy2lonlat

  !-----------------------------------------------------------------------------
  !> Lambert Conformal projection: (lon,lat) -> (x,y)
  subroutine MPRJ_LambertConformal_lonlat2xy( &
       lon, &
       lat, &
       x,   &
       y    )
    implicit none

    real(r_size), intent(in)  :: lon ! [rad]
    real(r_size), intent(in)  :: lat ! [rad]
    real(r_size), intent(out) :: x
    real(r_size), intent(out) :: y

    real(r_size) :: dlon, latrot, dist
    !---------------------------------------------------------------------------

    dlon = lon - MPRJ_basepoint_lon * CONST_D2R
    if ( dlon >  CONST_PI ) dlon = dlon - CONST_PI*2.0d0
    if ( dlon < -CONST_PI ) dlon = dlon + CONST_PI*2.0d0

    !latrot = 0.5d0*CONST_PI - lat
    latrot = 0.5d0*CONST_PI - MPRJ_hemisphere * lat ! by T.Honda (06/28/2017)

    dist = MPRJ_LC_fact * CONST_RADIUS * tan(0.5d0*latrot)**MPRJ_LC_c

    x = MPRJ_pole_x +                   dist * sin(MPRJ_LC_c*dlon)
    y = MPRJ_pole_y - MPRJ_hemisphere * dist * cos(MPRJ_LC_c*dlon)

    return
  end subroutine MPRJ_LambertConformal_lonlat2xy

  !-----------------------------------------------------------------------------
  !> Lambert Conformal projection: (lon,lat) -> (m1=m2)
  subroutine MPRJ_LambertConformal_mapfactor( &
       lat, &
       m1,  &
       m2, nlon , nlat   )
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(in)  :: lat(nlon,nlat) ! [rad]
    real(r_size), intent(out) :: m1 (nlon,nlat)
    real(r_size), intent(out) :: m2 (nlon,nlat)

    real(r_size) :: latrot
    integer :: i, j
    !---------------------------------------------------------------------------

    do j = 1, nlat
    do i = 1, nlon
       latrot = 0.5d0*CONST_PI - MPRJ_hemisphere * lat(i,j)

       m1(i,j) = MPRJ_LC_fact / sin(latrot) * MPRJ_LC_c * tan(0.5d0*latrot)**MPRJ_LC_c
       m2(i,j) = m1(i,j)
    enddo
    enddo

    return
  end subroutine MPRJ_LambertConformal_mapfactor

  !-----------------------------------------------------------------------------
  subroutine MPRJ_LambertConformal_rotcoef_0D( &
       rotc, &
       lon,  &
       lat   )
    implicit none

    real(r_size), intent(out) :: rotc(2)
    real(r_size), intent(in)  :: lon   ! [rad]
    real(r_size), intent(in)  :: lat   ! [rad]

    real(r_size) :: dlon
    real(r_size) :: alpha
    !---------------------------------------------------------------------------

    dlon = lon - MPRJ_basepoint_lon * CONST_D2R
    if ( dlon >  CONST_PI ) dlon = dlon - CONST_PI*2.0d0
    if ( dlon < -CONST_PI ) dlon = dlon + CONST_PI*2.0d0

    alpha = - MPRJ_LC_c * dlon * MPRJ_hemisphere

    rotc(1) = cos( alpha )
    rotc(2) = sin( alpha )

    return
  end subroutine MPRJ_LambertConformal_rotcoef_0D

  !-----------------------------------------------------------------------------
  subroutine MPRJ_LambertConformal_rotcoef_2D( &
       rotc, &
       lon,  &
       lat , nlon , nlat  )
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(out) :: rotc(nlon,nlat,2)
    real(r_size), intent(in)  :: lon (nlon,nlat) ! [rad]
    real(r_size), intent(in)  :: lat (nlon,nlat) ! [rad]

    real(r_size) :: dlon
    real(r_size) :: alpha

    integer  :: i, j
    !---------------------------------------------------------------------------

    do j = 1, nlat
    do i = 1, nlon
       dlon = lon(i,j) - MPRJ_basepoint_lon * CONST_D2R
       if ( dlon >  CONST_PI ) dlon = dlon - CONST_PI*2.0d0
       if ( dlon < -CONST_PI ) dlon = dlon + CONST_PI*2.0d0

       alpha = - MPRJ_LC_c * dlon * MPRJ_hemisphere

       rotc(i,j,1) = cos( alpha )
       rotc(i,j,2) = sin( alpha )
    enddo
    enddo

    return
  end subroutine MPRJ_LambertConformal_rotcoef_2D

  !-----------------------------------------------------------------------------
  !> Polar Stereographic projection
  subroutine MPRJ_PolarStereographic_setup
    implicit none

    real(r_size) :: lat0
    real(r_size) :: dlon, latrot, dist
    !---------------------------------------------------------------------------

    ! check hemisphere: 1=north, -1=south
    MPRJ_hemisphere = sign(1.0d0,MPRJ_PS_lat)

    lat0 = MPRJ_hemisphere * MPRJ_PS_lat * CONST_D2R

    ! pre-calc factor
    MPRJ_PS_fact = 1.0d0 + sin(lat0)

    ! calc (x,y) at pole point
    dlon = 0.0d0

    latrot = 0.5d0*CONST_PI - MPRJ_hemisphere * MPRJ_basepoint_lat * CONST_D2R

    dist = MPRJ_PS_fact * CONST_RADIUS * tan(0.5d0*latrot)

    MPRJ_pole_x = MPRJ_basepoint_x -                   dist * sin(dlon)
    MPRJ_pole_y = MPRJ_basepoint_y + MPRJ_hemisphere * dist * cos(dlon)

    !if( IO_L ) write(IO_FID_LOG,*)
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_PS_lat1    = ', MPRJ_PS_lat
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_hemisphere = ', MPRJ_hemisphere
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_PS_fact    = ', MPRJ_PS_fact
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_pole_x     = ', MPRJ_pole_x
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_pole_y     = ', MPRJ_pole_y

    return
  end subroutine MPRJ_PolarStereographic_setup

  !-----------------------------------------------------------------------------
  !> Polar Stereographic projection: (x,y) -> (lon,lat)
  subroutine MPRJ_PolarStereographic_xy2lonlat( &
       x,   &
       y,   &
       lon, &
       lat  )
    implicit none

    real(r_size), intent(in)  :: x
    real(r_size), intent(in)  :: y
    real(r_size), intent(out) :: lon ! [rad]
    real(r_size), intent(out) :: lat ! [rad]

    real(r_size) :: xx, yy, dist
    !---------------------------------------------------------------------------

    xx =                    ( x - MPRJ_pole_x ) / CONST_RADIUS / MPRJ_PS_fact
    yy = -MPRJ_hemisphere * ( y - MPRJ_pole_y ) / CONST_RADIUS / MPRJ_PS_fact

    dist = sqrt( xx*xx + yy*yy )

    lon = MPRJ_basepoint_lon * CONST_D2R + atan2(xx,yy)

    lat = MPRJ_hemisphere * ( 0.5d0*CONST_PI - 2.0d0*atan(dist) )

    return
  end subroutine MPRJ_PolarStereographic_xy2lonlat

  !-----------------------------------------------------------------------------
  !> Polar Stereographic projection: (lon,lat) -> (x,y)
  subroutine MPRJ_PolarStereographic_lonlat2xy( &
       lon, &
       lat, &
       x,   &
       y    )
    implicit none

    real(r_size), intent(in)  :: lon ! [rad]
    real(r_size), intent(in)  :: lat ! [rad]
    real(r_size), intent(out) :: x
    real(r_size), intent(out) :: y

    real(r_size) :: dlon, latrot, dist
    !---------------------------------------------------------------------------

    dlon = lon - MPRJ_basepoint_lon * CONST_D2R

    latrot = 0.5d0*CONST_PI - lat

    dist = MPRJ_PS_fact * CONST_RADIUS * tan(0.5d0*latrot)

    x = MPRJ_pole_x +                   dist * sin(dlon)
    y = MPRJ_pole_y - MPRJ_hemisphere * dist * cos(dlon)

    return
  end subroutine MPRJ_PolarStereographic_lonlat2xy

  !-----------------------------------------------------------------------------
  !> Polar Stereographic projection: (lon,lat) -> (m1=m2)
  subroutine MPRJ_PolarStereographic_mapfactor( &
       lat, &
       m1,  &
       m2, nlon , nlat )
    implicit none
    integer     , intent(in)  :: nlon,nlat
    real(r_size), intent(in)  :: lat(nlon,nlat) ! [rad]
    real(r_size), intent(out) :: m1 (nlon,nlat)
    real(r_size), intent(out) :: m2 (nlon,nlat)

    integer :: i, j
    !---------------------------------------------------------------------------

    do j = 1, nlat
    do i = 1, nlon
       m1(i,j) = MPRJ_PS_fact / ( 1.0d0 + sin(MPRJ_hemisphere*lat(i,j)) )
       m2(i,j) = m1(i,j)
    enddo
    enddo

    return
  end subroutine MPRJ_PolarStereographic_mapfactor

  !-----------------------------------------------------------------------------
  subroutine MPRJ_PolarStereographic_rotcoef_0D( &
       rotc, &
       lon,  &
       lat   )
    implicit none

    real(r_size), intent(out) :: rotc(2)
    real(r_size), intent(in)  :: lon   ! [rad]
    real(r_size), intent(in)  :: lat   ! [rad]

    real(r_size) :: dlon
    real(r_size) :: alpha
    !---------------------------------------------------------------------------

    dlon = lon - MPRJ_basepoint_lon * CONST_D2R
    if ( dlon >  CONST_PI ) dlon = dlon - CONST_PI*2.0d0
    if ( dlon < -CONST_PI ) dlon = dlon + CONST_PI*2.0d0

    alpha = - dlon * MPRJ_hemisphere

    rotc(1) = cos( alpha )
    rotc(2) = sin( alpha )

    return
  end subroutine MPRJ_PolarStereographic_rotcoef_0D

  !-----------------------------------------------------------------------------
  subroutine MPRJ_PolarStereographic_rotcoef_2D( &
       rotc, &
       lon,  &
       lat, nlon ,nlat   )
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(out) :: rotc(nlon,nlat,2)
    real(r_size), intent(in)  :: lon (nlon,nlat) ! [rad]
    real(r_size), intent(in)  :: lat (nlon,nlat) ! [rad]

    real(r_size) :: dlon
    real(r_size) :: alpha

    integer  :: i, j
    !---------------------------------------------------------------------------

    do j = 1, nlat
    do i = 1, nlon
       dlon = lon(i,j) - MPRJ_basepoint_lon * CONST_D2R
       if ( dlon >  CONST_PI ) dlon = dlon - CONST_PI*2.0d0
       if ( dlon < -CONST_PI ) dlon = dlon + CONST_PI*2.0d0

       alpha = - dlon * MPRJ_hemisphere

       rotc(i,j,1) = cos( alpha )
       rotc(i,j,2) = sin( alpha )
    enddo
    enddo

    return
  end subroutine MPRJ_PolarStereographic_rotcoef_2D

  !-----------------------------------------------------------------------------
  !> Mercator projection
  subroutine MPRJ_Mercator_setup
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none

    real(r_size) :: lat0
    real(r_size) :: latrot, dist
    !---------------------------------------------------------------------------

    lat0 = MPRJ_M_lat * CONST_D2R

    ! pre-calc factor
    MPRJ_M_fact = cos(lat0)

    if ( MPRJ_M_fact == 0.0d0 ) then
       write(*,*) 'xxx MPRJ_M_lat cannot be set to pole point! value=', MPRJ_M_lat
       STOP
       !call PRC_MPIstop
    endif

    ! calc (x,y) at (lon,lat) = (base,0)
    latrot = 0.5d0*CONST_PI - MPRJ_basepoint_lat * CONST_D2R

    dist = 1.0d0 / tan(0.5d0*latrot)

    MPRJ_eq_x = MPRJ_basepoint_x
    MPRJ_eq_y = MPRJ_basepoint_y - CONST_RADIUS * MPRJ_M_fact * log(dist)

    !if( IO_L ) write(IO_FID_LOG,*)
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_M_lat      = ', MPRJ_M_lat
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_M_fact     = ', MPRJ_M_fact
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_eq_x       = ', MPRJ_eq_x
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_eq_y       = ', MPRJ_eq_y

    return
  end subroutine MPRJ_Mercator_setup

  !-----------------------------------------------------------------------------
  !> Mercator projection: (x,y) -> (lon,lat)
  subroutine MPRJ_Mercator_xy2lonlat( &
       x,   &
       y,   &
       lon, &
       lat  )
    implicit none

    real(r_size), intent(in)  :: x
    real(r_size), intent(in)  :: y
    real(r_size), intent(out) :: lon ! [rad]
    real(r_size), intent(out) :: lat ! [rad]

    real(r_size) :: xx, yy
    !---------------------------------------------------------------------------

    xx = ( x - MPRJ_eq_x ) / CONST_RADIUS / MPRJ_M_fact
    yy = ( y - MPRJ_eq_y ) / CONST_RADIUS / MPRJ_M_fact

    lon = xx + MPRJ_basepoint_lon * CONST_D2R

    lat = 0.5d0*CONST_PI - 2.0d0*atan( 1.0d0/exp(yy) )

    return
  end subroutine MPRJ_Mercator_xy2lonlat

  !-----------------------------------------------------------------------------
  !> Mercator projection: (lon,lat) -> (x,y)
  subroutine MPRJ_Mercator_lonlat2xy( &
       lon, &
       lat, &
       x,   &
       y    )
    implicit none

    real(r_size), intent(in)  :: lon ! [rad]
    real(r_size), intent(in)  :: lat ! [rad]
    real(r_size), intent(out) :: x
    real(r_size), intent(out) :: y

    real(r_size) :: dlon, latrot, dist
    !---------------------------------------------------------------------------

    dlon = lon - MPRJ_basepoint_lon * CONST_D2R

    latrot = 0.5d0*CONST_PI - lat

    dist = 1.0d0 / tan(0.5d0*latrot)

    x = MPRJ_eq_x + CONST_RADIUS * MPRJ_M_fact * dlon
    y = MPRJ_eq_y + CONST_RADIUS * MPRJ_M_fact * log(dist)

    return
  end subroutine MPRJ_Mercator_lonlat2xy

  !-----------------------------------------------------------------------------
  !> Mercator projection: (lon,lat) -> (m1=m2)
  subroutine MPRJ_Mercator_mapfactor( &
       lat, &
       m1,  &
       m2, nlon ,nlat   )
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(in)  :: lat(nlon,nlat) ! [rad]
    real(r_size), intent(out) :: m1 (nlon,nlat)
    real(r_size), intent(out) :: m2 (nlon,nlat)
    integer :: i, j
    !---------------------------------------------------------------------------

    do j = 1, nlat
    do i = 1, nlon
       m1(i,j) = MPRJ_M_fact / cos(real(lat(i,j),r_size))
       m2(i,j) = m1(i,j)
    enddo
    enddo

    return
  end subroutine MPRJ_Mercator_mapfactor

  !-----------------------------------------------------------------------------
  subroutine MPRJ_Mercator_rotcoef_0D( &
       rotc )
    implicit none

    real(r_size), intent(out) :: rotc(2)
    !---------------------------------------------------------------------------

    rotc(1) = 1.0d0
    rotc(2) = 0.0d0

    return
  end subroutine MPRJ_Mercator_rotcoef_0D

  !-----------------------------------------------------------------------------
  subroutine MPRJ_Mercator_rotcoef_2D( &
       rotc , nlon , nlat)
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(out) :: rotc(nlon,nlat,2)
    !---------------------------------------------------------------------------

    rotc(:,:,1) = 1.0d0
    rotc(:,:,2) = 0.0d0

    return
  end subroutine MPRJ_Mercator_rotcoef_2D

  !-----------------------------------------------------------------------------
  !> Equidistant Cylindrical projection
  subroutine MPRJ_EquidistantCylindrical_setup
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none

    real(r_size) :: lat0
    !---------------------------------------------------------------------------

    lat0 = MPRJ_EC_lat * CONST_D2R

    ! pre-calc factor
    MPRJ_EC_fact = cos(lat0)

    if ( MPRJ_EC_fact == 0.0d0 ) then
       write(*,*) 'xxx MPRJ_EC_lat cannot be set to pole point! value=', MPRJ_EC_lat
       !call PRC_MPIstop
       STOP
    endif

    MPRJ_eq_x = MPRJ_basepoint_x
    MPRJ_eq_y = MPRJ_basepoint_y - CONST_RADIUS * MPRJ_basepoint_lat * CONST_D2R

    !if( IO_L ) write(IO_FID_LOG,*)
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_EC_lat     = ', MPRJ_EC_lat
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_EC_fact    = ', MPRJ_EC_fact
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_eq_x       = ', MPRJ_eq_x
    !if( IO_L ) write(IO_FID_LOG,*) '*** MPRJ_eq_y       = ', MPRJ_eq_y

    return
  end subroutine MPRJ_EquidistantCylindrical_setup

  !-----------------------------------------------------------------------------
  !> Equidistant Cylindrical projection: (x,y) -> (lon,lat)
  subroutine MPRJ_EquidistantCylindrical_xy2lonlat( &
       x,   &
       y,   &
       lon, &
       lat  )
    !use scale_process, only: &
    !   PRC_MPIstop
    implicit none

    real(r_size), intent(in)  :: x
    real(r_size), intent(in)  :: y
    real(r_size), intent(out) :: lon ! [rad]
    real(r_size), intent(out) :: lat ! [rad]

    real(r_size) :: xx, yy
    !---------------------------------------------------------------------------

    xx = ( x - MPRJ_eq_x ) / CONST_RADIUS / MPRJ_EC_fact
    yy = ( y - MPRJ_eq_y ) / CONST_RADIUS

    lon = xx + MPRJ_basepoint_lon * CONST_D2R

    lat = yy

    if ( abs(lat) >  0.5d0*CONST_PI ) then
       write(*,*) 'xxx Invalid latitude range! value=', lat
       !call PRC_MPIstop
       STOP
    endif

    return
  end subroutine MPRJ_EquidistantCylindrical_xy2lonlat

  !-----------------------------------------------------------------------------
  !> Equidistant Cylindrical projection: (lon,lat) -> (x,y)
  subroutine MPRJ_EquidistantCylindrical_lonlat2xy( &
       lon, &
       lat, &
       x,   &
       y    )
    implicit none

    real(r_size), intent(in)  :: lon ! [rad]
    real(r_size), intent(in)  :: lat ! [rad]
    real(r_size), intent(out) :: x
    real(r_size), intent(out) :: y

    real(r_size) :: dlon
    !---------------------------------------------------------------------------

    dlon = lon - MPRJ_basepoint_lon * CONST_D2R

    x = MPRJ_eq_x + CONST_RADIUS * MPRJ_EC_fact * dlon
    y = MPRJ_eq_y + CONST_RADIUS * lat

    return
  end subroutine MPRJ_EquidistantCylindrical_lonlat2xy

  !-----------------------------------------------------------------------------
  !> Equidistant Cylindrical projection: (lon,lat) -> (m1,m2)
  subroutine MPRJ_EquidistantCylindrical_mapfactor( &
       lat, &
       m1,  &
       m2, nlon,nlat   )
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(in)  :: lat(nlon,nlat) ! [rad]
    real(r_size), intent(out) :: m1 (nlon,nlat)
    real(r_size), intent(out) :: m2 (nlon,nlat)
    integer :: i, j
    !---------------------------------------------------------------------------

    do j = 1, nlat
    do i = 1, nlon
       m1(i,j) = MPRJ_EC_fact / cos(real(lat(i,j),r_size))
       m2(i,j) = 1.0d0
    enddo
    enddo

    return
  end subroutine MPRJ_EquidistantCylindrical_mapfactor

  !-----------------------------------------------------------------------------
  subroutine MPRJ_EquidistantCylindrical_rotcoef_0D( &
       rotc )
    implicit none

    real(r_size), intent(out) :: rotc(2)
    !---------------------------------------------------------------------------

    rotc(1) = 1.0d0
    rotc(2) = 0.0d0

    return
  end subroutine MPRJ_EquidistantCylindrical_rotcoef_0D

  !-----------------------------------------------------------------------------
  subroutine MPRJ_EquidistantCylindrical_rotcoef_2D( &
       rotc , nlon , nlat )
    implicit none
    integer     , intent(in)  :: nlon , nlat
    real(r_size), intent(out) :: rotc(nlon,nlat,2)
    !---------------------------------------------------------------------------

    rotc(:,:,1) = 1.0d0
    rotc(:,:,2) = 0.0d0

    return
  end subroutine MPRJ_EquidistantCylindrical_rotcoef_2D

end module scale_mapproj
