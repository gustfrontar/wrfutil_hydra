...
!-----------------------------------------------------------------------
  ...
  REAL(r_size),PARAMETER :: undef=-9.99d9
  REAL(r_sngl),PARAMETER :: undefs=-9.99e9
...
! TIMEINC
!-----------------------------------------------------------------------
SUBROUTINE com_timeinc_sec(itime,incr)
  IMPLICIT NONE
  INTEGER(8),INTENT(INOUT) :: itime  !initial date yyyymmddhhMMSS
  INTEGER :: iy
  INTEGER :: im
  INTEGER :: id
  INTEGER :: ih
  INTEGER :: imn
  INTEGER :: is
  INTEGER,INTENT(IN) :: incr  !increment in seconds
  LOGICAL :: CONT , LEAP

  CALL com_time2ymdh(itime,iy,im,id,ih,imn,is) 

  is = is + incr

  !From longer time unit to shorter.
   DO WHILE( is >= 86400 )
      id = id + 1
      is = is - 86400
   ENDDO
  ...
  !Correct the day
  CALL is_leap(LEAP,iy)
  CONT=.TRUE.
  DO WHILE(CONT)
    IF( id <= 28 )THEN
      CONT=.FALSE. 
    ELSE IF( id >= 29 .AND. im==2.AND. .NOT. LEAP ) THEN
      id = id - 28 
      im = im + 1
    ELSE IF(id>=30.AND.im==2 .AND. LEAP ) THEN
      id = id - 29
      im = im + 1
  ...
  CALL com_ymdh2time(iy,im,id,ih,imn,is,itime)

  RETURN
END SUBROUTINE com_timeinc_sec

SUBROUTINE is_leap(leap,iy)
...
leap=.FALSE.
   ...
   IF( MOD( iy , 4 ) == 0 )leap=.TRUE.
   IF( MOD( iy , 100 ) == 0 )leap=.FALSE.
   IF( MOD( iy , 400 ) == 0 )leap=.TRUE.
...
! TIMECONVERSION
!-----------------------------------------------------------------------
SUBROUTINE com_time2ymdh(itime,iy,im,id,ih,imn,is)
  IMPLICIT NONE
  ...
  iy = INT(  itime / 10000000000 )
  im = INT( (itime-iy*10000000000) / 100000000 )
  id = INT( (itime-iy*10000000000-im*100000000) / 1000000 )
  ih = INT( (itime-iy*10000000000-im*100000000-id*1000000 ) / 10000 )
  imn= INT( (itime-iy*10000000000-im*100000000-id*1000000-ih*10000 )/100 )
  is = INT(  itime-iy*10000000000-im*100000000-id*1000000-ih*10000-imn*100 )

  RETURN
END SUBROUTINE com_time2ymdh

SUBROUTINE com_ymdh2time(iy,im,id,ih,imn,is,itime)
  IMPLICIT NONE
  INTEGER,INTENT(IN) :: iy
  ...
  itime=iy*10000000000+im*100000000+id*1000000+ih*10000+imn*100+is
...
SUBROUTINE com_pos2ij(msw,nx,ny,flon,flat,num_obs,olon,olat,oi,oj)
  ...
  ELSE IF(msw == 3) THEN
    ! ==============================================================
    !   Surrounding 4 Grid Points Linear Interpolation
    ! ==============================================================   
    Obs_Loop_3 : DO inum=1,num_obs 
      IF(detailout) WRITE(6,'(A,I5,2F15.5)') '*** START OBS ',inum,olon(inum),olat(inum) 
      ! ------------------------------------------------------------
      !    Search Basic Point
      ! ------------------------------------------------------------ 
      ...
      jy = 1
      DO ix = 1, nx - 1
	rlon_max = maxval(flon(ix:ix + 1, jy))
        IF(rlon_max < olon(inum)) CYCLE
	rlon_min = minval(flon(ix:ix + 1, jy))
        IF(rlon_min > olon(inum)) CYCLE	  
	nxp = ix 
	EXIT 
      END DO
      ix = 1
      DO jy = 1, ny - 1
	rlat_max = maxval(flat(ix, jy:jy + 1))
        IF(rlat_max < olat(inum)) CYCLE
	rlat_min = minval(flat(ix, jy:jy + 1))
        IF(rlat_min > olat(inum)) CYCLE
	nyp = jy
	EXIT 
      END DO
      IF(detailout) WRITE(6,'(3X,A,2I7)') 'nxp, nyp =',nxp,nyp
      ...
      !    Interpolation
      ! ------------------------------------------------------------    
      oi(inum) = nxp + abs(olon(inum) - flon(nxp, nyp))/ abs(flon(nxp + 1, nyp) - flon(nxp, nyp))
      oj(inum) = nyp + abs(olat(inum) - flat(nxp, nyp))/ abs(flat(nxp, nyp + 1) - flat(nxp, nyp))
      IF(detailout) WRITE(6,'(3X,A,2F15.5)') 'position :',oi(inum), oj(inum) 
    END DO Obs_Loop_3  
  END IF
