...
mkdir $MODELDATAPATH/OBS_IDEAL_LIMITEDDV/
...
./model_to_radar_ideal_limiteddv.exe

#Copy files in radar format.
mkdir -p $MODELDATAPATH/OBS_IDEAL_LIMITEDDV/$iYY$iMM$iDD$ihh$imm$iss/
#if [ $iss -eq 00 ]
#then
#mv radarobs.dat $MODELDATAPATH/OBS_IDEAL_LIMITEDDV/$iYY$iMM$iDD$ihh$imm$iss/rad0101.dat
#else
mv radarobs.dat $MODELDATAPATH/OBS_IDEAL_LIMITEDDV/$iYY$iMM$iDD$ihh$imm$iss/rad0201.dat
#fi
