...
MODELDATAPATH=/data2/jruiz/NATURE_RUN/NATURE_RUN_1KM_SPECIFIED/BUBBLE_DATAV2/
RADARFILE=""

#Compile the code.

./make_pawr_centerradar.sh
...
mkdir -p $MODELDATAPATH/OBS/$iYY$iMM$iDD$ihh$imm/
if [ $iss -eq 00 ]
then
mv output_radar.grd $MODELDATAPATH/OBS/$iYY$iMM$iDD$ihh$imm/rad01001.dat
else
mv output_radar.grd $MODELDATAPATH/OBS/$iYY$iMM$iDD$ihh$imm/rad01002.dat
fi
