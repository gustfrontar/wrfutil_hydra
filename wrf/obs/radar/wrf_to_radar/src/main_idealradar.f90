...
! REAL(r_size),ALLOCATABLE :: radar3d(:,:,:,:)
 ...
 CHARACTER*20  :: model_file_name='input_model.nc'
  ...
  REAL(r_size)         :: u   , v  , w
  REAL(r_size)         :: dlon , dlat , dist
  REAL(r_size),ALLOCATABLE :: az(:,:,:) , elev(:,:,:)
  INTEGER              :: NOBS
...
ALLOCATE( az(nlon,nlat,nlev) , elev(nlon,nlat,nlev) )
...
!Compute dv and z at each model grid point.
 ...
 do ii=1,nlon-1
  do jj=1,nlat-1
   do kk=1,nlev-1
     ...
     az(ii,jj,kk)=rad2deg*atan2(dlon*cosd(radar_lat),dlat)
     IF( az(ii,jj,kk) .LT. 0)az(ii,jj,kk)=360.0d0 + az(ii,jj,kk)
     ...
     elev(ii,jj,kk)=rad2deg*atan2(rlev-radar_z,dist)
 ...
 !WRITE AUXILIAR OUTPUT
   IF( DEBUG )THEN
     OPEN(UNIT=60,FILE='auxout.grd',STATUS='unknown',FORM='unformatted')
     WRITE(*,*)'NX=',nlon-1,'NY=',nlat-1,'NZ=',kk-1
     DO kk=1,nlev-1
       WRITE(60)REAL(z3d(1:nlon-1,1:nlat-1,kk),r_sngl)
     ENDDO
     DO kk=1,nlev-1
       WRITE(60)REAL(dv3d(1:nlon-1,1:nlat-1,kk),r_sngl)
     ENDDO
     DO kk=1,nlev-1
       WRITE(60)REAL(az(1:nlon-1,1:nlat-1,kk),r_sngl)
     ENDDO
     DO kk=1,nlev-1
       WRITE(60)REAL(elev(1:nlon-1,1:nlat-1,kk),r_sngl)
     ENDDO
   ENDIF
