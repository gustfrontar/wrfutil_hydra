...
USE common_radar_tools
 ...
 REAL(r_size), PARAMETER :: radar_az_res = 1.0d0 , radar_r_res = 500.0d0 , radar_el_res = 5.0d0 
 ...
 METHOD_REF_CALC=2   ! 1-simple operator, 2-C band radar operator, 3-X-band operator
 ADD_OBS_ERROR=.TRUE.  !Are we going to add observation error?
 REFLECTIVITY_ERROR=2.0d0 !Error in dbz.
 RADIALWIND_ERROR=0.5d0   !Error in m/s.
 USE_WT=.TRUE.            !Are we going to use the fall speed of hydrometeors in the computation of RADIAL VELOCITY?
...
!Dealing with a fake radar, lets create the radar location and characteristics.
 RADAR_1 % lon0 = radar_lon
 RADAR_1 % lat0 = radar_lat
