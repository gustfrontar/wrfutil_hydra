...
SO = 'LETKF_SO2KM_NEWQC';        %Supperobing folder
...
% 
    ...
    [cart] = define_cartesian_grid(radar, dx, dz, maxz , maxrange ) ;
    ...
    data_rad_end_num = datenum( date_rad_ini_str , 'yyyymmddHHMMSS');

    date_rad_mean_str=datestr( 0.5*datenum(dateini,'yyyymmddHHMMSS') + 0.5*datenum(dateend,'yyyymmddHHMMSS') , 'yyyymmddHHMMSS' );
    ...
    if ( ~ initialized_so ) ;
       ...
       date_so_mean_num=(tmp*86400.0/sowindowl)*sowindowl/86400.0;
       date_so_ini_num=tmp-sowindowl/(2*86400);
       date_so_end_num=tmp+sowindowl/(2*86400);

       initialized_so = true ;

       display(['Starting a new SO window :' datestr( date_mean_so_num + refdate ,'yyyy-mm-dd-HH:MM:SS') ])
    ...
    while continue 
      ...
      if(   date_rad_end_num < date_so_end_num + refdate )
         ...
         display(['Writing data corresponding to superobbing slot centered at:' datestr( date_so_mean_num + refdate , 'yyyy-mm-dd-HH:MM:SS') ])
         fileout = [SODIR '/radar01_' datestr( date_so_mean_num + refdate ,'yyyymmddHHMMSS') ];
