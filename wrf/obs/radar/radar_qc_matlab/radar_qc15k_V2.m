...
%*********************************************************************
%Folders
OBS = 'PARANA';                  %Radar folder
CASE = '20091117';               %Case study folder
DATA = '120';                    %Range folder
INPUT = 'region_dealiased_data'; %Radar data folder
QC = 'QC_DATA';                  %QC folder
SO = 'LETKF_SO15KM_NEWQC';       %Supperobing folder
...
keep_close_ranges=false %If true this will preserve pixels where maximum height is below echo top threshold.
...
dz = 500;
maxz = 16e3;        %Maximum height for the observations
maxrange= 240e3;    %Maximum horizontal range for the observations
...
sowindowl=900;   %Length of so window in minutes.
...
for ifile=1:size(filelist,1)
    ...
    %filename = [ BASEDIR '/cfrad.20091117_200002.000_to_20091117_200342.000_PAR_SUR.nc3'  ];
    ...
    refc( echo_top_3d < echotop_thld & ref > minref ) = NaN;
    ...
    %if ( present_rhohv )
    %  rhohv( rhohv < 0 ) = NaN;
    %   [rhohvsmooth] = compute_boxmean(rhohv, 4, 4, 1);
...
% 
    ...
    %Define cartesian grid that will be used for superobbing.
    [cart] = define_cartesian_grid_v2(radar, dx, dz, maxz , maxrange );


    %Obtain date and time and add them to the radar structure 
    dateini = radar.time_coverage_start([1:4  6:7 9:10 12:13 15:16 18:19]);

    %Obtain date and time and add them to the radar structure 
    dateend = radar.time_coverage_end([1:4  6:7 9:10 12:13 15:16 18:19]);

    cdate=datestr( 0.5*datenum(dateini,'yyyymmddHHMMSS') + 0.5*datenum(dateend,'yyyymmddHHMMSS') , 'yyyymmddHHMMSS' );

    if ( ~ initialized_so ) ;
    
       dateref=[cdate(1:4) '0101000000' ];
       %This is the first volume. 
       %Get the date of the first SO.
       dateso= datenum( cdate , 'yyyymmddHHMMSS') - datenum( dateref ,'yyyymmddHHMMSS' );
       factor = 86400/sowindowl;
       dateso = round( dateso * factor ) ;
       dateso = dateso * ( 1.0/factor) + datenum( dateref , 'yyyymmddHHMMSS' );

       dateso_old=dateso;
       initialized_so = true ;

       display(['Starting a new SO window :' datestr( dateso ,'yyyy-mm-dd-HH:MM:SS') ])
       ...
       factor = 86400/sowindowl;
       dateso = round( dateso * factor ) ;
       dateso = dateso * ( 1/factor) + datenum( dateref , 'yyyymmddHHMMSS' );

       if( dateso ~= dateso_old ) 

         display(['Starting a new SO window :' datestr( dateso ,'yyyy-mm-dd-HH:MM:SS') ])

         %We need to start a new superobbing window and write the previous one.
         display(['Writing data corresponding to superobbing slot centered at:' datestr( dateso_old , 'yyyy-mm-dd-HH:MM:SS') ])
         fileout = [SODIR '/radar01_' datestr(dateso_old,'yyyymmddHHMMSS') ];
       ...
       display(['The date of the current volume is :', datestr( datenum(cdate,'yyyymmddHHMMSS') , 'yyyy-mm-dd-HH:MM:SS')])
       display(['The center of the current super-obbing period is:' datestr( dateso , 'yyyy-mm-dd-HH:MM:SS') ])

    end

    [gridrad] = radar_superobbing_4d(radar, refc , dvcc , cart, gridrad );
