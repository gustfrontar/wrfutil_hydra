...
SUBROUTINE monit_dep(nn,elm,dep,qc)
  ...
  REAL(r_size) :: rmse_u,rmse_v,rmse_t,rmse_q,rmse_ps,rmse_rh,rmse_tv
  REAL(r_size) :: bias_u,bias_v,bias_t,bias_q,bias_ps,bias_rh,bias_tv
  INTEGER :: n,iu,iv,it,iq,ips,irh,itv
  ...
  DO n=1,nn
    ...
    CASE(id_rh_obs)
      ...
      irh = irh + 1
    CASE(id_tv_obs)
      rmse_tv = rmse_tv + dep(n)**2
      bias_tv = bias_tv + dep(n)
      itv = itv + 1      
    END SELECT
  END DO
  IF(iu == 0) THEN
  ...
  IF(itv == 0) THEN
    ...
    bias_tv = undef
  ELSE
    rmse_tv = SQRT(rmse_tv / REAL(itv,r_size))
    bias_tv = bias_tv / REAL(itv,r_size)
  END IF

  WRITE(6,'(A)') '== OBSERVATIONAL DEPARTURE ========================================================='
  WRITE(6,'(7A12)') 'U','V','T','Q','PS','RH','TV'
  WRITE(6,'(7ES12.3)') bias_u,bias_v,bias_t,bias_q,bias_ps,bias_rh,bias_tv
  WRITE(6,'(7ES12.3)') rmse_u,rmse_v,rmse_t,rmse_q,rmse_ps,rmse_rh,rmse_tv
  WRITE(6,'(A)') '== NUMBER OF OBSERVATIONS TO BE ASSIMILATED ========================================'
  WRITE(6,'(7A12)') 'U','V','T','Q','PS','RH','TV'
  WRITE(6,'(7I12)') iu,iv,it,iq,ips,irh,itv
  WRITE(6,'(A)') '===================================================================================='
