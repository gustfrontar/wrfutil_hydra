...
for file = files'
    ...
    temp=inputdata(5,:)/10.0 + 273.16;
    dewt=inputdata(6,:)/10.0 + 273.16;
    pres=inputdata(7,:)/10.0;
    wdir=inputdata(8,:);
    wspd=inputdata(9,:)/10.0;
    ...
    [ uwnd vwnd ]=VelDirToUV(wspd,wdir);
    ...
    if ( ~isempty( station_index ) )
      ...
      ps=SlpToSP(pres,temp,dewt,tmpdata(4,:));
      %if( tmpdata(4,1) > 700 )
      % [ ps/100  ;  pres/100 ;  tmpdata(4,:) ]'
      %end
