...
!    READ(*,*)                                             ! for debugging
 ...
 ENDIF
    ...
    !Set the time difference from the scale analysis time based on a 3 hour
    !data assimilation cycle.
    if( NINT(station(5) ) == -3 ) wk(8) = 0.0
    if( NINT(station(5) ) == -2 ) wk(8) = 3600.0
    if( NINT(station(5) ) == -1 ) wk(8) = -3600.0
    if( NINT(station(5) ) ==  0 ) wk(8) = 0.0
    if( NINT(station(5) ) ==  1 ) wk(8) = 3600.0
    if( NINT(station(5) ) ==  2 ) wk(8) = -3600.0
    if( NINT(station(5) ) ==  3 ) wk(8) = 0.0
