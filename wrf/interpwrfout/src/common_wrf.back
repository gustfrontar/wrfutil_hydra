...
!-----------------------------------------------------------------------
  ...
  INTEGER,PARAMETER :: iv3d_phb=13
  INTEGER,PARAMETER :: iv3d_pb=14
  !3D CHEM VARS
  INTEGER,PARAMETER :: iv3d_coant=15
  INTEGER,PARAMETER :: iv3d_cobck=16
  INTEGER,PARAMETER :: iv3d_cobbu=17
  !2D VARIABLES
  INTEGER,PARAMETER :: iv2d_ps=1
  INTEGER,PARAMETER :: iv2d_t2=2
  INTEGER,PARAMETER :: iv2d_q2=3
  INTEGER,PARAMETER :: iv2d_mu=4
  INTEGER,PARAMETER :: iv2d_mub=5
  !SPECIAL VARIABLES
  !2D PARAMETERS
...
!  REAL(r_size),ALLOCATABLE,SAVE :: fcori(:,:)
  ...
  !   U          V         W         T         P         PH      QV        QC        PHB      PB
  & .true. , .true. , .true. , .true. , .true. , .true. , .true. , .false. , .true. , .true. , .true. , .true. ,&
  !   QR       QCI      QS       QG     CO_ANT  CO_BCK    CO_BBU     PS        T2M     Q2M       MU       MUB
  & .true. , .true. , .true.  /)
  !  HFX_F    QFX_F   UST_F 
...
IF ( flag .EQ. '3d' )THEN
   ...
   SELECT CASE (iv)
      ...
      CASE(iv3d_w)
       ...
       nyvar=nlat-1
       nzvar=nlev
      CASE(iv3d_p,iv3d_pb,iv3d_phb,iv3d_ph)
       nxvar=nlon-1
       nyvar=nlat-1
       nzvar=nlev-1
...
IF ( flag .EQ. '2d' )THEN
   ...
   ALLOCATE(start(3),count(3))
      SELECT CASE (iv)
      CASE(iv2d_ps,iv2d_t2,iv2d_q2,iv2d_mu,iv2d_mub)
       nxvar=nlon-1
       nyvar=nlat-1
      CASE DEFAULT
...
IF ( flag .EQ. '3d' )THEN
   ...
   SELECT CASE (iv)
      ...
      CASE(iv3d_w)
       ...
       nyvar=nlat-1
       nzvar=nlev
      CASE(iv3d_p,iv3d_pb,iv3d_ph,iv3d_phb)
       nxvar=nlon-1
       nyvar=nlat-1
       nzvar=nlev-1
      CASE(iv3d_t,iv3d_qv,iv3d_qc,iv3d_qr,iv3d_qci,iv3d_qg,iv3d_qs,iv3d_coant,iv3d_cobck,iv3d_cobbu)
...
INTEGER      :: slev , elev , iv
     ...
     !Get data
     DO iv=1,nv3d
       tmp=REAL(variables3d(:,:,:,iv),r_sngl)
       CALL write_var_wrf(ncid,iv,slev,elev,tmp,'3d')
     ENDDO
     DO iv=1,nv2d
       tmp(:,:,1)=REAL(variables2d(:,:,iv),r_sngl)
