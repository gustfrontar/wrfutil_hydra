...
echo "cat ./rsl.error.* > ./real\${MEM}.log                 "  >> $SCRIPT
...
if [ $RUN_ONLY_MEAN -ne 1 ] ; then
 ...
 MNODE=1
  ...
  while [ $MNODE -le $NODES_PER_MEMBER ]; do
   #PROC=0
   #while [ $PROC -le $PROC_PER_NODE ]; do
    echo "( $NODE  ) " >> $vcoord_file
   # PROC=`expr $PROC + 1 `
   #done
   MNODE=`expr $MNODE + 1 `
...
rm $host_map_file
  local NODE=0
  local tmptot_node=`expr $TOTAL_NODES - 1 `
  while [ $NODE -le $tmptot_node ]; do
   PROC=1
   while [ $PROC -le $PROC_PER_NODE ]; do
...
#CREATE THE SCRIPT TO BE SUBMITED TO K COMPUTER
    ...
    if [ $ANALYSIS -eq 1  ] ; then
     ...
     while [  $M -le $MM ];do
      ...
      MEM=`ens_member $M `
         echo "mpiexec --vcoordfile ./WRF/vcoord_${JOB} ./SCRIPTS/WRF_REAL.sh \${BASEDIR}/WRF${MEM}/ &  " >> $local_script
         JOB=`expr $JOB + 1 `
         M=`expr $M + 1 `
      done
    ...
    #Only for ensemble forecast and for the case in which the LETKF analysis grid an the forecast grids are different.
     ...
     while [  $M -le $MM ];do
      ...
      while [  $JOB -le $MAX_BACKGROUND_JOBS -a $JOB -le $MM ];do
      MEM=`ens_member $M `
         echo "mpiexec --vcoordfile ./WRF/vcoord_${JOB} ./SCRIPTS/WRF_WRF.sh \${BASEDIR}/WRF${MEM} &  " >> $local_script
         JOB=`expr $JOB + 1 `
         M=`expr $M + 1 `
      done
      echo "time wait " >> $local_script
  ...
  if [ $ANALYSIS -eq 1 ] ; then  #If this is not a forecast JOB, add the analysis
     ...
     echo "time mpiexec -np ${TOTAL_NODES} --of-proc std-file ./letkf.exe " >> $local_script
