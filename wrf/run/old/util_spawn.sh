...
if [ $SYSTEM -eq 0 ];then
#COPY SPAWN
cp $SPAWN/spawn.exe     $TMPDIR/SPAWN
fi
...
#CREATE THE SCRIPT TO BE SUBMITED TO K COMPUTER
      ...
      echo "#PJM --mpi \"proc=1                        \"        ">> $local_script
    ...
    if [ $ANALYSIS -eq 1  ] ; then
     ...
     #COPY WRF MODEL AND DUMMY_MPI
      ...
      echo "#PJM --stgin \"${TMPDIR}/SPAWN/spawn.exe      ./WRF/ \" ">> $local_script
     ...
     echo "mpiexec -np 1 \${BASEDIR}/WRF/spawn.exe \${BASEDIR}/WRF/real.exe ${PROC_PER_MEMBER} \${BASEDIR} $INIMEMBER $MM  " >> $local_script
    ...
    #Only for ensemble forecast and for the case in which the LETKF analysis grid an the forecast grids are different.
     ...
     echo "mpiexec -np 1 \${BASEDIR}/WRF/spawn.exe \${BASEDIR}/WRF/wrf.exe ${PROC_PER_MEMBER} \${BASEDIR} $INIMEMBER $MM  " >> $local_script 
