...
if [ $RUN_ONLY_MEAN -ne 1 ] ; then
 ...
 local MNODE=1
  local vcoord_file=$TMPDIRL/vcoord_${JOB}
  rm -fr $vcoord_file
  while [ $MNODE -le $PROC_PER_MEMBER ]; do
  ...
  #Generate individual vcoord_files for each ensemble member.
  local JOB=1
  local MNODE=1
  local vcoord_file=$TMPDIRL/vcoord_${JOB}
  rm -fr $vcoord_file
...
#Generate host map
local host_map_file=$TMPDIRL/host_map
rm $host_map_file
  local NODE=0
  while [ $NODE -le $TOTAL_PROC ]; do
   echo "( $NODE ) " >> $host_map_file
   NODE=`expr $NODE + 1 `
  done
...
generate_run_cycle_script_k_mpmd () {
#CREATE THE SCRIPT TO BE SUBMITED TO K COMPUTER
      ...
      echo "#PJM --mpi \"proc=${TOTAL_PROC}           \"         ">> $local_script
      echo "#PJM --mpi \"rank-map-hostfile=${TMPDIR}/WRF/host_map\"   ">> $local_script
    ...
    if [ $ANALYSIS -eq 1  ] ; then
     ...
     command="time  mpiexec " 
     M=$INIMEMBER
     while [  $M -le $MM ];do
      JOB=1
      while [  $JOB -le $MAX_BACKGROUND_JOBS -a $JOB -le $MM ];do
      MEM=`ens_member $M `
         command="${command} -np ${PROC_PER_MEMBER} ./SCRIPTS/WRF_REAL.sh \${BASEDIR}/WRF${MEM}/ : "
         JOB=`expr $JOB + 1 `
         M=`expr $M + 1 `
      done
      echo $command >> $local_script
    ...
    #Only for ensemble forecast and for the case in which the LETKF analysis grid an the forecast grids are different.
     ...
     while [  $M -le $MM ];do
      ...
      while [  $JOB -le $MAX_BACKGROUND_JOBS -a $JOB -le $MM ];do
      MEM=`ens_member $M `
         command="${command} -np 1 ./SCRIPTS/WRF_INTERPANA.sh \${BASEDIR}/WRF${MEM}/ : "
         JOB=`expr $JOB + 1 `
         M=`expr $M + 1 `
      done
      echo ${command} >> $local_script
     ...
     while [  $M -le $MM ];do
      ...
      while [  $JOB -le $MAX_BACKGROUND_JOBS -a $JOB -le $MM ];do
      MEM=`ens_member $M `
         command="${command} -np 1 ./SCRIPTS/WRF_PRE.sh \${BASEDIR}/WRF${MEM}/ ${MEM} : "
         JOB=`expr $JOB + 1 `
         M=`expr $M + 1 `
      done
      echo ${command} >> $local_script
     ...
     while [  $M -le $MM ];do
      ...
      while [  $JOB -le $MAX_BACKGROUND_JOBS -a $JOB -le $MM ];do
      MEM=`ens_member $M `
         command="${command} -np ${PROC_PER_MEMBER} ./SCRIPTS/WRF_WRF.sh \${BASEDIR}/WRF${MEM}/ : "
         JOB=`expr $JOB + 1 `
         M=`expr $M + 1 `
      done
      echo ${command} >> $local_script
     ...
     while [  $M -le $MM ];do
      ...
      while [  $JOB -le $MAX_BACKGROUND_JOBS -a $JOB -le $MM ];do
      MEM=`ens_member $M `
         command="${command} -np 1 ./SCRIPTS/WRF_POST.sh \${BASEDIR}/WRF${MEM} ${MEM} : "
         JOB=`expr $JOB + 1 `
         M=`expr $M + 1 `
      done
      echo ${command} >> $local_script
   ...
   if [ $ANALYSIS -eq 1 ] ; then  #If this is not a forecast JOB, add the analysis
     ...
     echo "time mpiexec -np ${TOTAL_PROC} --of-proc std-file ./letkf.exe " >> $local_script
