...
GRIBDIR=$HOME/share/CFSR/

IE=20060101000000   #Experiment initial date.
EE=20091231180000   #Experiment end date.
   ...
   RUNNING=0
    while [ $RUNNING -le $MAXRUNNING -a $CDATE -le $EE ] ; do
     if [ ! -e $TMPDIR/$RUNNING/ ] ; then
      ...
      FILE=pgbh00.gdas.${CDATES}.grb2
      if [ ! -e $GRIBDIR/$FILE ] ; then
        #If file does not exist download it from the web.
        $GRIBDIR/download_file.sh $CDATES
      fi
     
      ln -sf $GRIBDIR/pgbh00.gdas.${CDATES}.grb2 ./$CDATE.grib
