...
for key in ctl_vars  :
  ...
  skew_regional_mean[key]=np.zeros([ntimes,nregs])
  skew_regional_max[key] =np.zeros([ntimes,nregs])
  skew_regional_min[key] =np.zeros([ntimes,nregs])

  skew_time_mean[key]=np.zeros([nx,ny,nlev])
  skew_time_std[key]=np.zeros([nx,ny,nlev])
...
while ( ctime <= etime ):
    ...
    skew=bio.read_data_scale_2(my_file,nx,ny,nz,ctl_vars,ctl_inirecord,ctl_endrecord,dtypein='f4',undef_in=undef_in,undef_out=undef_out)
 ...
 #For each horizontal grid point accumulate the vertical profile discriminating between rain and no-rain
    ...
    for key in skew :
      ...
      for ilev in range(0,nlev) :     
         ...
         my_skew=skew[key][buffer_zone_size:-buffer_zone_size,buffer_zone_size:-buffer_zone_size,ilev]
         my_sigma=sigma[key][buffer_zone_size:-buffer_zone_size,buffer_zone_size:-buffer_zone_size,ilev]   
         my_skew=my_skew/np.power(my_sigma,1.5)   
    
         vprofile[key][ilev,it,0]=np.nansum(np.abs(my_skew[rain_mask]))
         vprofile[key][ilev,it,1]=np.nansum(np.abs(my_skew[norain_mask]))
...
for key in vprofile  :
    ...
    print("Plotting the skew Vertical profile rain")
    varname= key
    ...
    print( 'Generationg the following figure : ' + 'Figure_skew_profile_rain_var_' + key + '.png' )
    plt.savefig( my_plotdir +  'Figure_skew_profile_var_' + key + '.png' )

    my_profile=vprofile[key] 

    print("Plotting the skew Vertical profile no rain")
    varname= key
    ...
    print( 'Generationg the following figure : ' + 'Figure_skew_profile_norain_var_' + key + '.png' )
    plt.savefig( my_plotdir +  'Figure_skew_profile_var_' + key + '.png' )
