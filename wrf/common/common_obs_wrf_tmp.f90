...
!=======================================================================
  ...
  INTEGER , PARAMETER :: id_reflectivity_obs_c=4001
  INTEGER , PARAMETER :: id_reflectivity_obs_x=4010
 ...
 TYPE obs_type
  REAL(r_size),ALLOCATABLE,SAVE :: elm(:)
  REAL(r_size),ALLOCATABLE,SAVE :: lon(:)
  REAL(r_size),ALLOCATABLE,SAVE :: lat(:)
  REAL(r_size),ALLOCATABLE,SAVE :: lev(:)
  REAL(r_size),ALLOCATABLE,SAVE :: dat(:)  !Obs value
  REAL(r_size),ALLOCATABLE,SAVE :: datf(:) !Forecast interpolated to obs location
  REAL(r_size),ALLOCATABLE,SAVE :: err(:)
  REAL(r_size),ALLOCATABLE,SAVE :: typ(:)
  REAL(r_size),ALLOCATABLE,SAVE :: lot(:)
  REAL(r_size),ALLOCATABLE,SAVE :: ri(:)
  REAL(r_size),ALLOCATABLE,SAVE :: rj(:)
  REAL(r_size),ALLOCATABLE,SAVE :: rk(:)
  REAL(r_size),ALLOCATABLE,SAVE :: dep(:)
  REAL(r_size),ALLOCATABLE,SAVE :: radar(:)
  REAL(r_size),ALLOCATABLE,SAVE :: az(:),ra(:),el(:)
!  REAL(r_size),ALLOCATABLE,SAVE :: qc(:)
...
!-----------------------------------------------------------------------
! Allocate obs_type arrays
!-----------------------------------------------------------------------

SUBROUTINE allocate_obs_type(var,varsize)
IMPLICIT NONE
TYPE(OBS_TYPE) :: var
INTEGER        :: varsize

  ALLOCATE( var%elm(varsize) )
  ALLOCATE( var%lon(varsize) )
  ALLOCATE( var%lat(varsize) )
  ALLOCATE( var%lev(varsize) )
  ALLOCATE( var%dat(varsize) )
  ALLOCATE( var%datf(varsize) )
  ALLOCATE( var%err(varsize) )
  ALLOCATE( var%typ(varsize) )
  ALLOCATE( var%lot(varsize) )
  ALLOCATE( var%ri(varsize) )
  ALLOCATE( var%rj(varsize) )
  ALLOCATE( var%rk(varsize) )
  ALLOCATE( var%dep(varsize) )
!  ALLOCATE( var%qc(varsize) )
  ALLOCATE( var%radar(varsize) )
  ALLOCATE( var%az(varsize) )
  ALLOCATE( var%ra(varsize) )
  ALLOCATE( var%el(varsize) )
...
INTEGER, INTENT(OUT) :: iobs_out
 ...
 SELECT CASE(obstype_in)
      ...
      CASE(id_tclon_obs,id_tclat_obs,id_tcmip_obs,id_tcr15_obs,id_tcr25_obs)
        iobs_out=7
      CASE(id_reflectivity_obs_c,id_reflectivity_obs_x)
        iobs_out=8
...
SUBROUTINE Trans_XtoY(elm,typ,ri,rj,rk,raz,rel,v3d,v2d,yobs)
  ...
  CASE(id_reflectivity_obs_x,id_reflectivity_obs_c,id_radialwind_obs)
     ...
     IF( NINT(elm) .EQ. id_reflectivity_obs_c )method_ref_calc=2
     IF( NINT(elm) .EQ. id_reflectivity_obs_x )method_ref_calc=3

     CALL calc_ref_vr(qvr,qcr,qrr,qcir,qsr,qgr,ur,vr,wr,tr,pr,raz,rel,method_ref_calc,ref,radialv,att_coef)

     IF( NINT(elm) .EQ. id_reflectivity_obs_c .OR. NINT(elm) .EQ. id_reflectivity_obs_x )THEN
         yobs=ref
  ...
  DO n=1,nn
    ...
    CASE(id_pwv_obs)
     ...
     CASE DEFAULT
      ...
      WRITE(6,*)NINT(elm(n)),dep(n),qc(n),n  
    END SELECT
  END DO
  IF(iu == 0) THEN
    rmse_u = undef
...
SUBROUTINE write_obs_forecast(cfile,nn,elem,rlon,rlat,rlev,odat,odatf,oerr,otyp,oqc)
  USE common
  USE common_wrf
  USE common_obs_wrf

  !This writes the observation as well as the forecast interpolated to the
  !observation location and transformed to the observation type.
  ...
  REAL(r_size),INTENT(IN) :: odatf(nn)
  REAL(r_size),INTENT(IN) :: oerr(nn)
  REAL(r_size),INTENT(IN) :: otyp(nn)
  REAL(r_size),INTENT(IN) :: oqc(nn)
  REAL(r_sngl) :: wk(8)
  ...
  DO n=1,nn
   ...
   !We write all the observations that are not undefined
   IF ( oqc(n) == 0.0d0 )THEN  
    wk(5)=undefs
    wk(6)=undefs
   ELSE
    wk(5)=REAL(odat(n),r_sngl)   !Observation
    wk(6)=REAL(odatf(n),r_sngl)  !Forecast interpolated to the observation location
   ENDIF 
    wk(7)=REAL(oerr(n),r_sngl)
    wk(8)=REAL(otyp(n),r_sngl)
    SELECT CASE(NINT(wk(1)))
    CASE(id_u_obs)
      wk(4) = wk(4) /  100.0 ! hPa <- Pa
    CASE(id_v_obs)
    ...
    CASE(id_ps_obs)
      wk(5) = wk(5) / 100.0 ! hPa <- Pa
      wk(6) = wk(6) / 100.0 ! hPa <- Pa
      wk(7) = wk(7) / 100.0 ! hPa <- Pa
    CASE(id_rh_obs)
      wk(4) = wk(4) / 100.0 ! hPa <- Pa
      wk(5) = wk(5) / 0.01  ! percent output
      wk(6) = wk(6) / 0.01  ! percent output
      wk(7) = wk(7) / 0.01  ! percent output
    END SELECT
    WRITE(iunit)wk
  END DO
  CLOSE(iunit)

  WRITE(6,*)"A total number of ",writenobs," observations have been written"
RETURN
