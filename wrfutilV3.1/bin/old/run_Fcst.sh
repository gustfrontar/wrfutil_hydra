...
if [ $FILE_COUNTER -eq 0 ] ; then 
   ...
   echo "We found all the required wrfout files for member $MIEM, skiping REAL and WRF."
   ERROR=0
   ...
   if [ $ERROR -eq 0 ] ; then
      ...
      if [ $ERROR -gt 0 ] ; then
         ...
         OUTPUTPATH="$HISTDIR/FCST/$(date -u -d "$INI_DATE_FCST UTC" +"%Y%m%d%H%M%S")/$MIEM/"
         mkdir -p $OUTPUTPATH
