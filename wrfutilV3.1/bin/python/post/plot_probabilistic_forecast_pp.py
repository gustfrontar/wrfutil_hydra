...
VARIABLE_NAME = 'PP'                # The original variable name (e.g., PRCP, TEMP, etc.)
...
RQPE_DIR='/home/ra000007/a04037/data/DATOS_RADAR/RQPE/30min/'
...
def plot_probability_field(data_array,obs,obs_lon,obs_lat,obs_mask, threshold, init_time, valid_time, plot_dir):
    ...
    ax.set_extent([obs_lat.min(),obs_lat.max(),obs_lon.min(),obs_lon.max()], crs=ccrs.PlateCarree())
    # Map features
...
while current_init_time <= end_init_time:
    ...
    with xr.open_dataset(INPUT_FILENAME) as ds:
       ...
       for time_step in prob_data['time'].values:
          ...
          obs_file = RQPE_DIR + '/RQPE_RMA2_30min_' + valid_time.strftime('%Y_%m_%d_%H_%M') + '.nc'
          # Load the observed data. 
          if os.path.exists( obs_file ) :
             print('Will read the following file : ' + obs_file )
             tmp_data = nc.Dataset( obs_file )
             tmp_precip =  tmp_data.variables['rain_rate'][:].squeeze()
             tmp_precip[ tmp_precip < 0.0 ] = 0.0 
             obs_precip_acc.append(  tmp_precip )
             obs_lat = tmp_data.variables['lat'][:]
             obs_lon = tmp_data.variables['lon'][:]
             print( len( obs_precip_acc ) )
             accum_precip = np.array( obs_precip_acc ).sum( axis=0 ).squeeze()
             print( accum_precip.shape )
             obs_mask = ( accum_precip == -9999.0 ).astype( float ) 
             accum_precip[  obs_mask.astype(bool)  ] = np.nan
             accum_precip[ accum_precip < 0.0 ] = 0.0
             tmp_data.close()
             # Loop over all thresholds
             for threshold_value in prob_data['threshold'].values:
                ob_to_plot =  ( accum_precip > threshold_value ).astype(float)
