    ...
    for idx, precip_threshold in enumerate(thresholds):
        ...
        try:
            ...
            tpr_list.append(tpr)
             ...
             # Catch cases where roc_curve might fail (e.g., if y_true is all 0s or all 1s 
             # over the masked area, although the unique check above tries to prevent this)
             print(f"Error computing ROC metrics for {precip_threshold}: {e}. Skipping.")
             auc_values.append(np.nan)
...
def compute_reliability_curve(
    ...
    for idx, precip_threshold in enumerate(thresholds):
        ...
        # Store results for this threshold (filter out NaNs for bins with no samples)
        valid_bins = ~np.isnan(mean_prob_in_bin)
        
        if np.sum(valid_bins) > 0:
            mean_forecast_prob_list.append(mean_prob_in_bin[valid_bins])
            observed_freq_list.append(observed_frequency_in_bin[valid_bins])
        else:
            print(f"Warning: No samples fell into any bin for threshold {precip_threshold}. Skipping storage.")
