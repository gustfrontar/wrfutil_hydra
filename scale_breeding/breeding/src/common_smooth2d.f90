MODULE common_smooth2d
  ...
  REAL(r_size), DIMENSION(:,:),     ALLOCATABLE :: v2d, w2d          ! raw data,  filtered result
  REAL(r_size)                                  :: fn, rspval        ! cutoff freq/wavelength, spval

  REAL(r_size), DIMENSION(:,:),     ALLOCATABLE :: dec2d             ! working array
  REAL(r_size), DIMENSION(:),       ALLOCATABLE :: dec, de           ! weight in r8, starting index 0:nband
...
CONTAINS
  SUBROUTINE filter_2d(inputvar2d,outputvar2d,mask,lambda,ctyp,nx,ny)
  IMPLICIT NONE
  REAL(r_size) :: inputvar2d(nx,ny),outputvar2d(nx,ny)
  INTEGER      :: mask(nx,ny)
  INTEGER      :: nx,ny
  INTEGER      :: lambda
  CHARACTER(LEN=256)                            :: ctyp              ! filter type
  ...
  SUBROUTINE filterinit(kfilter, pfn, kband)
    ...
    REAL(r_size),    INTENT(in) :: pfn      ! filter cutoff frequency/wavelength
  ...
  SUBROUTINE filter (kfilter, px, kpx, py)
    ...
    IMPLICIT NONE
    INTEGER,                 INTENT(in) :: kfilter ! filter number
    REAL(r_size), DIMENSION(:,:),    INTENT(in) :: px      ! input data
    INTEGER, DIMENSION(:,:), INTENT(in) :: kpx     ! validity flag
    REAL(r_size), DIMENSION(:,:),   INTENT(out) :: py      ! output data
    !!----------------------------------------------------------------------
    SELECT CASE ( kfilter)
    CASE ( jp_lanc )
  ...
  SUBROUTINE initlanc(pfn, knj)
    ...
    !!----------------------------------------------------------------------
    IMPLICIT NONE
    REAL(r_size),    INTENT(in) :: pfn  ! cutoff freq/wavelength
    INTEGER, INTENT(in) :: knj  ! bandwidth
    INTEGER             :: ji   ! dummy loop index
    REAL(r_size)                :: dl_pi, dl_ey, dl_coef
    !!----------------------------------------------------------------------
    dl_pi   = ACOS(-1.d0)
  ...
  SUBROUTINE lislanczos2d(px, kiw, py, kpi, kpj, pfn, knj)
    ...
    !!----------------------------------------------------------------------
    IMPLICIT NONE
    REAL(r_size),    DIMENSION(:,:), INTENT(in ) :: px               ! input array
    INTEGER, DIMENSION(:,:), INTENT(in ) :: kiw              ! flag input array
    REAL(r_size),    DIMENSION(:,:), INTENT(out) :: py               ! output array
    INTEGER,                 INTENT(in ) :: kpi, kpj         ! size of input/output
    REAL(r_size),                    INTENT(in ) :: pfn              ! cutoff frequency/wavelength
    ...
    REAL(r_size), DIMENSION(kpi,kpj)             :: dl_tmpx, dl_tmpy
    REAL(r_size)                                 :: dl_yy, dl_den
    !!----------------------------------------------------------------------
    inxmin   =  knj
    inxmaxi  =  kpi-knj+1
    inymin   =  knj
  ...
  SUBROUTINE lishan2d(px, kiw, py, korder, kpi, kpj)
    ...
    REAL(r_size), DIMENSION(:,:), ALLOCATABLE    :: ztmp
    !!----------------------------------------------------------------------
  ...
  SUBROUTINE lisshapiro1d(px, kiw, py, korder, kpi, kpj)
    ...
    REAL(r_size), PARAMETER                      :: rp_aniso_diff_XY = 2.25 !  anisotrope case
    REAL(r_size)                                 :: zalphax, zalphay, znum
    REAL(r_size), DIMENSION(:,:), ALLOCATABLE    :: ztmp , zpx , zpy, zkiw
